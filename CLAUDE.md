...
- **XKTExportCommand stub**: `StingTools/BIMManager/SpeckleLinkCommands.cs` (new file) with `[Transaction(ReadOnly)]` `IExternalCommand`. Writes a minimal XKT-shaped JSON envelope (xktVersion=10, empty positions/indices/meshes/entities graph, generator metadata) to `{BIMManagerDir}/model.xkt` so the viewer loads without error before the full IFC→XKT pipeline ships. Reads vertex coordinates from an optional `speckle_snapshot.json` sidecar if present. Marked as TODO for replacement with `ifcconvert` or `@xeokit/xeokit-convert` output once the P7 Model Derivative job is wired.
