...
- **17 healthcare validators** under `Core/Validation/Healthcare/` gated through `HealthcareValidatorGate` against `PRJ_ORG_HEALTH_PACK_PROFILE_TXT` (FULL / ACUTE / COMMUNITY / DENTAL / IMAGING-ONLY / MENTAL-HEALTH); `CLN_ROOM_CLASS_TXT` resolves through the canonical `RoomClassCodes` registry (`Data/HEALTHCARE_ROOM_CLASSES.json`), and `RoomClassCodeValidator` flags any non-canonical room-class value
...
5. Healthcare Pack tables have creating migrations and all five `DbSet`s are registered (`HealthcarePressureLogs`, `HealthcareMgasVerifications`, `HealthcareAntiLigatureAudits`, `HealthcareRdsSnapshots` via `20260515000000_HealthcarePack.cs`; `HealthcareWaterLogs` via `20260627000000_HealthcareWaterLog.cs`); `PenetrationSignoffs` has its own creating migration (`20260517000010_CreatePenetrationSignoffs.cs`), ordered before `20260601000000_CrossHostIdentityFields`. These healthcare migrations are documentation-DDL (no `[Migration]` attribute) — dev/local stacks build schema from the model via `CreateTables()`; repairing the prod EF migration pipeline is backlog P3-2
6. The Healthcare tab is integrated into the **BIM Coordination Centre** (`UI/BIMCoordinationCenter.cs` — `BuildHealthcareTab()`, appended when `PRJ_ORG_HEALTH_FACILITY_TYPE_TXT` is set), surfacing the pressure / MGPS / anti-ligature / RDS dashboards (live off the server `GetHealthcareDashboardAsync` endpoint) and validator dispatch. Healthcare commands also dispatch via `WorkflowEngine.ResolveCommand` / `StingCommandHandler` button tags
