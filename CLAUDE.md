...
└── StingTools/                         # C# project root
    ...
    │   ├── ComplianceScan.cs          # Cached compliance scanner for live dashboard (RAG status, issue tracking)
    │   ├── StingAutoTagger.cs         # IUpdater real-time auto-tagger + AutoTaggerToggleCommand
    │   └── WorkflowEngine.cs          # Workflow orchestration engine (JSON presets, cancellation, atomic rollback) + 3 commands
    │
    ...
    │
    ├── UI/                             # WPF dockable panel UI + project wizard + progress dialog (5 C# files + 2 XAML, ~8,369 lines)
    ...
    │   ├── StingProgressDialog.cs     # Reusable modeless WPF progress dialog (progress bar, ETA, cancel support)
...
### Temp Panel (9 pulldown groups, 57 commands)
| Group | Commands | Description |
|-------|----------|-------------|
...
| Schedules | **Full Auto-Populate**, Batch Create, Auto-Populate (Tokens Only), Evaluate Formulas, Export CSV, Corporate Title Block Schedule, Drawing Register Schedule | Schedule creation + zero-input automation + corporate schedules (7 commands) |
...
| **Data Pipeline** | Validate Template, Dynamic Bindings, Schema Validate, BOQ Export, Template VG Audit, IFC Property Map, BEP Compliance, Clash Detection, IFC Export, Excel BOQ Import, Keynote Sync | Data integrity validation + dynamic CSV-driven bindings + BOQ export + IFC/BEP/clash integration (11 commands) |
...
| `Core/StingLog.cs` | 0 (logger + EscapeChecker utility) | 117 |
...
| `Core/StingAutoTagger.cs` | 1 (AutoTaggerToggleCommand) + IUpdater | 223 |
...
| `Temp/MasterSetupCommand.cs` | 3 (MasterSetup + 2 additional) | 345 |
| `Temp/ProjectSetupCommand.cs` | 4 (ProjectSetup + 3 additional) | 1,126 |
...
| `Temp/TemplateManagerCommands.cs` | 19 (AutoAssign, Audit, Diff, Compliance, AutoFix, SyncOverrides, FillPatterns, LineStyles, ObjectStyles, TextStyles, DimStyles, VGOverrides, BatchFamilyParams, TemplateSchedules, SetupWizard, CloneTemplate, BatchVGReset + 2 additional) + TemplateManager engine | 3,434 |
...
| `UI/StingCommandHandler.cs` | 0 (IExternalEventHandler — dispatches 400+ button tags to 250 commands + ~96 inline helpers) | 4,533 |
| `UI/StingDockPanel.xaml.cs` | 0 (WPF code-behind) | 223 |
| `UI/StingDockPanelProvider.cs` | 0 (IDockablePaneProvider) | 37 |
| `UI/StingProgressDialog.cs` | 0 (reusable WPF progress dialog) | 211 |
...
- Also contains `EscapeChecker` static class (Win32 `GetAsyncKeyState` for Escape key detection)
...
- Lightweight cached compliance scanner for live dashboard display
- `Scan(doc, forceRefresh)` — quick scan returning `ComplianceResult` (cached for 30s)
- `ComplianceResult` tracks: TotalElements, TaggedComplete, TaggedIncomplete, Untagged, FullyResolved, IssuesByType
- `CompliancePercent`, `StrictPercent` — computed compliance percentages
- `RAGStatus` — Red/Amber/Green status (thresholds: 50%/80%)
- `StatusBarText` — formatted one-line summary for status bar display
- `InvalidateCache()` — call after tagging operations to force refresh

### `StingAutoTagger` (IUpdater) — `Core/StingAutoTagger.cs` (223 lines)
- Real-time auto-tagger using Revit's `IUpdater` API — tags newly placed elements immediately
- Registers `DocumentChanged` listener for element additions in 22 tagged categories
- Starts **disabled** by default — user toggles on/off via `AutoTaggerToggleCommand`
- Uses `TokenAutoPopulator.PopulateAll` + `TagConfig.BuildAndWriteTag` inline
- `HashSet<long>` of processed IDs prevents re-trigger loops (auto-trims at 10,000)
- `Register(app)` — called from `StingToolsApp.OnStartup()`
- `Unregister()` — called from `StingToolsApp.OnShutdown()`
- `Toggle()` — enables/disables the updater
- Contains `AutoTaggerToggleCommand` (1 command)

### `WorkflowEngine` (internal static) — `Core/WorkflowEngine.cs` (551 lines)
- Workflow orchestration engine for zero-touch project delivery
- Chains named command sequences from JSON preset files (`WORKFLOW_*.json` in data/ directory)
- `WorkflowPreset` model: name, description, ordered list of `WorkflowStep` entries
- `WorkflowStep` model: commandTag (matching StingCommandHandler dispatch), label, optional flag, condition
- **Features**: per-step progress reporting, Escape key cancellation, atomic `TransactionGroup` with rollback on failure
...
- Win32 `GetAsyncKeyState` wrapper for Escape key detection during batch operations
- `IsEscapePressed()` — returns true if Escape key is held down
- Used by `WorkflowEngine` and batch commands for cancellation support

### `StingProgressDialog` — `UI/StingProgressDialog.cs` (211 lines)
- Reusable modeless WPF progress dialog for batch operations
- Shows progress bar, element count (`N0` formatted), estimated time remaining, cancel button
- Thread-safe updates via `Dispatcher.Invoke` (updates UI every 50 elements for performance)
- Win32 `GetAsyncKeyState` for Escape key detection (works even when Revit has focus)
- `Show(title, total)` — create and display the dialog
- `Increment(statusMessage)` — advance progress by 1 with ETA calculation
- `IsCancelled` — check for cancel button or Escape key
...
| `StingCommandHandler` | `UI/StingCommandHandler.cs` | `IExternalEventHandler` — dispatches 400+ dockable panel button tags to 250 command classes + ~96 inline helpers on the Revit API thread |
...
| `ComplianceScan` | `Core/ComplianceScan.cs` | Lightweight cached compliance scanner: RAG status, issue tracking, status bar text, 30s cache |
| `WorkflowEngine` | `Core/WorkflowEngine.cs` | Workflow orchestration: JSON preset loading, command dispatch, TransactionGroup rollback, 3 built-in presets |
...
| `StingCommandHandler` | `UI/StingCommandHandler.cs` (4,382 lines) | `IExternalEventHandler` — maps 400+ button Tag strings to 250 command classes + ~96 inline helpers, ensures Revit API calls run on the main thread |
...
| `StingProgressDialog` | `UI/StingProgressDialog.cs` (211 lines) | Reusable modeless WPF progress dialog with progress bar, ETA, cancel button, Escape key detection |
...
| ~~**No cancellation support**~~ | All batch commands | **DONE** — `EscapeChecker` (Win32 GetAsyncKeyState) + `StingProgressDialog` (WPF progress bar with Cancel button + ETA). `WorkflowEngine` checks escape between steps. | Done |
...
| ~~Batch command chaining / workflow presets~~ | **DONE** — `WorkflowEngine` with JSON presets, 3 built-in workflows (ProjectKickoff/DailyQA/DocumentPackage), cancellation, atomic rollback | Done |
...
32. **Cancellation support** — `EscapeChecker` (Win32 GetAsyncKeyState) + `StingProgressDialog` (WPF progress bar, ETA, cancel button)
33. **Workflow orchestration engine** — `WorkflowEngine` with JSON presets, 3 built-in workflows (ProjectKickoff 26-step, DailyQA 6-step, DocumentPackage 6-step), per-step progress, Escape cancellation, atomic `TransactionGroup` rollback
34. **Real-time auto-tagger** — `StingAutoTagger` IUpdater for zero-touch tagging on element placement (22 categories, toggle on/off)
35. **Compliance scanner** — `ComplianceScan` with cached RAG status, issue tracking, status bar integration
36. **IFC/BEP/Clash commands** — `ExportIfcPropertyMapCommand`, `ValidateBepComplianceCommand`, `ClashDetectionCommand`, `IFCExportCommand`, `ExcelBOQImportCommand`, `KeynoteSyncCommand` (6 new data pipeline commands)
37. **Corporate schedules** — `CorporateTitleBlockScheduleCommand`, `DrawingRegisterScheduleCommand` for corporate title block and drawing register
