...
└── StingTools/                         # C# project root
    ...
    │
    ├── Core/                           # Shared infrastructure (9 files, ~8,944 lines)
    │   ├── StingToolsApp.cs            # IExternalApplication — ribbon UI + dockable panel registration + ToggleDockPanelCommand
    │   ├── StingLog.cs                 # Thread-safe file logger (Info/Warn/Error/Shutdown)
    ...
    │   ├── WorkflowEngine.cs           # Workflow orchestration engine — JSON preset chaining, atomic TransactionGroup, cancellation (3 commands)
    │   ├── StingAutoTagger.cs          # IUpdater — real-time auto-tagging on element placement, toggle on/off (1 command)
    │   └── ComplianceScan.cs           # Lightweight cached compliance scanning for live dashboard (30s cache TTL)
    │
    ...
    │
    ├── UI/                             # WPF dockable panel UI + project wizard (5 C# files + 2 XAML, ~9,802 lines)
    ...
    │   ├── StingProgressDialog.cs      # Reusable modeless WPF progress window — progress bar, ETA, cancel button, Dispatcher-safe
    ...
    │   ├── DocAutomationExtCommands.cs # Batch views/sheets/sections/elevations, doc package, scope boxes, templates, drawing register, browser organizer, title block
...
| Schedules | **Full Auto-Populate**, Batch Create, Auto-Populate (Tokens Only), Evaluate Formulas, Export CSV, Grid Populate | Schedule creation + zero-input automation (6 commands) |
...
| **Data Pipeline** | Validate Template, Dynamic Bindings, Schema Validate, BOQ Export, Template VG Audit, Clash Detection, IFC Export, Excel Import, Anomaly Fix, Batch Clash, Drawing Register Schedule | Data integrity + clash detection + IFC/Excel interop + anomaly resolution (11 commands) |
...
| `Core/ComplianceScan.cs` | 0 (cached compliance scanning utility) | 144 |
...
| `Docs/DocAutomationExtCommands.cs` | 11 (BatchViews, BatchSheets, DependentViews, ScopeBox, ViewTemplate, DocPackage, Sections, Elevations, DrawingRegister, BrowserOrganizer, TitleBlock) | 2,580 |
...
| `Tags/BatchTagCommand.cs` | 3 (BatchTag + variants) | 534 |
...
| `Organise/TagOperationCommands.cs` | 40 (7 Tag Ops + 14 Leaders + 7 Analysis + 5 Annotation Color + 5 Tag Appearance + 1 SwapTagType + 1 extra + LeaderHelper + AnnotationColorHelper) | 4,369 |
...
| `Temp/ScheduleCommands.cs` | 6 (FullAutoPopulate, BatchSchedules, AutoPopulate, ExportCSV + extras) + ScheduleHelper | 1,997 |
...
| `Temp/DataPipelineCommands.cs` | 11 (ValidateTemplate, DynamicBindings, SchemaValidate, BOQExport, TemplateVGAudit + ClashDetection, IFCExport, ExcelImport, AnomalyFix, BatchClash, DrawingRegisterSchedule) | 2,980 |
| `UI/StingCommandHandler.cs` | 1 (IExternalEventHandler — dispatches 400+ button tags to 251 commands + ~96 inline helpers) | 4,533 |
| `UI/StingDockPanel.xaml.cs` | 0 (WPF code-behind) | 223 |
| `UI/StingDockPanelProvider.cs` | 0 (IDockablePaneProvider) | 37 |
| `UI/StingProgressDialog.cs` | 0 (reusable modeless progress window) | 211 |
...
### `ParamRegistry` (static) — `Core/ParamRegistry.cs` (1,070 lines)
...
- Chains named command sequences from JSON preset files (stored in `data/` directory)
...
- Commands: `WorkflowPresetCommand`, `ListPresetsCommand`, `CreatePresetCommand`

### `StingAutoTagger` (IUpdater) — `Core/StingAutoTagger.cs` (223 lines)
- Real-time auto-tagging via `IUpdater` / `DocumentChanged` listener
- Auto-tags newly placed elements the moment they appear in the model
...
- `AutoTaggerToggleCommand` — toggles auto-tagging on/off

### `ComplianceScan` (static) — `Core/ComplianceScan.cs` (144 lines)
- Lightweight cached compliance scanning for live dashboard display
- Quick tag completeness stats without full ISO validation overhead
- Thread-safe cached `ComplianceResult` with 30-second TTL
- Tracks: TotalElements, TaggedComplete, TaggedIncomplete, Untagged, FullyResolved, IssuesByType
- Used by dockable panel status bar for real-time compliance percentage
...
- Uses Win32 `GetAsyncKeyState` for Escape key cancellation
- Usage: `StingProgressDialog.Show("Batch Tag", totalElements)` → `progress.Increment(msg)` → `progress.Close()`
...
| `StingProgressDialog` | `UI/StingProgressDialog.cs` (211 lines) | Reusable modeless WPF progress window: progress bar, element count, ETA, cancel button, Dispatcher-safe |
...
| ~~**No cancellation support**~~ | All batch commands | **DONE** — `StingProgressDialog` provides reusable modeless WPF progress window with cancel button, Escape key support, and ETA. `WorkflowEngine` supports Escape key cancellation between steps. | Done |
...
| ~~Batch command chaining / workflow presets~~ | **DONE** — `WorkflowEngine` with JSON preset chaining, atomic TransactionGroup, built-in presets | Done |
| ~~Cancellation support for batch commands~~ | **DONE** — `StingProgressDialog` with cancel button, Escape key, ETA display | Done |
| ~~Real-time auto-tagging~~ | **DONE** — `StingAutoTagger` (IUpdater) for zero-touch tagging on element placement | Done |
| ~~Live compliance dashboard~~ | **DONE** — `ComplianceScan` cached scanning for real-time status bar display | Done |
...
32. **Cancellation support** — `StingProgressDialog` (reusable modeless WPF progress window) with cancel button, Escape key, ETA display
33. **Batch command chaining / workflow presets** — `WorkflowEngine` with JSON preset chaining, atomic TransactionGroup, built-in presets (ProjectKickoff, DailyQA, DocumentPackage)
34. **Real-time auto-tagging** — `StingAutoTagger` (IUpdater) for zero-touch auto-tagging on element placement
35. **Live compliance dashboard** — `ComplianceScan` cached compliance scanning for real-time status bar display
36. **Clash detection** — `ClashDetectionCommand` in `DataPipelineCommands.cs` for element interference checks
37. **IFC export** — `IFCExportCommand` for ISO 19650-compliant IFC export
38. **Excel import** — `ExcelImportCommand` for spreadsheet data import via ClosedXML
39. **Anomaly fix** — `AnomalyFixCommand` for automated data anomaly resolution
40. **Title block management** — `TitleBlockCommand` in `DocAutomationExtCommands.cs` for corporate title block creation
41. **Drawing register schedule** — `DrawingRegisterScheduleCommand` for automated drawing register creation

#### Next Priorities

42. **Configurable tag format** — Separator, padding, segments via project_config.json
43. **Dynamic discipline bindings** — Load CATEGORY_BINDINGS.csv (10,661 entries) to replace hardcoded `DisciplineBindings`
44. **Family parameter auto-binding** — Load FAMILY_PARAMETER_BINDINGS.csv (4,686 entries) for family-level validation
