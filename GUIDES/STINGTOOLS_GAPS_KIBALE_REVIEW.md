...
> **57 of 103 entries are closed** on `claude/kibale-integration` — 9 partial, 1 held, 2 blocked, 3 recommend, 31 open. Generated by `tools/register_count.py` from the table rows — do not hand-edit, it has drifted three times when maintained by hand.
...
| **G-35 masonry mortar has a THIRD owner, 6.6x out** | **P0** | **open — measured, not fixed** | new, found by the formula-vs-formula sweep G-34 prompted. `CST_CALC_MORTAR_M3 = CST_CALC_BLOCKS_NR * 0.0063` still computes masonry mortar, after G-15 deleted `CST_S_MAS_MORTAR_VOLUME_CU_M` and gave `CompoundTakeoff` the quantity. Worked case: 577.5 blocks x 0.0063 = **3.638 m³** against CompoundTakeoff's 50x0.011 = **0.550 m³** — **6.62x** (7.52x on net area). Sanity check says the formula is the wrong one: 0.0063 m³/block is **39% of a 400x200x200 block's own volume**, where 10-15% is typical; CompoundTakeoff implies 0.00088 m³/block. Larger than G-15's 2.27x |
