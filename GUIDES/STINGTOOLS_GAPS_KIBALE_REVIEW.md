...
> **65 of 121 entries are closed** on `claude/kibale-integration` — 9 partial, 1 held, 2 blocked, 1 rule, 3 recommend, 40 open. Generated by `tools/register_count.py` from the table rows — do not hand-edit, it has drifted three times when maintained by hand.
...
| **G-45 two controlled vocabularies for the same product** | **P0** | **open — this is why 2.1 alone changes no door price** | new. The tag pipeline's `ProdMap` and the rate table's `MAT_CODE` column are independent lists that disagree: **Doors DR vs DOR**, **Air Terminals GRL vs ATU**; Windows agree (WIN/WIN) only by luck. So even with precedence fixed, a door's PROD code `DR` finds nothing — the rate row is keyed `DOR`. Sixth dual-vocabulary defect after G-15/G-34/G-35/the two binders/G-44 |
| **G-46 the rate table has no product tier at all** | **P0** | **open** | new. 43 rows, **one per category**, indexed twice — `LoadCsvRatesUncached` calls `Put(cols[0])` for Category **and** `Put(cols[1])` for MAT_CODE, so both keys point at the SAME row and return the SAME rate. Only *Air Terminals* has 2 rows. There is no product-level granularity to reach, which is why the precedence fix is necessary but not sufficient. This is 2.2 |
...
| **G-50 the CSV already has a product tier, disguised as categories** | **P1** | open | new — the 16 with no `ProdMap` entry are **not Revit categories**: `SPD Type 1`/`1+2`/`2`/`3`, `Pipe Hanger`, `Pipe Sleeve`, `Test Clamps`, `Earth Bar`, `Earth Electrodes`, `Bonding Conductor`, `Down Conductors`, `Pipe Systems`, `Duct Systems`, `Pipe Insulations`, `Duct Insulations`, `Structural Foundation`. They are **products** written into the category column. So 3.1 is not "add a tier" — it is "separate the tier that is already there", which changes the migration design |
