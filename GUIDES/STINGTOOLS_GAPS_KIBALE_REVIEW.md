...
> **72 of 127 entries are closed** on `claude/kibale-integration` — 10 partial, 1 held, 2 blocked, 1 rule, 3 recommend, 38 open. Generated by `tools/register_count.py` from the table rows — do not hand-edit, it has drifted three times when maintained by hand.
...
| K-11f affix check — a SIXTH affix bug | P1 | **closed** | `a2be49d24` + `28ab32b3a` — `FamilyConformanceCheckCommand.cs:105` reads `TAG_3_BOLD_BLUE_BOOL`; the declared name is `TAG_3BOLD_BLUE_BOOL` (no underscore after the 3). So check (4)'s style-matrix sample could never pass for **any** family — including the per-category ones the `IsUniversalTag` change deliberately kept it for. Found by the new affix check, not by hand. BOTH names in that two-line array were wrong: `TAG_3_BOLD_BLUE_BOOL` -> `TAG_3BOLD_BLUE_BOOL`, and `TAG_2_5_NOM_BLACK_BOOL` -> `TAG_2.5NOM_BLACK_BOOL` (literal dot). Check (4) could pass for no family at all; both now match the declaration |
...
| G-21 `PRJ_ORG_PROJECT_CODE_TXT` never written, one reader broken | P1 | **closed** | `a2be49d24` (2.3) — 9 files READ it; **zero write it** (no `SetString`/`Set` anywhere), and `ProjectSetupWizard` does not populate it. `TemplateManifest.cs:198` silently falls back to `ProjectInformation.Number`, which is how folder-tree code and document-number code drift apart with nothing validating agreement. Separately `DrawingDispatcher.cs:106` looks up `PRJ_ORG_PROJECT_CODE` **without `_TXT`** — 0 rows in MR_PARAMETERS.txt, so it always returns null. Third instance of the missing-affix class after `WARN_VISIBLE_BOOL`. **Closed by K-11c**: `ProjectInformation.Number` is the single source and the three duplicate params are written as derived stamps at setup with a divergence warning. The `DrawingDispatcher.cs:106` missing-`_TXT` reader was fixed as bucket B in `98cd5a047` |
...
| **G-15 formula take-off not material-aware** | **P0** | **closed** | `CST_S_MAS_MORTAR_VOLUME_CU_M` queried `BRICK_BOND` unconditionally, so blockwork fell to `BRICK_BOND DEFAULT` = 0.025 m³/m² — a **brick** figure. On 50 m² of 200mm block the formula said **1.25 m³**, `CompoundTakeoffBuilder` said **0.55 m³**: **2.27× apart, nothing flagged either side.** Formula deleted along with its two dependants (`CST_S_MAS_CEMENT_BAGS_NR`, `CST_S_MAS_SAND_VOLUME_CU_M`); `CompoundTakeoff` owns masonry mortar and already emits `mortar_cement`/`mortar_sand` line items. The two aggregates that summed the masonry half are narrowed to **concrete-only** with the reason stated in their description, so they cannot double-count. 7 tests in `StingTools.Boq.Tests` (203 pass) |
| G-15b brick/block decided on a NAME | P1 | **closed** | `material.Contains("brick")` sent "Brick-faced blockwork" down the brick branch. The proposed fix — *bond present ⇒ brick* — was **checked and rejected**: `InferBrickBond` exists because a genuine brick wall may carry no bond parameter and resolve it from the type name, so that test would misclassify every such wall as block. Replaced with `MasonryClassifier` (Revit-free, unit-tested): **block evidence first** (a block size is unambiguous and brick never carries one), then bond evidence, then the name with `block` vetoing `brick` |
| **G-27 all 26 `lookup()` calls can silently take a DEFAULT** | **P0** | **open — measured, not fixed** | new. See [`docs/BOQ_LOOKUP_FORMULA_AUDIT.md`](../docs/BOQ_LOOKUP_FORMULA_AUDIT.md). 28 formulas called `lookup()` before this pass, 25 after the G-15 deletions, **26 individual calls**. **26 of 26** read a table shipping a `DEFAULT` row, so an absent key yields a NUMBER rather than reporting it could not be measured — the exact G-15 mechanism, universally present. Contradicts A-1/H-1. The C# side already flags this via `Resolution` (RC-1); the formula side has no equivalent |
| **G-28 15 of 26 lookup calls have TWO owners** | **P0** | **open — measured, not fixed** | new. The same physical quantity is computed in both a formula and C#: CONCRETE (6), FORMWORK (3), BLOCK/BRICK_BOND (4), PLASTER (2). Masonry was one of these and was 2.27× out. **None of the other 15 has been compared against its C# counterpart.** Both numbers are individually plausible on the page, which is why G-15 was found by accident rather than by review |
| G-29 highest-value KNP26 rows have no second opinion | P1 | open | new. TILE (2), GROUT (1), PAINT (2), PLASTER (2) — 7 of 26, all silently defaultable, and **5 of the 7 have no C# counterpart at all**, so nothing can disagree with them. For a lodge whose floor finishes are the product being sold, these are the rows that matter most |
| G-27a MEASURED / DEFAULTED / UNRESOLVED flag | P0 | **partial** | `QuantityResolution` promotes `CompoundTakeoffBuilder`'s RC-1 `Resolution` (was `private sealed`, `:218`) into a shared type so the formula side records into the SAME vocabulary — a second scheme is how the two paths diverged. Adds the third state (`Unresolved`) that existed as behaviour (G-5) but was never recorded. RC-1's floors preserved exactly (35/55/100) so promotion cannot change take-off behaviour. `FormulaEvaluatorCommand.ParseLookup` instrumented; surfaced in the command's report, not only the log. **REPORT-ONLY — blocks nothing.** 5 tests, 208 pass |
| **G-27b the DEFAULTED / UNRESOLVED counts** | **P0** | **BLOCKED — needs Revit** | the flag ships, but the counts require a run against a real model. Static precondition measured instead: **all 25 key parameters ARE bound**, so none is *guaranteed* defaulted by an unbound key — better than feared. The rate now depends on whether elements carry VALUES, which only a run answers. Added to the operator session |
| **G-30 blocking on DEFAULTED — owner decision** | — | **open, for the owner** | do exports BLOCK when a quantity is `ASSUMED (default)`? A DEFAULT row is a legitimate estimating assumption and may be the normal case, so blocking before G-27b's counts are known could stop every BOQ in the product overnight. **Decide after G-27b.** Not decided here |
| **G-31 formula outputs are read by NOTHING in C#** | **P1** | open — changes G-28's shape | measured: of the 14 dual-owner quantities, **not one** formula output parameter is read by any C# code, including the BOQ export (`CST_S_CON_CEMENT_BAGS_NR`, `CST_CALC_STEEL_KG`, `CST_S_FRM_TIMBER_VOLUME_CU_M`, `CST_S_MAS_BLOCKS_NR`, `CST_CALC_BLOCKS_NR`, `CST_S_MAS_BRICK_QUANTITY_NR` — all 0 references). **The export reads the C# path.** But the formula outputs ARE bound (39-40 categories each), so they are schedulable and a QS may have them in a schedule today. The two are therefore NOT interchangeable surfaces: one is a parameter on an element, the other a BOQ line item. Deleting a formula removes a schedule column that C# cannot refill — which is why blanket deletion of the 14 was NOT done |
| G-32 `CST_CALC_PLASTER_M3` has THREE owners | P1 | open | new — the formula (`lookup(PLASTER,…)`), `CompoundTakeoffBuilder` (plaster volume + cement/sand), **and** `PlasteringEngine.cs:872` which writes it directly via `TrySet`. G-28 assumed two owners per quantity; this one has three |
| G-33 `CST_S_MAS_BLOCKS_NR` binding | P2 | **closed by G-34** | new — **0 rows** in `CATEGORY_BINDINGS.csv`, so the formula computes a value that lands nowhere and no schedule can show it. Dead output, unlike its siblings which are bound to 39-40 categories |
| **G-34 two block-count formulas disagreed by 15.8%** | **P0** | **closed** | `CST_CALC_BLOCKS_NR` (NET area x frozen 1.03) vs `CST_S_MAS_BLOCKS_NR` (GROSS area x wastage parameter) — two axes at once. Verified independently: `BLOCK 400x200 BLOCKS_PER_M2` = 12.5, wastage param declared and bound to 39 categories. 44.0x12.5x1.03 = **566** vs 50.0x12.5x1.05 = **656**, divergence **15.84%**. **Neither flags, because neither DEFAULTS** — G-27's measured/assumed flag cannot see this class. Resolved to one owner on the basis that neither was right: NET area (blocks are not bought for openings) x the project wastage parameter (3% should not be frozen when a tunable parameter exists). Twin deleted, 0 residual references. 4 tests, 212 pass |
| G-36 formula-vs-formula sweep | — | **measured** | new — the dual-owner audit only compared formula-vs-C#. Swept all 298 formula rows for the third class. **1 collision on (table,column)**: `PAINT.COVERAGE_M2_PER_L` used by `BLE_FINISH_PAINT_VOLUME_L` and `CST_CALC_PRIMER_LITERS` — legitimate, different keys and different products. `CST_CALC_PAINT_LITERS` turned out to be a straight ALIAS of `BLE_FINISH_PAINT_VOLUME_L`, so it cannot diverge. The only real find was G-35 |
| G-35 masonry mortar's third owner | P0 | **closed** | `CST_CALC_MORTAR_M3 = CST_CALC_BLOCKS_NR * 0.0063` deleted — 0 C# readers, 0 formula dependants. Was 6.6x out (3.638 m³ vs CompoundTakeoff's 0.550) and not a defensible alternative method: 0.0063 m³/block is **39% of a 400x200x200 block's own volume** against a 10-15% norm. It was bound to 39 categories, so a schedule column is lost — accepted, because a wrong number on a schedule is worse than an absent one. `CompoundTakeoff` is now the sole owner |
| **K-16 REOPENED — the diagnosis was wrong** | **P0** | **open** | the live G-8 audit disproves it. K-16 claimed `ASS_PRODCT_COD_TXT` and `ASS_SYSTEM_TYPE_TXT` were Type-bound and therefore invisible to `Element.LookupParameter`. **Measured on KNP26: both are actual=Instance**, so the lookup finds them. Worse, the contrast K-16 drew against `MAT_CODE` does not exist — `MAT_CODE` is *also* declared=Type/actual=Instance. All three behave identically. `ReadInstanceThenType` is harmless (instance-first) and is kept, but it fixed nothing |
| G-8 declared-vs-actual binding | P1 | **closed — documentation defect** | 2,654 of 3,033 bound params contradict `MR_PARAMETERS.csv`, **all** declared=Type/actual=Instance. Cause: **two binders**. `LoadSharedParamsCommand` — the one operators run — calls `NewInstanceBinding` exclusively; `DataPipelineCommands.cs:792` and `TemplateManagerCommands.cs:2813` honour `Binding_Type`. The column is live in one path and decorative in the path everybody uses. **Instance is correct** — `ASS_TAG_1_TXT` must be per-instance or every door of a type shares one asset tag. Caveat recorded: this column **misled a P0** (K-16) |
| G-8b the 22 bound-but-undeclared | — | **closed** | a **second shared-parameter file**: `STING_TITLE_BLOCK_PARAMETERS.txt` declares **75 params, none of them in `MR_PARAMETERS.csv`**. The audit only checked against the one file |
| G-37 propagation copied the wrong category | P1 | **closed** | `PropagateUniversalTagCommand.cs:278` read the target category off the TARGET, so a mis-categorised family stayed mis-categorised through every propagation. Confirmed live: *STING - Air Terminal Tag* is a **Generic Model Tag**, so Revit never offers it for Air Terminals. Now resolves via `TagCategoryResolver` against the declared category, falling back to the family's own. **147 families declare a category across 133 host categories** (independently counted; brief said 146/134). `IsMismatch`/`Note` surfaced as a finding, never a silent correction |
| G-38 conformance check had no category test | P1 | **closed** | which is how 206 families could carry wrong categories undetected — it read `FamilyCategory` at `:183` and never validated it. Now compares against the declared category and reports a mismatch as a defect |
| G-39 FUNC renders empty, silently | P1 | open | new — live on KNP26: `M-EXT-Z01-L02-HVAC--SAT-`. The air terminal's System Type reads *Undefined*, so the system-aware FUNC lookup has nothing to key on. **Decision needed**: fall back, or fail loud as K-13 did for ISO sheet tokens |
| G-40 SEQ renders empty, silently | P1 | open | new — same tag, trailing segment blank. The sequence counter did not assign. Separate cause from G-39 |
| G-41 LOC defaulted to EXT inside a building | P1 | open | new — the F-2/LOC class. KNP26 has no declared LOC codes yet, so it took a default rather than reporting it could not resolve |
| G-42 incomplete tags reached drawings silently | P1 | **closed** | `TagConfig.TagIsComplete` already existed and was called from **eight sites — every one a READER** (`ComplianceScan:690/842/843/1013`, `BOQSupportCommands:135`, `BIMManagerCommands:4068/4909/8950`). Nothing checked at WRITE time. Now wired into `BuildAndWriteTag` after the TAG1 write, reusing the existing check rather than a second one. Counts surface in the tagging result beside `Tagged:`, with up to 10 samples |
| G-42b complete-but-ASSUMED is now distinct | P1 | **closed** | the validate-before-write block silently substituted `A`/`BLD1`/`Z01`/`L00`/`GEN` to guarantee non-empty — the exact behaviour A-1/K-13/G-27 forbid elsewhere. Kept (removing it emits doubled separators) but every substitution is recorded, and G-27's vocabulary carried through: `IncompleteTagCount` vs `AssumedTokenTagCount`. Only LOC and ZONE were counted before; the other five were invisible |
| A.2 token policy is DATA, not code | — | **closed** | `Data/STING_TAG_TOKEN_POLICY.json`, corporate baseline + project override at `<project>/_BIM_COORD/tag_token_policy.json`. Classifies all ten tokens MANDATORY / DERIVED / OPTIONAL on K-13's model. **MANDATORY: DISC, LVL, SEQ. DERIVED: LOC, ZONE, SYS, FUNC, PROD. OPTIONAL: STATUS, REV.** ZONE is deliberately DERIVED in the baseline and flagged as the token sectors will disagree about — healthcare should raise it in the project override |
| **G-43 the on-drawing tag is a THIRD string** | **P1** | **open — lead, not yet fixed** | new. `ASS_TAG_1_TXT` is the canonical tag; `ASS_DISPLAY_TXT` is *the ON-DRAWING tag* built by `BuildDisplayTag`, which applies a display mode and segment mask. **`BuildAndWriteTag` substitutes `GEN` for a blank FUNC, so it cannot produce the observed `M-EXT-Z01-L02-HVAC--SAT-`** — and `AssembleContainer` skips empties, so it cannot either. The doubled and trailing separators point at the mask-resolved display string. That is where G-39/G-40's *rendering* should be chased; the completeness check above catches the canonical tag, not the display one |
| G-44 two writers own `ASS_TAG_1_TXT` | P1 | open | new — `BuildAndWriteTag` writes it directly with defaulted tokens; `WriteContainers`→`AssembleContainer` also writes it (container #1, `tokens="all"`) and SKIPS empties instead. Same dual-owner class as G-15/G-34/G-35, now on the tag string itself. Different inputs produce different strings for the same parameter |
| K-16b rate pass order was backwards | P0 | **closed** | category was consulted FIRST and RETURNED, so PROD was unreachable on any element whose category had a row — and `cost_rates_5d.csv` has one for every common category. Reordered to specificity: **product → category\|system → material → category**, confidence following specificity (95/92/85/70). A category rate is the LEAST specific answer, not the most confident |
| K-16c every rate declares how it resolved | P1 | **closed** | `RateResolutionLevel` = None/Category/Material/System/Product on `RateLookup`, mirroring G-27's `LookupState` rather than inventing a third vocabulary. Every branch sets it and the provenance names the level ("category average (Doors)") |
| G-48 review rule — establish the caller | — | **rule** | new, earned six times this session: `TagIsComplete` read at 8 sites, enforced at none · `AnnotationRunner` writing unbound parameters · `PRJ_SHEET_*` bound with no writer · `Binding_Type` honoured by one binder of two · `GetFuncCode` with two callers, neither the pipeline · the G-44 pair. **When you find a check or a map, establish WHO CALLS IT before assuming it runs.** A mechanism that exists is not a mechanism that fires |
| G-47 token change now marks the tag stale | P1 | **partial — code done, gate 1.3 needs Revit** | `StingStaleMarker` watched only geometry and `MATERIAL_ID_PARAM`. Added document-scoped triggers for the eight ISO source tokens. These are SHARED parameters, so unlike `MATERIAL_ID_PARAM` there is no fixed `BuiltInParameter` id — the `ElementId` is per-document, which is why `SetEnabled` now takes a `Document` and uses the document-scoped `AddTrigger` overload. STATUS/REV excluded: OPTIONAL in the policy and not in the 8-segment string, so watching them adds cost for no staleness they could detect. **1.2 perf could not be measured without Revit**; scope was minimised to the 8 that matter rather than guessing |
| **G-49 D1's blanket migration would destroy the only real product tier** | **P0** | **open — gate failed, Phase 3 STOPPED** | new. Measured all 42 rate categories against `ProdMap`: **15 agree · 10 rename cleanly · 1 COLLAPSE · 16 have no ProdMap entry.** The COLLAPSE is *Air Terminals*, which carries **two distinct products** — `ATU` (mechanical air terminal, UGX 555,000) and `LAT` (lightning air terminal, UGX 666,000). Both map to `GRL`, so migrating would merge the only genuine product differentiation in the table into one rate |
| **G-46 the rate table now has a product tier** | **P0** | **closed — GATE 3.2 PASSED** | D6/D7. Keyed on **(DISC, PROD)**, not PROD alone: *Air Terminals* carries a mechanical terminal and a lightning terminal, both `GRL` in ProdMap, and PROD alone would have collapsed them. **`M\|GRL` → UGX 555,000 · `E\|GRL` → UGX 666,000** — the first time two products in one Revit category price differently. `Pass 0` added at confidence 97 above the category pass. Locked by 5 tests; 218 pass |
| G-45 vocabulary reconciled without touching ProdMap | P0 | **closed** | the CSV's `MAT_CODE` (`DOR`, `CTR`, `SNY`…) disagreed with `ProdMap` (`DR`, `CBLT`, `FIX`…). A new `PROD` column carries the ProdMap code; `MAT_CODE` is kept so legacy keys still resolve. **ProdMap itself is untouched** — migrating it would have rewritten every tag in every existing model, which is why D1 was withdrawn |
| G-50 mis-filed products promoted | P1 | **partial** | 7 of 16 promoted to a real Revit category + PROD (`Pipe Systems`→Pipes/PP, `Duct Systems`→Ducts/DU, `Pipe/Duct Insulations`, `Pipe Sleeve`/`Pipe Hanger`→Pipe Accessories, and the `Structural Foundation` singular typo-twin folded into `Structural Foundations`). **9 LPS rows NOT promoted**: `LSP1/LSP2/LSP12/LSP3/LDC/LEE/LEB/LTC/LBC` appear **only in this CSV** — nowhere in code or any other data file. The LPS product vocabulary lives in **FUNC** (AT/DC/EE/BOND/SPD/TC), not PROD, so a PROD column cannot reach them. Before: unreachable. After: unreachable. Promotion would move them, not fix them |
| **G-51 D8's precondition is FALSE — Phase B stopped** | **P0** | **open — GATE B.1 FAILED** | `ResolveLpsFunc` (`TagConfig.cs:1702`) returns exactly `AT/DC/EE/BOND/SPD/TC` as documented — and has **ZERO callers**. `GetSmartFuncCode` has HVAC/HWS/SAN branches and no LPS branch, so it falls to `FuncMap["LPS"]="LPS"`: **every LPS element carries FUNC=LPS**, never a sub-code. A (DISC,PROD,FUNC) key would resolve all 9 LPS rows to one key — the exact collapse D1 would have caused. Third vocabulary problem: `ISO19650Validator.cs:600` declares the valid LPS FUNC set as `AIR/DOW/ERT/BND/SPD/TST`, disagreeing with `ResolveLpsFunc`'s own returns |
| **G-52 D9 — 213 materials price with NO unit** | **P0** | **closed** | the library DOES have `MAT_COST_UNIT_OF_MEASURE` (the brief assumed it did not); **1,066 of 1,279 populated, 213 NULL — and all 213 carry a rate.** Both tiers of `MaterialLibraryRateProvider` defaulted `Unit` to `"each"` when unknown, so a per-m² material billed per item. **That is the UGX 2,220 vs 96,200 defect.** Now refuses to price rather than guessing (A-1/H-1), logs the material, and drops to the next provider |
| **G-53 declared-but-uncalled: 58 of 310** | **P1** | **closed (gate)** | `tools/validate_declared_but_uncalled.py`, ratcheted in CI. Counts CALLERS, not mentions — a grep for a symbol finds its definition and reads as "present", which is how seven of these survived. **58 public static maps/predicates in Core have zero callers**, far more than the 7 known. It catches `ResolveLpsFunc`, the case that had just invalidated D8 |
| **G-54 D10 — internal units leaked onto drawings** | **P0** | **closed** | a tag rendered `Flow:0.882867` for an element whose Air Flow is 25.00 L/s — 25 ÷ 28.3168, Revit's internal ft³/s. Cause: `ParameterHelpers.cs:234` `p.AsValueString() ?? p.AsDouble().ToString("0.###")` — `AsValueString()` is PROJECT units and the **fallback emitted raw internal**. Now converts through the parameter's own `GetUnitTypeId()`. **Audit found 2 more drawing-facing sites**: `DrawingTokenContext.cs:289` (title-block tokens) and `BOQParagraphEnhancer.cs:817` (BOQ descriptions), both fixed via the shared helper rather than a second implementation. The inverse of the ThermalConductivity trap: there the value was already SI and converting corrupted it; here it is genuinely internal and NOT converting corrupts it |
| G-55 D11 — shipped gate defaults | P1 | **closed** | tiers 1+2 ON, 3–10 OFF, and `TAG_WARN_VISIBLE_BOOL` **ON**. It shipped OFF "to avoid expensive per-element warning evaluation", which silently disabled the entire warning surface — the G-42 completeness enforcement produced counts the operator could never SEE, because the family's warning row was gated off by default. A check whose output is invisible is the same defect as a check nobody calls |
| G-56 D.2 dual-owner gate | P1 | **closed (gate)** | three narrow checks rather than one heuristic, per the earlier objection: **(a) two formulas one output = 0** (G-34/G-35 fixed), **(b) one parameter written from 2+ C# files = 34**, **(c) formula output also written by C# = 5**. Ratcheted in CI. **Stated limitation**: (b)/(c) see string-literal writes only, so a write through a constant (`ParamRegistry.TAG1`) is invisible — meaning it does NOT catch G-44, the defect that motivated check (b). The count is a floor, not a total |
