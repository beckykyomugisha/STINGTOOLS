...
> **52 of 93 entries are closed** on `claude/kibale-integration` — 9 partial, 1 held, 1 blocked, 3 recommend, 27 open. Generated by `tools/register_count.py` from the table rows — do not hand-edit, it has drifted three times when maintained by hand.
...
| G-33 `CST_S_MAS_BLOCKS_NR` is bound to nothing | P2 | open | new — **0 rows** in `CATEGORY_BINDINGS.csv`, so the formula computes a value that lands nowhere and no schedule can show it. Dead output, unlike its siblings which are bound to 39-40 categories |
