...
> **63 of 114 entries are closed** on `claude/kibale-integration` — 8 partial, 1 held, 2 blocked, 3 recommend, 37 open. Generated by `tools/register_count.py` from the table rows — do not hand-edit, it has drifted three times when maintained by hand.
...
| **K-16b the REAL cause: Pass A short-circuits, and no PROD rates exist** | **P0** | **open** | `RateProviders.cs:175` Pass A matches on CATEGORY and RETURNS (confidence 90) **before** Pass B's PROD match at `:192` is ever reached. And `cost_rates_5d.csv` has **43 rows, all category-level** — grep for `SAT`, `DR`, `WIN`, `AHU` returns **0**. So `Doors,DOR,A,450.00,1665000,each` prices **every door in KNP26 identically at 1,665,000 UGX**, fire door or cupboard door, regardless of whether PROD is populated. The tag `M-EXT-Z01-L02-HVAC--SAT-` shows PROD is *not* empty — the empty-value hypothesis is also wrong |
