...
> **48 of 90 entries are closed** on `claude/kibale-integration` — 8 partial, 2 blocked, 1 held, 6 recommend/correction/rule, 25 open. Counted from the table rows, not maintained by hand; the previous "58 of 104" was a hand-updated figure that had drifted from the document. See the ID ledger below.
...
| G-18 conformance checker scored two things wrong | P2 | **closed** | `37074d6a2` style matrix N/A for the universal tag (+10); `a762c206f` check (5) required `WARN_VISIBLE_BOOL`, which has 0 rows in MR_PARAMETERS.txt — now sourced from `ParamRegistry`. Worth **+2 not +4**: `Math.Min(visPts, 10)` caps it, 8 -> 10 |
| K-11 41 `PRJ_*` declared but unbound | P0 | **closed** | measured 140 defined / 99 bound / **41 absent** / 7 absent from both — all four confirmed. 36 bound to Project Information (`Instance`, matching `LoadSharedParamsCommand`'s exclusive `NewInstanceBinding`); 5 retired as superseded twins. Sim delta +7 newly bound −5 retired = **+2 bound, −7 skipped, 0 conflicts**, fully reconciled |
| **STANDING RULE — do not mint a second parameter for an existing concept** | — | **rule** | Regardless of GUID scheme. Phase 112 minted a whole `PRJ_ORG_*` set beside an existing `PRJ_*` set with the same meaning, using hand-minted GUIDs (`a1b2c3d4-505c-4a01-b001-…`) where the old set used UUIDv5 — and nothing flagged it, **because GUIDs cannot detect this: two spellings of one concept have different GUIDs by construction.** Only the NAMES collide. `validate_param_readership.py --duplicates` now reports them: **11 concepts with multiple spellings today**, including `ORIGINATOR`, `PROJECT` and `PHASE` — all three Phase 112 `PRJ_ORG_*` duplicates. Reported, never auto-merged: merging project-identity fields breaks sheet numbers, transmittals and the folder tree at once |
| K-11d originator pair — **my retirement was wrong, reversed** | P0 | **closed** | I retired `PRJ_ORIGINATOR_COD_TXT` on "0 C# consumers". That test was too narrow: a `.rfa` **label** is a consumer the C# scan cannot see, and the live Kibale title block reads that spelling. Restored and bound. **The inversion test the owner asked for came back positive**: `STING_TITLE_BLOCK_PARAMETERS.txt:20` declares `PRJ_ORG_ORIGINATOR_CODE_TXT` and `STING_TITLE_BLOCKS.json` references it, so STING's own title-block generator reads the `ORG_` spelling. Both spellings are therefore live on the family layer in *different* families — so the 6 C# consumers were **NOT** repointed (that would break STING-generated title blocks). Both are now bound; both work; no C# change |
| K-11e `.rfa` evidence is unobtainable | — | **BLOCKED** | Searched all 207 shipped families for both spellings: **0 of 207 yield ANY readable parameter name**, not even `ASS_TAG_*` which they certainly contain — the payload is compressed, so `strings` returns nothing and a "0 hits" result proves nothing. No STING title-block `.rfa` is shipped at all (207 repo families are all tags; the content library holds tags + symbols only). Which spelling a given title block reads can only be established by opening it in Revit |
| K-11b duplicate spellings, perfect inversion | P1 | **closed** | the binder shipped the spelling **nobody reads** and left the read spelling unbound, on all three `PRJ_TB_SHOW_*` pairs: `RESOLVED_BINDINGS` had `KEYPLAN`/`NORTHARROW`/`SCALEBAR` (1 row each), `STING_TITLE_BLOCKS.json` reads `KEY_PLAN`(1)/`NORTH_ARROW`(9)/`SCALE_BAR`(9). Every `respectShowToggle` slot in 9 title blocks was gated on an unbindable name. Kept the underscored spellings consumers read; retired the others plus `PRJ_ORIGINATOR_COD_TXT` and `PRJ_VOLUME_COD_TXT` (both read by nothing) |
| K-11c project-code: one concept or two | — | **recommend, not decided** | `PRJ_PROJECT_COD_TXT` feeds `{project}` (`DrawingTokenContext.cs:66`); `PRJ_ORG_PROJECT_CODE_TXT` feeds the template engine (9 readers, 0 writers, G-21). Both live, different consumers. **Recommendation: one concept split in two** — same value in every real workflow — but merging touches sheet numbers on issued drawings, so it needs an owner decision, not a data edit |
| **G-23 nothing validated consumer names against binder data** | **P0** | **closed (gate)** | new — the root cause of G-8, G-20, G-21, K-11 and the `TAG_PARA_STATE_*` finding. `tools/validate_param_readership.py` asserts every STING-shaped literal read from C# is declared in `MR_PARAMETERS.txt` **and** has a binding row. Self-test proves all four known defect shapes are caught with no false positives. Wired into `stingtools-plugin.yml`. **Measured backlog: 603 violations** (598 undeclared / 5 unbound) across 1,469 literals — baseline-gated at 603 so new violations fail the build; existing backlog NOT fixed this pass |
| G-23b the 603 triaged | P1 | **closed** | A=102 missing entirely · **B=3 fixed** · C=242 correctly-undeclared prefix tests + alias keys · D=248 no param context. C was the inflation (`StartsWith("ASS_LOC")`, `_extendedParams["SHORT"]="REAL_TXT"`), now excluded at the scan rather than absorbed. Gate re-baselined **603 -> 379**; a gate set at an inflated number trains people to ignore it. Bucket B: `PLACE_ANCHOR_TXT`/`PLACE_SIDE_TXT` -> `ASS_`-prefixed constants (`FamilyHintsBridge.cs:57,58,154,155`), `PRJ_ORG_PROJECT_CODE` -> `_TXT` (`DrawingDispatcher.cs:106`). `PLACE_OFFSET_X_MM` left: Y and Z do not exist under any spelling, so fixing X alone yields a half-working triple |
| G-24 K-7 hole generalised across all 16 tokens | P1 | **closed** | `{project}` and `{originator}` are the **only** two tokens with no fallback of any kind — every other falls back to the profile, and `{seq}` is deliberately omitted so the literal survives (GAP-D). They do share the hole, as expected. `AuditPattern` already existed and **was** wired in (`DrawingProducer.cs:803,806`), so the warning was not missing — but its advice was wrong for exactly those two: it told operators to set `IsoNaming` defaults for parameter-backed tokens. Now names the source parameter and the fix |
| **G-25 param_binding_resolver would orphan 10 live `SHT_*`** | **P0** | **open — apply BLOCKED** | new. `--apply` regenerates `RESOLVED_BINDINGS.csv` and **removes 10 `SHT_*` rows** (`SHT_NUMBER_TXT`, `SHT_NAME_TXT`, `SHT_DISC_TXT`, `SHT_ORIGINATOR_TXT`, `SHT_TAG_1_TXT`, …) because line 67 excludes the `SHT` prefix by design. Those params have **0 rows in CATEGORY_BINDINGS.csv** and **are read from C#**, so adopting the resolver's output would silently unbind them. The resolver is the wrong side here. Apply reverted; the tool is now `--check`-by-default so this is visible instead of arriving as a side effect |
| G-26 resolver was a mutating verifier | P1 | **closed** | it wrote all three generated files on every run, so a read-only baseline produced a 335-line diff. Now `--check` (default, provably writes nothing, exit 1 on difference) / `--apply`, following `restamp_content_manifest.py`. Also ships `tools/binding_simulator.py` + baseline as a committed script; **3,018/374/0 marked SUPERSEDED and non-reproducible** — it measured runtime CategorySet resolution, the committed sim measures binding-data rows |
| K-13 `{project}`/`{originator}` fail loud | P0 | **closed** | they now follow the `{seq}` rule — OMITTED when the source parameter is empty or unbound, so the literal survives and Revit rejects the sheet number outright. **No placeholder substituted**: a fabricated project code produces a sheet that looks correct and is not, which is worse than one that will not save (same principle as G-5 null-not-0, and the path resolver refusing rather than defaulting to PRJ). Warning names the token AND its source parameter. The K-7 comment is rewritten to state the rule for all 16 tokens — fallback-to-profile / omit / blank-is-safe — instead of describing one instance |
| K-12 seven sheet segments populated | P1 | **closed** | **12** `PRJ_SHEET_*` exist, not 8 — all bound, **11 with zero writers**. `DrawingProducer.StampSheetSegments` now writes the seven ISO segments + `PRJ_SHEET_FULL_REF_TXT` as their join, from values `DrawingTokenContext` had already resolved and was discarding. Enables per-segment title-block labels (playbook Part 1) — better than tag presets because each segment is its own parameter |
| K-12b the "no sheet exists to mislead" argument is FALSE | P1 | **closed** | the 6.3 reasoning does **not** hold: `DrawingProducer` creates the sheet, THEN assigns `SheetNumber` inside a try/catch that only logs — so a sheet **does** exist carrying Revit's default number after a K-13 rejection. An empty segment on it would read as "this project has no volume code". Unresolved segments are therefore written as their **literal** (`{project}`), visibly wrong, consistent with K-13 |
| K-12c MatchLine readers were NOT resolving against empty | — | **correction** | all three (`MatchLineEngine.cs:374`, `:947`, `:1025`) already fall back to `sheet.SheetNumber` — `:374` returns it when the param is empty; `:947`/`:1025` add BOTH values to the ref set. So cross-references worked, just on the native number instead of the full ISO ref. They now receive the richer value; nothing was broken, it was less precise |
| K-12d `PRJ_TB_SHEET_NR_TXT` — 4th spelling | — | **recommend, not decided** | bound, **zero consumers**, described as "sheet number string, may differ from Revit native sheet number". **Recommend RETIRE**: `PRJ_SHEET_FULL_REF_TXT` now holds the authored ISO reference and `sheet.SheetNumber` holds the native one, so the stated purpose is served twice over and a third field can only drift. Not executed — it is bound, so something may read it from a title-block label the C# scan cannot see (K-11e) |
| K-11f affix check — a SIXTH affix bug | P1 | open | new. `FamilyConformanceCheckCommand.cs:105` reads `TAG_3_BOLD_BLUE_BOOL`; the declared name is `TAG_3BOLD_BLUE_BOOL` (no underscore after the 3). So check (4)'s style-matrix sample could never pass for **any** family — including the per-category ones the `IsUniversalTag` change deliberately kept it for. Found by the new affix check, not by hand. Reported, not fixed |
| **K-16 BOQ rate read the wrong binding scope** | **P0** | **closed** | `11b1f2c96` — `ResolveRate` keyed on `ASS_PRODCT_COD_TXT` and `ASS_SYSTEM_TYPE_TXT`, both **Type**-bound (19 rows each), but read them off the INSTANCE via `Element.LookupParameter`, which is instance-scoped. Both were **always empty**, so the PROD-code and system-type rate passes never matched and every element fell to the single category rate — a fire door and a cupboard door priced identically. Quantity was never wrong. Fixed with `ReadInstanceThenType` (instance first, so a placement override still wins). Same class as the AnnotationRunner writes removed in `f5ae772d9` |
| G-25 `SHT_*` orphaning | P0 | **closed** | `3cbec5cb6` — the resolver's `SHT` exclusion was the wrong side, not the data. All ten bind to `Sheets` exactly as `PRJ_SHEET_*` does, and **nine of ten have no `CATEGORY_BINDINGS` row**, so that was their only binding; `SHT_REV_TXT`/`SHT_REV_DATE_TXT` are read by live revision-sync code. `SHT` now maps to `Sheets`; `--apply` landed as its own commit: **+27 bound, only the 4 deliberately-retired twins removed, nothing live orphaned** |
| 3B.4 presentation types auto-tagged | P1 | **closed** | `28ab32b3a` — 6 client-facing `pres-*` types carried `autoTag=true`. Set false; `StampDrawingTypeChecksums` re-run deliberately (6 of 93 drifted, 6 stamped, `--check` 93/93) so the data change and the re-stamp are reviewable together |
| G-20 type-mark generator | P1 | **closed** | `a2be49d24` — `TypeMarkSequencer`: monotonic and never reused, existing marks adopted not overwritten, idempotent, PROD resolved through `TagConfig.GetFamilyAwareProdCode` so the mark can never disagree with tag segment 7, preview as a separate command, `VerifyJoin` on every run |
| G-20 no type-mark generator | P1 | open | new — `ASS_TYPE_MARK_TXT` is a one-way mirror: `ParameterHelpers.cs:3884` `MapBuiltIn`s an **existing** `ALL_MODEL_TYPE_MARK` from the type. Nothing generates `DR-01` / `WIN-03` sequences per type, so door and window marks are hand-entered. Documented in playbook 3E as a known manual step; not built this pass |
| G-21 `PRJ_ORG_PROJECT_CODE_TXT` never written, one reader broken | P1 | open | new — 9 files READ it; **zero write it** (no `SetString`/`Set` anywhere), and `ProjectSetupWizard` does not populate it. `TemplateManifest.cs:198` silently falls back to `ProjectInformation.Number`, which is how folder-tree code and document-number code drift apart with nothing validating agreement. Separately `DrawingDispatcher.cs:106` looks up `PRJ_ORG_PROJECT_CODE` **without `_TXT`** — 0 rows in MR_PARAMETERS.txt, so it always returns null. Third instance of the missing-affix class after `WARN_VISIBLE_BOOL` |
| G-22 "PRJ" fallback was silent | P0 | **closed** | `2ee8356a0` — `DetectProjectCode` returns `PRJ` when Number **and** Name are empty, then mints the tree and the sticky ES root stamp with no output. Now warns per occurrence + one dialog per model; deliberately does **not** auto-correct |
