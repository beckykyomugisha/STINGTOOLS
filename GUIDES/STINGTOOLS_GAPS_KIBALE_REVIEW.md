...
> **61 of 111 entries are closed** on `claude/kibale-integration` — 8 partial, 1 held, 2 blocked, 3 recommend, 36 open. Generated by `tools/register_count.py` from the table rows — do not hand-edit, it has drifted three times when maintained by hand.
...
| **G-42 a tag with two empty segments passed silently** | **P1** | open | new — whatever G-39/G-40/G-41 resolve to, an ISO tag rendering with two blank segments should not reach a drawing without a word. This is the tag-side equivalent of K-13, which fixed exactly this for sheet numbers |
