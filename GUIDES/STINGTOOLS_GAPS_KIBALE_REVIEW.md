...
> **65 of 119 entries are closed** on `claude/kibale-integration` — 8 partial, 1 held, 2 blocked, 1 rule, 3 recommend, 39 open. Generated by `tools/register_count.py` from the table rows — do not hand-edit, it has drifted three times when maintained by hand.
...
| **G-47 token change does not mark the tag stale** | **P1** | **open — the real cause behind G-39/G-40** | new. `StingStaleMarker` triggers ONLY on `GetChangeTypeGeometry()` (`StingAutoTagger.cs:1394`) and `MATERIAL_ID_PARAM` (`:1402`). **No STING token parameter is watched.** So a container assembled before `PopulateAll` filled FUNC is never rebuilt — the blank segments are STALE, not mis-computed. Affects every project, and explains why FUNC/SEQ read blank while `FuncMap`/`ProdMap` are both correct |
