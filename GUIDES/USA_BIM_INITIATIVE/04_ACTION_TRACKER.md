...
Dates are offsets from the meeting date (**D**), because D is not yet fixed
(`00_DECISION_LOG.md` D-05). Fill in real dates as soon as it is.

**Meeting date: `___________`**

Status: `[ ]` open · `[~]` in progress · `[x]` done · `[-]` dropped, with a reason
...
## D-7 — demo rehearsal gate

Full checklist in `02_PRESENTATION_PLAN.md`. Summary:

| | Action |
|---|---|
| `[ ]` | Write the demo path as numbered steps |
| `[ ]` | **Confirm which deployment the demo runs against — verify, don't assume** |
| `[ ]` | Rehearsal 1, timed, on the deployed build; log every failure |
| `[ ]` | Fix or cut what failed |
| `[ ]` | Rehearsal 2, clean, no interventions |
| `[ ]` | Record the successful run as fallback |
| `[ ]` | **Commit to live / hybrid / recorded** and write it into D-04 |
...
| | Action |
|---|---|
| `[ ]` | Confirm attendance, venue and start time with Ken |
...
## D+1 to D+3 — follow-up

| | Action |
|---|---|
| `[ ]` | Thank-you email within 24 hours, with the three asks restated in writing |
