...
namespace StingTools.BOQ
{
    ...
    public class BOQExportCommand : IExternalCommand
    {
        ...
        private void BuildSummarySheet(IXLWorksheet ws, BOQDocument boq)
        {
            ...
            ws.Cell(row, 6).Value = "Net works (measured + FF&E / PC)"; ws.Cell(row, 6).Style.Font.SetBold();
            ws.Cell(row, 7).FormulaA1 = $"SUM(G{firstDataRow}:G{row - 4})"; ws.Cell(row, 7).Style.NumberFormat.Format = "#,##0"; row++;
            // Phase C.2 — Owner-procured FF&E + PC sums are carried at cost; prelims +
            // contingency apply to the measured works only (FF&E / PC excluded).
            if (boq.MarkupExemptUGX > 0)
            {
                ws.Cell(row, 6).Value = "  of which Owner-procured FF&E + PC sums (at cost, no markup)";
                ws.Cell(row, 6).Style.Font.SetItalic(true);
                ws.Cell(row, 7).Value = boq.MarkupExemptUGX; ws.Cell(row, 7).Style.NumberFormat.Format = "#,##0"; row++;
            }
