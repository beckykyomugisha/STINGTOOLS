    ...
    public class ConvertPackToManagedCommand : IExternalCommand
    {
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            try
            {
                ...
                if (string.IsNullOrEmpty(packLabel)) return Result.Succeeded;
                var pack = packs.First(p => (p.Name ?? p.Id) == packLabel);

                var templates = new FilteredElementCollector(doc).OfClass(typeof(View)).Cast<View>().Where(v => v.IsTemplate).Select(v => v.Name).ToList();
                if (templates.Count == 0) { TaskDialog.Show("STING", "No view templates in project."); return Result.Succeeded; }
                var tplName = StingListPicker.Show("Source Template", "Pick a template to import settings from", templates);
                if (string.IsNullOrEmpty(tplName)) return Result.Succeeded;
                var tpl = new FilteredElementCollector(doc).OfClass(typeof(View)).Cast<View>().FirstOrDefault(v => v.IsTemplate && v.Name == tplName);
                if (tpl == null) return Result.Succeeded;
                ...
                TaskDialog.Show("STING", $"Pack '{pack.Name}' converted to managed. Run 'Regenerate Templates' to apply.");
                return Result.Succeeded;
            }
            catch (Exception ex) { message = ex.Message; StingLog.Error("ConvertPackToManaged", ex); return Result.Failed; }
        }
    }
    ...
    public class DetachFromManagedCommand : IExternalCommand
    {
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            try
            {
                ...
                int renamed = 0;
                using (var t = new Transaction(doc, "STING Detach Managed Pack"))
                {
                    t.Start();
                ...
                TaskDialog.Show("STING", $"Detached pack '{pack.Name}'. Renamed {renamed} template(s).");
                return Result.Succeeded;
            }
            catch (Exception ex) { message = ex.Message; StingLog.Error("DetachFromManaged", ex); return Result.Failed; }
        }
    }
