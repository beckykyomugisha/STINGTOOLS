...
namespace StingTools.Commands.Electrical.Lighting
{
    ...
    public class LightingCalcSheetCommand : IExternalCommand
    {
        ...
        private static RoomSheet BuildSheet(Document doc, SpatialElement room, LuxTargetTable luxTargets)
        {
            int fixtures = 0;
                ...
                .WhereElementIsNotElementType().OfType<FamilyInstance>())
            {
                ...
                try { if (StingTools.Core.Placement.SpatialCompat.Contains(fi, room)) inRoom = true; } catch { }
                if (!inRoom)
                {
