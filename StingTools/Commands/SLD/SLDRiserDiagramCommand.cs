...
namespace StingTools.Commands.SLD
{
    ...
    public class SLDRiserDiagramCommand : IExternalCommand
    {
        ...
        private static void DrawRiser(Document doc, ViewDrafting view,
            StingTools.Core.SLD.SLDNode root, RiserOptions opts)
        {
            ...
            DrawFeeders(doc, view, root, positions, boxW, boxH, vertical);
        }

        private static void DrawBox(Document doc, ViewDrafting view,
            double x, double y, double w, double h, StingTools.Core.SLD.SLDNode node, RiserOptions opts)
        {
            try
            {
                ...
                if (opts.ShowFaultKa && node.RevitElement != null)
                {
                    ...
                    if (!string.IsNullOrEmpty(fk) && ts != null)
                        TextNote.Create(doc, view.Id, new XYZ(x + w / 2, y - h * 0.15, 0), $"Iₖ {fk} kA", ts.Id);
                }
                if (opts.ShowLoadingPct && node.LoadKW > 0 && ts != null)
                {
                    int pct = (int)Math.Min(100, node.LoadKW); // best-effort estimate
        ...
        private static void DrawFeeders(Document doc, ViewDrafting view,
            ...
            double boxW, double boxH, bool vertical)
        {
    ...
    public class SLDUpdateRiserCommand : IExternalCommand
    {
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            ...
            var view = new FilteredElementCollector(doc)
                .OfClass(typeof(ViewDrafting)).Cast<ViewDrafting>()
                .FirstOrDefault(v => (v.Name ?? "").StartsWith("STING - Riser Diagram", StringComparison.OrdinalIgnoreCase));
            if (view == null)
            {
            ...
            using (var tx = new Transaction(doc, "STING Update Riser Diagram"))
            {
                ...
                {
                    var opts = StingElectricalCommandHandler.CurrentRiserOptions;
                    typeof(SLDRiserDiagramCommand)
                        .GetMethod("DrawRiser", System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Static)
                        ?.Invoke(null, new object[] { doc, view, root, opts });
                }
                tx.Commit();
            }
