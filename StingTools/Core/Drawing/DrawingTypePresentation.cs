...
namespace StingTools.Core.Drawing
{
    public static class DrawingTypePresentation
    {
        ...
        public static ApplyResult Apply(Document doc, View view, DrawingType dt, ApplyOptions options)
        {
            ...
            // on first miss per (docKey, templateName). Pack fallback only
            // applies for external packs — managed packs override below.
            if (!string.IsNullOrWhiteSpace(effectiveTemplateName))
            {
                try
                {
                    ElementId tplId = ResolveViewTemplate(doc, effectiveTemplateName);
                    if (tplId != ElementId.InvalidElementId)
                    {
                        view.ViewTemplateId = tplId;
                        ...
                        if (templateFromPack)
                            StingTools.Core.StingLog.Info(
                                $"DrawingType '{dt.Id}' inherited view template '{effectiveTemplateName}' from pack '{fallbackPack.Id}'.");
                    }
                    else
                    {
                        var src = templateFromPack ? $" (from pack '{fallbackPack.Id}')" : "";
                        r.Warnings.Add($"View template '{effectiveTemplateName}'{src} not found in project.");
                    }
                }
