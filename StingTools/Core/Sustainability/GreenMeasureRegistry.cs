...
namespace StingTools.Core.Sustainability
{
    ...
    public class GreenMeasureRegistry
    {
        ...
        private static string SafeRead(string path)
        {
            ...
            catch (Exception ex) { SustainOverrideHealth.Report("GreenMeasure", $"read failed for {path}: {ex.Message}"); return null; }
        }

        private void Apply(string json)
        {
            JObject root;
            try { root = JObject.Parse(json); } catch (Exception ex) { SustainOverrideHealth.Report("GreenMeasure", $"malformed override/data JSON: {ex.Message}"); return; }
