...
3. **Check for an existing family** — Looks in `Data/TagFamilies/` for a family whose labels are already bound, and uses it directly if found
...
```
1. Family exists in Data/TagFamilies/            → Load directly (best — labels pre-configured)
...
```
Data/TagFamilies/STING - Mechanical Equipment Tag.rfa           ← Priority 1
Data/TagFamilies/STING - Mechanical Equipment Tag_seed.rfa      ← Priority 2 (alt naming)
...
- Package tag family .rfa files in the `Data/TagFamilies/` directory
...
**Fix**:
- Delete the family file from `Data/TagFamilies/`
...
| `Data/TagFamilies/` | Tag families, labels already bound. **There is no `Seeds/` subfolder** — see the note above |
...
□ 13. Manual: Copy configured .rfa to Data/TagFamilies/              Reused on the next run
```
