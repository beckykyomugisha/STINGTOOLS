...
> **There is no `Data/TagFamilies/Seeds/` folder, and nothing should recreate one.**
> It held 137 pre-Phase-188 tag families and was deleted. 88 of them duplicated the
> flat `TagFamilies/` set and were shadowed by it; 39 more were superseded by a
> differently-named family covering the same category. All of them bound labels to
> parameters that Phase 188 retyped, which is what produced the recurring
> "Inconsistent Units" error — and because `Seeds/` was searched *first*, a stale
> copy beat the current family for every category it covered.
>
> `Data/TagFamilies/` is the one location. Configure Labels writes there, and the
> creator reads from there.
...
3. **Check the library** — Looks for an existing family in the shared content library (`<STING_CONTENT_LIB>/Tags/`) then `Data/TagFamilies/` and loads it directly if found. The former `Data/TagFamilies/Seeds/` sub-folder was RETIRED: its 137 files pre-dated the tier change and shadowed the live set for 88 categories.
...
```
1. Family exists in <content library>/Tags/ or Data/TagFamilies/ → Load directly (best — labels pre-configured)
...
**Fix**:
- Delete the file from `Data/TagFamilies/` (or the shared library)
...
| `<STING_CONTENT_LIB>/Tags/` | Shared firm library — searched first (labels already bound) |
...
□ 13. Manual: Copy configured .rfa to the shared library or Data/TagFamilies/  For future zero-config deployments
```
