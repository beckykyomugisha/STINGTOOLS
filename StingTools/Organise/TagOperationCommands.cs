...
namespace StingTools.Organise
{
    ...
    public class ColorTagsByDisciplineCommand : IExternalCommand
    {
        public Result Execute(ExternalCommandData cmd, ref string msg, ElementSet el)
        {
            ...
            if (resultDlg.Show() == TaskDialogResult.Yes)
            {
                ...
                catch (Exception legendEx)
                {
                    StingLog.Warn($"Legend creation failed: {legendEx.Message}");
                    TaskDialog.Show("Color Tags", $"Tags colored successfully but legend creation failed:\n{legendEx.Message}");
                }
            }
                ...
                foreach (var tag in tags)
                {
                    try
                    {
                        ...
                        else
                        {
                            ...
                            merged.SetSurfaceTransparency(0 /* SurfaceTransparency not readable */);
            ...
            using (Transaction tx = new Transaction(doc, "STING Tag Line Weight"))
            {
                ...
                foreach (var tag in tags)
                {
                    try
                    {
                        ...
                        ogs.SetSurfaceTransparency(0 /* SurfaceTransparency not readable */);
    ...
    public class SnapLeaderElbowCommand : IExternalCommand
    {
        ...
        private static XYZ CalculateElbowPosition(XYZ hostCenter, XYZ tagHead, XYZ delta, string angleMode)
        {
            ...
            {
                // 45° elbow: short diagonal from host (arrowhead), then straight to tag
                ...
                // Elbow near arrowhead: diagonal starts at host, bends toward tag
                if (absDx > absDy)
            ...
            {
                // 90° elbow: short vertical from host (arrowhead), then horizontal to tag head
