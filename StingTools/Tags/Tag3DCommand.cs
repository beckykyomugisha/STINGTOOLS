...
namespace StingTools.Tags
{
    ...
    /// <summary>
    /// Repair duplicate SEQ numbers across the project by re-assigning unique
    /// sequential numbers while preserving all other tag tokens.
    /// </summary>
    [Transaction(TransactionMode.Manual)]
    [Regeneration(RegenerationOption.Manual)]
    public class RepairDuplicateSeqCommand : IExternalCommand
    {
            ...
            ref string message, ElementSet elements)
        {
            ...
            // Find all elements with tags
            var taggedElements = new Dictionary<string, List<Element>>();
            ...
            foreach (Element el in allElements)
            {
                ...
                if (!taggedElements.ContainsKey(tag))
                    taggedElements[tag] = new List<Element>();
                taggedElements[tag].Add(el);
            }

            var duplicates = taggedElements.Where(kv => kv.Value.Count > 1).ToList();
                    ...
                    for (int i = 1; i < kv.Value.Count; i++)
                    {
                        ...
                        try
                        {
                            ...
                            if (!string.IsNullOrEmpty(newTag))
                            {
                                ...
                                ParamRegistry.WriteContainers(el, toks, catName,
                                    overwrite: true, skipParam: ParamRegistry.TAG1);
                                TagConfig.WriteTag7All(doc, el, catName, toks, overwrite: true);
                            }
                            repaired++;
                        }
                        catch (Exception ex)
                        {
                            StingLog.Warn($"RepairDuplicateSeq for {el.Id}: {ex.Message}");
                        }
                    }
                }
                tx.Commit();
            ...
            TaskDialog.Show("Repair Duplicate SEQ",
                $"Found {duplicates.Count} duplicate tag groups.\n" +
                $"Repaired: {repaired} elements with new unique SEQ numbers.");
            return Result.Succeeded;
        }
    }
}
