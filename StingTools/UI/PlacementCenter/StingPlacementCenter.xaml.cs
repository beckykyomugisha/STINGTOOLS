    ...
    public partial class StingPlacementCenter : Window
    {
                        ...
                        req.Result = FixturePlacementEngine.PlaceFixturesInScope(
                            ...
                            progress: (done, total) =>
                            {
                                try
                                {
                                    ...
                                    try { req.HeartbeatCts?.Cancel(); } catch (Exception ex) { StingLog.Warn($"Suppressed: {ex.Message}"); }
        ...
        public static void ShowOrFocus(UIApplication uiApp)
        {
            if (_instance != null && !_instance._closed)
            {
                ...
                {
                    try { _instance.Close(); } catch (Exception ex) { StingLog.Warn($"Suppressed: {ex.Message}"); }
                    _instance = null;
                }
                else
                {
        ...
        private void HookDocumentLifecycle()
        {
            ...
            try
            {
                ...
                this.Closed += (_, __) =>
                {
                    try { _uiApp.Application.DocumentClosing -= OnRevitDocumentClosing; } catch (Exception ex) { StingLog.Warn($"Suppressed: {ex.Message}"); }
                };
            }
        ...
        private void OnRunPlacement_Click(object sender, RoutedEventArgs e)
        {
            ...
            try { progress.SetStatus("Pre-flight — scanning loaded families…"); } catch (Exception ex) { StingLog.Warn($"Suppressed: {ex.Message}"); }
            ...
            System.Threading.Tasks.Task.Run(() =>
            {
                ...
                while (!heartbeatCts.IsCancellationRequested)
                {
                    ...
                    string label = string.IsNullOrEmpty(phase)
                        ...
                        : $"{phase} ({n * 1.5:F0}s)…";
                    try { progress.SetStatus(label); } catch (Exception ex) { StingLog.Warn($"Suppressed: {ex.Message}"); break; }
                }
            }, heartbeatCts.Token);
            ...
            {
                EnsureRunEvent();
                try { if (btnRunPlacement != null) btnRunPlacement.IsEnabled = false; } catch (Exception ex) { StingLog.Warn($"Suppressed: {ex.Message}"); }
            ...
            catch (Exception ex)
            {
                ...
                try { heartbeatCts.Cancel(); } catch (Exception ex) { StingLog.Warn($"Suppressed: {ex.Message}"); }
                try { progress?.Close(); } catch (Exception ex) { StingLog.Warn($"Suppressed: {ex.Message}"); }
                ...
                try { if (btnRunPlacement != null) btnRunPlacement.IsEnabled = true; } catch (Exception ex) { StingLog.Warn($"Suppressed: {ex.Message}"); }
                return;
            }
        ...
        {
            try { req?.HeartbeatCts?.Cancel(); } catch (Exception ex) { StingLog.Warn($"Suppressed: {ex.Message}"); }
            try { req?.Progress?.Close(); } catch (Exception ex) { StingLog.Warn($"Suppressed: {ex.Message}"); }
