...
- ✅ [`LPS_TAG_MASTER_BUILD_SHEET.md`](LPS_TAG_MASTER_BUILD_SHEET.md) — the 88-row
  hand-authoring spec for `STING_LPS_Tag_Universal`, the Multi-Category master the
  nine LPS tags take their label from. GENERATED by
  `tools/gen_lps_authoring_sheet.py`; regenerate rather than edit.

- ✅ [`LPS_MASTER_CORRECTIONS.md`](LPS_MASTER_CORRECTIONS.md) — the twelve rows that
  changed when numeric parameters were repointed at their TEXT twins, for a master
  already part-built. Generated by `tools/gen_delta.py`.

- ✅ [`SPECIALIST_TAG_BUILD_SHEET.md`](SPECIALIST_TAG_BUILD_SHEET.md) — hand-authoring
  spec for the four bespoke drawing tags (Fire Door, Accessible Door, Room Finish,
  Fire Compartment): label rows, formulas, types, and the post-build steps that wire
  them into the fire-strategy, floor-finishes and accessibility drawing types.
