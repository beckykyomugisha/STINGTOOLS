...
| `DWG-STRUCT-DEEP-6` Junction-type assignment beyond detection | Phase 141 wires `DetectJunctions` warnings into TextNote markers, but the analytical model still has no per-junction connection-detail elements. A future pass would create connection geometry (bolt patterns, weld lines) at each L/T/Cross junction. | Connection-element synthesizer per junction type, consumed by Revit structural connection families. |
