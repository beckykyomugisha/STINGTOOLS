...
governs is narrower than the file implies, and that gap is tracked here rather than
papered over.

| ID | Status | Detail |
|---|---|---|
| TOKPOL-1 | ✅ **CLOSED** (Phase 294) | The policy governed the tag STRING from Phase 288 but never the **derivation**: `DetectZone` / `DetectLoc` returned the literals `"Z01"` and `"BLD1"` straight from `ParameterHelpers.cs`, so a project that overrode those fallbacks still got the hardcoded pair written onto every element — and the tag could disagree with the parameter it came from. Closed **without touching the 22 call sites**: `PolicyFallback(doc, token, legacy)` asks the policy at the two points where the literals were, the signatures are unchanged and the result is never empty, so the silent-blank risk that made this worth declining twice does not arise. The legacy value survives only as a floor. Guarded by `check_token_policy_wired.py`, and the guard is **method-scoped** — a file-wide check went red on a correct tree because `ParseZoneCode` legitimately maps "NORTH" to Z01. |
...
| ID | Status | Detail |
|---|---|---|
| MAPTYPE-1 | ✅ **CLOSED — measured in Revit 2026-09-16** | Not reversed. On a tagged `M_Door-Passage-Single-Flush 750 x 2000mm`, `BLE_DOOR_WIDTH_MM` read **blank**, not `274320` and not `2.95`, so the raw/formatted arguments are the right way round. The blank had a different cause — see MAPTYPE-4. A blank is the less dangerous failure: a wrong number is worse than none. |
...
| MAPTYPE-4 | ✅ **CLOSED** (this phase) | A **type** built-in read from the **instance** returns null, so the mapping wrote nothing and `MarkBipMissing` then CACHED the miss for the session. `FAMILY_WIDTH_PARAM` / `FAMILY_HEIGHT_PARAM` are type-scoped, which is why a 900 mm door reported no width. The differential that proved it, on one door: `BLE_DOOR_HEAD_HEIGHT_MM` = 2000 (`INSTANCE_HEAD_HEIGHT_PARAM`) beside `BLE_DOOR_WIDTH_MM` = empty. Fixed once in `ReadBipInstanceOrType` / `ReadNamedInstanceOrType` rather than per call site, so every helper and every future category is covered. Only the SOURCE may come from the type; targets stay on the instance, and CachedLookup is deliberately NOT changed because it also resolves write targets. |
| MAPTYPE-5 | **OPEN — needs Revit** | MAPTYPE-4 is a source-read change verified by the compiler and six gates, not by a tag. Re-run Auto Tag on the door from the finding and confirm `BLE_DOOR_WIDTH_MM` = 900, `BLE_DOOR_HEIGHT_MM` = 2000, and the same for a window. |

## Binding scope, and what one door in Revit found (2026-09-16, Phase 294)

Five tag defects, all found in a single Revit session by placing one door and reading the
log — none of them visible to any of the five gates that existed before. Every one had the
house failure shape: bound, present, writable-looking, silently wrong.

| ID | Status | Detail |
|---|---|---|
| BINDSCOPE-1 | ⚠️ **PARTLY RETRACTED 2026-09-17 — the CSV edit changes nothing at runtime** | The defect is real and reproduced: `ASS_SEQ_NUM_TXT` type-bound, greyed, reading `-215`, and a door tagged `A-BLD1-Z01-L01-ARC-FIT-DR-` with no SEQ. **The fix was not.** 1,198 rows were flipped Type → Instance in `CATEGORY_BINDINGS.csv` — and `SharedParamGuids.LoadPerParamCategoryBindings` reads `cols[0]` and `cols[1]` and **never reads `cols[2]`**, while `LoadSharedParamsCommand` calls `NewInstanceBinding` unconditionally. The scope column is not an input to anything. The rows are now accurate documentation and the gate still stops the data drifting, but the type binding in an affected model came from a template, a manually loaded shared-parameter file or an earlier binder — and only a re-bind inside Revit can move it. That is BINDSCOPE-2. **This is the second time this phase that a fix was asserted before its mechanism was traced end to end.** |
| BINDSCOPE-2 | ✅ **CLOSED — the migration now exists** (this phase) | `Tags/MigrateBindingScopeCommand.cs`: **Scope Audit** (ReadOnly) walks `Document.ParameterBindings` and reports STING per-element parameters held by a `TypeBinding`; **Fix Scope** (Manual) re-inserts them as `InstanceBinding` over the same categories. Both on the SETUP tab beside **Load Params**, dispatched as `AuditBindingScope` / `MigrateBindingScope`. ⚠️ **Re-binding Type → Instance DISCARDS values held on types** — Revit stores the two scopes separately and offers no API to carry values across. For a SEQ or a LOC that is the point, but it is a real loss, so the count is shown and confirmed before anything is written. **Re-tag afterwards**: the migration only makes the per-element write possible, it does not perform it. |
| BINDSCOPE-3 | **OPEN — reported, not a defect list** | 16,752 rows remain Type-bound and most are correct — a door's width does belong to the door type. Only the intersection (Type **and** written per element) was touched. Reclassifying the rest needs a per-parameter judgement, and a wrong flip writes one element's value onto every sibling. |
| TAGLOG-1 | ✅ **CLOSED** (this phase) | `TagCollisionMode.Skip` returns `false` **on purpose** for an element that already carries a complete tag, and the caller logged every false as `BuildAndWriteTag failed`. A clean run of 332 elements emitted **278** such lines and **one** genuine fault — the real one buried in a 278:1 noise ratio. Fixed with an optional `TagWriteReport` out-channel: every existing return value is unchanged, so the eleven call sites that ignore the bool keep their exact behaviour, and the five that log now warn only on `Failed`. |
| TAGPROD-1 | ✅ **CLOSED** (this phase) | `GetFamilyAwareProdCode` minted `$"{baseProd}-{suffix}"` → `FSP-CON`, `DR-GLZ`. `-` is the tag separator, so `SanitiseSourceTokenWrite` — which exists to stop corrupt values reaching a token — truncated it back to `FSP` on every write and logged it as malformed. One half of the codebase minting what the other half is built to reject: **no material suffix has ever reached a tag**. Now joined by `ProdSuffixJoin()`, which picks the first candidate that is not the active separator, so the collision cannot return when a project overrides `Separator`. |
| TAGPROD-2 | **OPEN — residual risk, stated not gated** | `check_token_separator_safety.py` guards the source join only. Its first draft also scanned the 13 `*TAG_CONFIG*.csv` files for vocabulary codes containing `-`; that half reported 0 and **would have reported 0 for ever**, because it looked for a header row and those files have none — it passed an injected `DR-GLZ` without a word. The obvious repair (flag short ALL-CAPS hyphenated cells) collides with the standards citations the same files carry (`HTM-02`, `BS-EN`). Dropped rather than shipped vacuous. A hand-authored code containing `-` would still be truncated; the only signal is the `SetString` warning. |
| MAPTYPE-6 | ✅ **CLOSED** (this phase, found by the pre-test sweep) | `SetString` on a non-String target was a bare `return false`. **52 write sites across 36 parameters**, measured tree-wide, pass a formatted string to a target declared NUMBER or YESNO in `MR_PARAMETERS.txt` — so `ELC_VLT_DROP_PCT`, `ELC_CDT_CBL_FILL_PCT`, `ELC_PNL_SHORT_CIRCUIT_RATING_KA`, `HVC_PEAK_SENS_W` / `_LAT_W` / `HVC_OA_LS` and `ELC_EMERG_COVERED_BOOL` never reached the model. A voltage drop, a conduit fill, a panel fault rating, a block load and an emergency-lighting coverage flag. Now writes Integer / YesNo / unitless Double; **the unit trap is refused, not guessed** — a LENGTH target is logged and declined rather than storing a millimetre display value as feet, which is MAPTYPE-1 reintroduced by a convenience. Strictly additive: the path was previously unreachable by definition. |
| MAPTYPE-7 | ✅ **CLOSED** (Phase 294) | All ten type-scoped built-ins read from instances now route through `ParameterHelpers.GetBip` / `TryGetBipDouble`. Re-measured after the fix: the one remaining match is `TypeMarkSequencer.cs:386`, whose receiver is already a TYPE. |
| DOCDRIFT-1 | ✅ **CLOSED** (this phase) | `CLAUDE.md` claimed the block-load `HVC_PEAK_*` stamps "are TEXT-typed". They are declared **NUMBER**. The row was describing the write, not the parameter — and the write did nothing at all until MAPTYPE-6. Corrected in place rather than deleted, so the mistake is visible. |
| GENPH-1 | ✅ **CLOSED** (this phase, found by the category-by-category audit) | `"GEN"` was both a **sentinel** in `TagConfig._placeholders` (meaning "never resolved") and a **real code** in three places: `GetDiscDefaultSysCode case "G"`, the `SysMap` key covering **12 categories** (Generic Models, Specialty Equipment, Medical Equipment, Mass, Parts, Assemblies, Detail Items, Model Groups, Materials, Profiles, RVT Links, Zones), and `ProdMap["Generic Models"]`. So every element in those categories produced a tag containing `-GEN-`, which `TagHasPlaceholders` rejects — **their tags could never be complete.** Silent consequences: `skipComplete` never skipped them so every run re-derived and rewrote the tag and churned the audit trail; the idempotency guard could never fire; and `ComplianceScan` counted them non-compliant for ever, so the dashboard could not reach 100% whatever anyone did in the model. Real codes are now `GNL` (sys/func) and `GM` (product); `GEN` means only "unresolved". Guarded by `tools/check_tag_vocabulary.py`, proven RED before GREEN. |
| GENPH-2 | ✅ **CLOSED — answered by the standard, not by me** | The letters did not need inventing. `Core/Drawing/Iso19650Vocabulary.cs`, citing **BS EN ISO 19650-2 §A.5**, already carried the role table: `Z` = General / multi-disciplinary. The twelve generic categories now use `Z`. `GNL` / `GM` remain the sys/func and product mnemonics, which no ISO table supplies — see ISO19650DISC-2. |
| PRODDUP-1 | **OPEN — reported, low severity** | `COL` is the product code for **both** `Columns` and `Structural Columns`. The full tags still differ because DISC differs (A vs S), so no tag is ambiguous today — but a code that is supposed to identify a product identifies two. Splitting it (e.g. `COL` / `SCO`) is a naming call, not a bug fix, so it is reported rather than changed. |
| CATCOV-1 | **OPEN — reported, measured** | 11 categories are tagged (`DiscMap`) but have no `LABEL_DEFINITIONS` entry — Analytical Duct/Pipe Segments, Area Based Loads, Curtain Panels / Systems / Wall Mullions, MEP Ancillary, Railings, Structural Connections, Temporary Structures, Wash. They receive tag parameters but no tag family rows are specified, so there is nothing to display. **Railings and Curtain Wall Mullions look like genuine omissions**; the Analytical ones probably should not be tagged at all. |
| CATCOV-2 | **OPEN — reported, NOT a defect** | 25 tagged categories have zero rows in `CATEGORY_BINDINGS.csv`. Checked before reporting: the eight ISO tokens bind `<ALL>`, so those categories **do** still get tags — only the extended per-discipline parameters are absent. Recorded so the number is not re-discovered and mistaken for the more alarming thing it resembles. |
| ISO19650DISC-1 | ✅ **CLOSED** (this phase) | `TagConfig.DiscMap` coded twelve categories `"G"`. Per **BS EN ISO 19650-2 §A.5** — and per `Core/Drawing/Iso19650Vocabulary.cs`, which has carried the table in this repo all along — **`G` is GIS / Land Surveyor** and **`Z` is General / multi-disciplinary**. Two subsystems in one codebase gave one standard code two meanings. Now `Z`. Done in the same phase as GENPH-1 deliberately: those categories could never produce a complete tag before, so **no correct tag exists for them yet and there is nothing to migrate** — the cheapest possible moment. `DrawingDispatcher` also mapped `civil` → `G`; civil is `C`. Guarded by `check_tag_vocabulary.py`, which now compares DiscMap against the vocabulary FILE rather than a re-typed list, so the two cannot drift. |
| ISO19650DISC-2 | **OPEN — a decision, and an honest limit** | **ISO 19650 does not define product codes.** It defines the information-container *role* field (§A.5), which is the DISC segment — and `A`/`S`/`M`/`E`/`P` are already correct. The standard for products is **Uniclass 2015 `Pr_` tables**, which STING already carries in a SEPARATE parameter (`ASS_UNICLASS_2015_TXT`, `STING_UNICLASS_MAP.csv`). So the PROD mnemonics (`GRL`, `DAC`, `AHU`, `GM`) are a **house layer on top of the standard, not a substitute for it** — which is a defensible design, but should be stated in the BEP rather than assumed. The real standards gap is that **18 of 45 categories have no Uniclass entry** (`STING_UNICLASS_MAP.csv` covers 27). Filling them is a data edit; I have not invented `Pr_` codes from memory because a wrong classification is worse than a missing one. |
| ISO19650DISC-3 | ✅ **CLOSED** (this phase) — **and my scoping of it was wrong** | I recorded this as touching "the healthcare pack’s 60 tag families", from CLAUDE.md’s claim of "3 new disciplines (`H` Healthcare...)". Measured instead: **`DiscMap` has no `H`, `MG` or `RP` entry at all**, and three call sites already treated `H` correctly as Heating & Ventilation. The real surface was the drawing layer — **10 drawing types + 6 routing rules** — and the real consequence was worse than a naming clash: `Iso19650DocumentCode` folds `H` → `M`, so **every healthcare drawing was filed under Mechanical**, attributing it to the M&E engineer. Exactly the failure that file’s own comment describes for fire protection under `S`. Now `HC`, mapping to role `Y` (Specialist Designer) alongside `RP` and `FP`; healthcare sheets get their own prefix instead of inheriting `M`; drawing-type checksums re-stamped (10 drifted, round-trip verified). |
| ISO19650DISC-4 | **OPEN — a BEP question, deliberately not migrated** | `LV`, `FP` and `FLS` are multi-letter house extensions, so unlike `G` and `H` they **cannot collide** with the single-letter role table — there is no defect to fix. They are systems rather than roles, and `Iso19650DocumentCode` already folds them to standard roles on issue (`LV`→`E`, `FP`→`Y`), so issued containers are already standard-compliant. Changing the internal codes would lose the system distinction STING uses for routing and gain nothing on the drawing. What remains is to **state in the BEP** that the DISC segment means discipline/system, not the ISO role field — a sentence, not a migration. |
| CATBIND-1 | ✅ **CLOSED** (this phase, third sweep) | `SharedParamGuids.LoadPerParamCategoryBindings` skips any category name `CategoryEnumMap` does not know — **no log line, the binding simply never happens**. 89 of the 194 names in `CATEGORY_BINDINGS.csv` were in that state. Four were singular where Revit is plural, silently dropping **29 binding rows**: `Structural Connection`, `Curtain Wall Mullion`, `Curtain Panel`, `Railing`. They are the same four the label-spec audit flagged as tagged-but-unspecified — what a half-existing category looks like from two directions. **Fixed as aliases in `category_enum_map`, not by renaming the CSV**: renaming was tried first and `check_tag_row_bindings` caught it, because `LABEL_DEFINITIONS.json` and the schedule spec key off the SINGULAR names — the rename would have killed 29 tag rows and 25 schedule columns to save 29 bindings. Guarded by a near-miss check in `check_binding_scope.py`, proven RED. |
| CATBIND-2 | ✅ **CLOSED — by making the drop LOUD, not by guessing** | The 85 names with no near match are spec families, not Revit categories (`Clinical Room`, `MEP Sleeve`, `Architectural Sheet`), and their 242 rows are inert. Mapping them would mean guessing a category, and binding a parameter to the WRONG category is worse than leaving it unbound. **The defect was the silence, not the rows** — the loader discarded 242 rows on every load with nothing to show for it, which is exactly why a singular/plural typo (CATBIND-1) survived. `LoadPerParamCategoryBindings` now emits **one summary line** naming the count, the distinct names and a sample — one line, not 242, because per-row warnings at that volume are the noise that hid the single real fault in TAGLOG-1. A `CategoryEnumMap` entry that names a non-existent `BuiltInCategory` is now reported too. |
| CATBIND-3 | ✅ **CLOSED** | 207 duplicate rows removed (19,616 → 19,409). Checked before deleting: **0 of the duplicate pairs disagreed with each other**, so this was pure noise and no binding changed. Worth the check — a duplicate pair with different scopes would have been a real conflict, not tidying. |
| SYSAMB-1 | ✅ **CLOSED** (this phase) | Two separate problems under one symptom. **(a) A genuine duplication:** `FLS` was a strict SUBSET of `FP` — both `Fire Alarm Devices` and `Fire Protection` appeared in each, so which code an element got depended on `Dictionary` order. Split on the domain line: detection is FLS, suppression is FP. 27 ambiguous categories → 25. **(b) The remaining 25 are real**, and every one is an **overlay**: `LPS` (lightning protection) is listed against Walls, Roofs, Gutters, Structural Rebar, Conduits and Electrical Equipment, because any of those CAN carry it. `GetSysCode` returned `list[0]` — **had LPS been enumerated first, every wall in the model would have tagged as lightning protection, and nothing would have said so.** An overlay now never wins the fallback (lightning protection is asserted per element by `LpsMarkElementTypesCommand`, never assumed), and an ambiguous resolution is logged **once per category per session**, not once per element. |
| IDENTITY-1 | ✅ **CHECKED CLEAN — recorded so it is not re-audited** | Across **3,604** declared shared parameters: **0** GUIDs reused by more than one name (Revit would treat those as one parameter), **0** names carrying more than one GUID (the binder could bind the wrong one), and **0** disagreements between `PARAMETER_REGISTRY.json` and `MR_PARAMETERS.txt` across the 384 registry entries that carry a GUID. The identity layer is sound. |
| CREATEPARM-1 | ✅ **CLOSED** (2026-09-17) | **57 → 0.** Categories derived from the WRITE RECEIVER, not a file census and not the name — which corrected two: `ELC_WIRE_*` binds to **Conduits** (the receiver is literally `conduit`; `WireElementAnnotationCommands:16-19` states no `ELC_WIRE_*` is written to wires and defers the `OST_Wire` question, so binding it would have pre-empted a deferred decision) and `ELC_LIGHTING_*` to **Lighting Fixtures** (receiver `fi`; the `OST_Rooms` collector is the iteration source). Rows alone were **not sufficient** — `param_binding_resolver.py` overrides by prefix rule, so `HVC_PEAK_*` fell into `MEP_ALL` and `PEN_*` onto hosts only. Added an `HVAC_SPACE` group (`MEP Spaces|Rooms`) and `Specialty Equipment` to `PEN`; regeneration diff was 64 lines, all intended. `BLE_ROOM_NUM_TXT` mirrors its sibling `BLE_ROOM_NAME_TXT` rather than going universal. See `docs/CREATEPARM_BINDING_DRAFT.md`. |
| CREATEPARM-2 | ✅ **CLOSED — answered, and one answer is permanent** | The 103 non-written skips are correct. **`TB_*` can never bind**: the resolver’s own comment records that mapping it to Title Blocks was committed, deployed and run, and all 33 came back skipped because `OST_TitleBlocks` answers false to `Category.AllowsBoundParameters` — a Revit rule, not a gap in the data. `TAG_*` is `annotation-only` by rule, `Qto_*` is an IFC mapping vocabulary, `WARN_*` are family parameters injected into `.rfa` files. |
| CREATEPARM-3 | ✅ **CLOSED** | `tools/binding_simulator_baseline.txt` still held 3,401 declared / 3,238 bound / 163 skipped from `claude/kibale-integration`. The file has since grown by 203 parameters, so the baseline could not have caught a rise in the skip count — a stale baseline is a gate that has stopped watching. Refreshed to 3,604 / 3,444 / 160 with the reason recorded in the file. |
| LIGHTGRID-1 | ✅ **CLOSED** (2026-09-17) | `LightingGridCommand` collected `OST_Rooms` only, so on an MEP model — where the lighting designer works in **Spaces** — it found nothing to light, placed no fixtures and reported success. A silent no-op across a whole class of model. The fix was small because the engines were already wider than the command: `LightingGridCalculator.Compute` and `ObstructionIndex.BuildForRoom` both take a `SpatialElement`; only the collector and the plan’s field type were narrower, which is exactly why the limitation was invisible. Now collects Rooms **and** MEP Spaces, de-duplicated, Rooms first so an architectural model behaves identically. `ROOM_NAME` is a Room built-in and is null on a Space, so the label falls through to `SpatialElement.Name`, which both carry. |
| LIGHTGRID-2 | ✅ **CLOSED** (2026-09-17) | All seven remaining lighting commands now see MEP Spaces: `ComCheckExport`, `EmergencyLightingAudit`, `LightingCalcSheet`, `LightingControlZone`, `LightingPowerDensity`, `QuickLuxEstimate`, `PhotometricDesignReview`. Done one at a time with a build between each, as promised. The shared `Core/Placement/SpatialCompat.cs` carries the **five things Room and Space do not share**, each of which fails silently and plausibly: `Number` (declared separately, absent from `SpatialElement`), name (`ROOM_NAME` is null on a Space), `FamilyInstance.Room` vs `.Space`, `IsPointInRoom` vs `IsPointInSpace`, and `GetRoomAtElement` returning only a Room. Writing those inline eight times is how a fix of this shape goes wrong. `LightingGridCommand` was folded onto the same helper so there is one implementation, not two. **Not verified in Revit** — a Spaces-based model is needed; the compiler and the gates confirm only that nothing regressed. |
| LIGHTGRID-3 | ✅ **CLOSED — measured properly, and the answer was not 79** | I had classified the remainder by domain intuition. Measured by BEHAVIOUR instead — does the file TYPE to `Room`, or merely name the category — the 79 split: **56 only mention `OST_Rooms`** in filters, pickers, category lists and dispatch and have nothing to change, and **23 actually iterate rooms**. Of those 23, **five needed the fix and got it**: `ElectricalSnapshotBuilder`, `IfcResultsImportCommand`, `MultiEngineAggregatorCommand`, `PhotometricPreflightCommand`, and `UniformityValidator` — the last one filed under `Core/Validation` but a lighting check by nature, which is exactly why a directory is not evidence. **18 remain and are correctly room-native**: room numbering, room tags, finishes, room data sheets, viewports, model creation. Converting those would be a regression. |
| LIGHTGRID-4 | ✅ **CLOSED — and my description of it was wrong** | I wrote that `BuildRoomIndex` drives `DetectLoc` / `DetectZone`. It does not. That index is keyed by **room id** and looked up by **element id**, so for any normal element it always misses and falls through — it is a cache and an "are there rooms at all" flag, not the resolver. The real resolver is **`GetRoomAtElement`**, which is Room-only at every step (`FamilyInstance.Room`, then `Document.GetRoomAtPoint`), and both detectors open by calling it. So the conclusion held and the mechanism did not. **Fix:** a new `GetSpatialAtElement` tries the Room path first and falls back to `FamilyInstance.Space` / `Document.GetSpaceAtPoint`; `DetectLoc`, `DetectZone` and `MapRoomNameNumber` use it. **Rooms are tried first and still win**, so an architectural model — or a mixed model with Rooms and Spaces over the same floor — resolves exactly as before: this can ADD a derivation, never change one. `GetRoomAtElement` is untouched, keeping its 26 call sites across 18 files unaffected. |
| LIGHTGRID-5 | **OPEN — needs Revit, and it is the check that matters** | LIGHTGRID-1..4 are verified by the compiler, 1,542 tests and seven gates — none of which can see a Revit Space. **On a Spaces-based model:** tag an element and confirm LOC and ZONE now derive rather than falling back, and that `ASS_ROOM_NAME_TXT` / `ASS_ROOM_NUM_TXT` fill. **On an architectural (Rooms) model:** confirm the tokens are UNCHANGED — that is the regression risk, and the room-first ordering is the reason to expect none. Then run Quick Lux on the Spaces model: it is the cheapest proof the lighting suite works. |
| MIRROR-1 | ✅ **CLOSED** (2026-09-17, found in Revit) | `SetDouble` returned early when the parameter already held a value — **skipping `WriteTxtMirror` with it**. The tag label reads the `_TXT` MIRROR, not the `_MM` parameter, so any element whose value was written by an earlier run had a permanently blank tag row over correct data, and re-tagging could never heal it: the value being present is exactly what skipped the mirror. Observed on a door with `BLE_DOOR_WIDTH_MM` = 750 and `BLE_DOOR_WIDTH_TXT` empty. The mirror is now written on that path too — **but only when the caller supplied `displayText`**, because `existing` is the raw internal value and falling back to it would publish decimal feet under a millimetre label: MAPTYPE-1 re-introduced while fixing a blank, trading a missing number for a wrong one. |
| ROOMTAG-1 | **OPEN — evidence gathered, cause NOT established** | A room tags as `A-BLD1-Z01-L01-ARC---`: DISC, LOC, ZONE, LVL and SYS resolve, **FUNC and PROD are empty** (SEQ is the separate BINDSCOPE-2 issue). Ruled out by inspection: the vocabulary is complete (`DiscMap[Rooms]=A`, `SysMap[ARC]` contains Rooms, `ProdMap[Rooms]=RM`, `FuncMap[ARC]=FIT`), `ProdResolver.Resolve` falls through to the category default and would return `RM` or at worst `GEN`, and the log records **no** read-only skips and **no** SetString failures. A Door in the same run gets `FIT-DR_GLZ` correctly, so it is specific to Rooms — the obvious difference being that a Room is a `SpatialElement`, not a `FamilyInstance`. **Not guessed at**: every fix this phase that was asserted before its mechanism was traced turned out wrong. |
| CONTAINER-1 | **OPEN — measured, unexplained** | `WriteContainers: failed to write ASS_TAG_1_TXT` appears **175 times** in the 2026-09-17 run, across many elements. `BuildAndWriteTag`’s own TAG1 write succeeds (the tags exist), so this is the container pass specifically. Was 1 occurrence in the earlier run on the same model; needs its own look. |
| TAGDEPTH-1 | **OPEN — a rule contradicts a write** | `TAG_PARA_DEPTH_INT` is written onto a tag by `SmartTagPlacementCommand`, but `param_binding_resolver.py` classifies every `TAG_*` parameter as `NONE` / "annotation-only" by rule, so it resolves to nothing. The four tag categories were added to `category_enum_map` and the rows are in `CATEGORY_BINDINGS.csv`, but the rule still wins. Either the rule needs a carve-out or the write should target the host — a decision, not a typo. |
...
`tools/check_tag_row_bindings.py` with a ratchet baseline at
`docs/TAG_ROW_BINDING_BASELINE.json` — it may shrink, never grow.

| ID | Status | Detail |
|---|---|---|
| TAGBIND-1 | ✅ **CLOSED** (Phase 290) | 624 rows of template spillage removed (8 params × 78 categories); the other 370 were authored per-category and are now bound. |
| TAGBIND-2 | ✅ **CLOSED** (Phase 290) | Areas / Spaces / Zones have bespoke spatial templates and `MapSpatialElement` / `MapZoneElement` writers. 172 generic asset rows → 29 spatial ones. |
| TAGBIND-3 | **OPEN — deliberately not closed, analysed 2026-09-16** | Adding `BLE_DOOR_CLEAR_WIDTH_MM` was attempted and stopped. Three reasons, any one sufficient: (1) Revit has **no native door clear width**, so the writer would read a family parameter named "Clear Width" that most families do not declare — producing a bound, writable, **empty** row, which is the state this phase spent itself removing; (2) deriving it (leaf width − stop − projection) needs family-specific geometry STING does not have, and guessing is what created the original defect (the row displayed leaf width under a Part M label); (3) the GUID convention in MR_PARAMETERS.txt is **not** `uuid5` under `transform_mr_params.py`'s namespace — verified against three existing parameters — so minting one would invent a house rule. The row stays removed. Closing this needs a decision about where clear width comes from, which is a standards question, not a coding one. |
| TAGBIND-4 | **OPEN — needs Revit** | Every fix in Phase 288 is a data change verified against shipped data. **None has been seen on a tag in Revit.** Place a room, door and window tag at tier depth 3 and confirm the previously blank rows now carry values — finishes, headroom, occupancy, ventilation, escape capacity on the room; type, operation, glazing, threshold on the door; vent area on the window. |
| TAGBIND-5 | ✅ **CLOSED** (Phase 291) — by classification and one writer | `BLE_DOOR_TYPE_TXT` now maps from the door's type name. `BLE_DOOR_AREA_SQ_M` / `BLE_WINDOW_AREA_SQ_M` already had a producer — the FormulaEngine computes them from width × height, which were themselves dead until Phase 289, so the formula was starved rather than missing. The rest (`BLE_ROOM_OCCUPANCY_NR`, `BLE_ROOM_FIRE_ESCAPE_CAPACITY_NR`, door threshold / operation / glazing type) are genuine DESIGN INPUTS with no derivable source; they are recorded in `PARAM_CONTRACT_BASELINE.json` with role `input` rather than left looking unfinished. |
...
| ROOM-2 | ✅ **CLOSED** (Phase 288, #958) | `STING_TAG_TOKEN_POLICY.json` is read by `TagConfig.BuildAndWriteTag` through `TagTokenPolicyRegistry`; `check_token_policy_wired.py` keeps it that way. |
| ROOM-3 | ✅ **CLOSED** (Phase 291) | Room Audit reports through `StingResultPanel` with one clickable row per finding (click selects the room), amber metrics for non-zero counts, and the cap raised 50 → 300. The plain text is kept as `RawText` so the clipboard copy is unchanged. |
| ROOM-4 | ✅ **CLOSED** (Phase 291) | Stated in the renumber preview rather than left to be discovered: room numbers and the ISO SEQ counter are independent, renumbering a room does NOT renumber the assets in it (their tags identify the asset), and the LVL token comes from the same level code so the two cannot disagree about level. |
| ROOM-6 | **OPEN — needs Revit** | Phase 287 fills `ASS_DESCRIPTION_TXT` from the room name so tier 2 of the room tag stops rendering a blank. Confirmed by code inspection only: place a `STING - Room Tag`, set tier depth to 2, and check the name appears between the ISO code and `Area:`. If it does not, the likely cause is that the `.rfa`'s tier-2 row 3 was never authored (the label rows are hand-built — see `docs/` notes on the universal-tag work), not the mapping. |
| ROOM-7 | **OPEN — manual Family Editor work, scoped** | A name-first room tag (`STING - Room Name Tag`): tier 1 = `BLE_ROOM_NAME_TXT`, tier 2 = number, area, department, tier 3 = the ISO code. Needed because the existing tag is code-first — at Compact depth a room reads `A-BLD1-Z01-GF-ARC-FIT-RM-0007` and nothing else, which is right for asset tagging and wrong for an architectural floor plan. **The plugin cannot build it.** The Revit API cannot author label rows (`TagFamilyCreatorCommand.cs:1496`) and cross-category label paste is blocked, so this is: create the family from `Room Tag.rft`, run `MigrateTagFamilies` to inject the params and the 128 type variants, then hand-author ~6 label rows in ONE Edit Label session (params added to the field list are lost if the dialog closes before the rows are pushed). Everything except those six rows is automatable today. |
| ROOM-5 | ✅ **CLOSED** (Phase 291) | `Hvac_FanStaticReport` resolves in `WorkflowEngine.ResolveCommand`; `check_dispatch_parity.ps1` is clean. |
...
| ID | Item | Detail |
|---|---|---|
| PARAM-1 | ~~28 rows in `MR_PARAMETERS.csv` carry a `Group_Name` that disagrees with `MR_PARAMETERS.txt`~~ **CLOSED 2026-09-08** | `sync_csv_from_txt.py` now corrects `Group_Name` from the `.txt` the same way it already corrected `Data_Type`, and the 28 rows are swept. Drift re-measured 28 → **0** across all 3,598 rows. **The count was right; the breakdown in this row was wrong.** It said "six `BLE_SLAB_*` … and twenty-two `WARN_SHT_*`". There are only **twelve** `WARN_SHT_*`; the rest are seven `WARN_SLV_*` plus `WARN_PROP_CARBON_MATERIALS` (→ `STINGTags_ISO19650`, not `RGL_CMPL`), and **two rows drift the other way** — `BLE_DOOR_MATERIAL_FINISH_TXT` and `BLE_STRUCT_STEEL_SECTION_TXT` say `BLE_ELES` where the `.txt` says `CST_PROC`, which the row missed entirely. So: 12 + 8 + 6 + 2, not 6 + 22. **Two defects found that were not in the row, and the gate could not have been added without fixing them.** (1) The generator stamped `_dt.date.today()` into the file header, so regenerating changed the file on any day it ran — a "regenerate is a no-op" gate would have failed spuriously the day after any commit. The header now states only what the file HOLDS (row count), which is the rule the script's own comment already gave two paragraphs above the line that broke it; git records *when*. (2) It wrote through `Path.write_text`, whose newline translation is platform-dependent, so the same script produced different bytes on Windows and Linux. The writer is pinned to LF and `.gitattributes` pins the checkout, matching what `param_binding_resolver.py` and `binding-spec-drift.yml` already do. An **undeclared GROUP id now raises** instead of writing the raw id into the name column — the old new-row path did `GROUP_NAMES.get(id, id)`, which is how #758's `GROUP 38` produced rows naming a group that did not exist. `.github/workflows/param-csv-drift.yml` regenerates and diffs, modelled on `binding-spec-drift.yml`. **Sabotage-verified**, staging each mutation first because `git diff` compares the worktree to the index and an unstaged sabotage would fail the gate for the wrong reason: a `Group_Name` reverted to its drifted value, a hand-edited `Data_Type`, and a deleted row all fail it; the clean tree passes. Repeat runs are byte-identical. Nothing reads `Group_Name` from the `.csv` at runtime (`LoadSharedParamsCommand` groups from the `.txt`), so this was a consistency defect, not a live one — the value is that the two files are mirrors again and cannot silently stop being. |
...
| MOB-5 | ~~`--apply` on the TIDP merge leaves the register hand-edited~~ | **CLOSED 2026-09-09 (PR #898).** `--emit-python` is implemented: it prints each accepted row as the `row(...)` call `tools/midp_rows.py` is made of, so the register can be **regenerated** with the deliverable rather than hand-edited. `--apply` keeps its place as the Information Manager's working copy between issues and now points at the new mode on completion. Rows are printed, not written: placing a deliverable in the right section of a curated 122-row file is editorial, and a tool appending to the end would degrade it a little every run. Stage and discipline emit as the module CONSTANTS (`MOB`, `IM`) by reading `midp_rows`, not by restating the names. **The `source` field is derived, not invented** — the first draft emitted `'New'`, which `build_midp.SRC_LABEL` does not map, so every emitted row would have looked right and raised a `KeyError` at the next regeneration; it now copies the newest `New …` source already in the file. Verified by round trip: the emitted lines are executed against the real `row()` and the resulting dict is asserted field by field, because `row()` is positional for fifteen arguments and a mis-ordered mapping produces a call that runs and files every value under the wrong name. |
...
| MOB-11 | ~~THE SHAPE: a parameter named by string, written with `SetString`, and never checked against the shipped data~~ **PARTIALLY CLOSED 2026-09-08 — the shared assertion exists and is gated; 17 known-bad names are baselined, not repaired** | `tools/check_param_name_targets.py` + `.github/workflows/param-name-targets.yml` implement the single shared assertion the row asked for, over the four sources it named as unaudited: the COBie attribute templates, the ExLink link profiles, `NativeParamMapper`, and the Fohlio map. It was never run before; the audit found **52 call sites naming 40 distinct parameters, of which 17 do not exist in `MR_PARAMETERS.txt` at all**. **The row's proposed assertion was wrong in one respect and the gate does not implement it.** It asked to assert `DATATYPE=TEXT` for every named parameter. That is not right as a blanket rule: `NativeParamMapper` writes `BLE_CEILING_HEIGHT_MM` (LENGTH) and `HVC_EFF_RATIO_NR` (NUMBER) through `MapDimension`/`MapLookup`, which are numeric writers. TEXT is a requirement of `SetString` targets, not of named parameters generally, and a static gate cannot tell which writer a name reaches. Asserting it would have failed three correct call sites. **The severity splits in two, which the row did not distinguish.** Eleven of the seventeen are `StingParamKey` values in `COBIE_ATTRIBUTE_TEMPLATES.csv` — and that column is **never passed to `SetString` or `LookupParameter`**. `COBieDataCommands` only prints it into a TaskDialog as `AttributeName [unit] → STING:KEY`. So nothing is silently discarded there: the software *tells an engineer to populate a parameter that does not exist*, which is a defect of guidance, not of data. The other six are `ExLinkDefaultLinks` profile properties, which **are** real export/import targets and **are** the MOB-11 shape — those columns export blank and those imports land nowhere. **Nothing was repaired, deliberately.** Almost every one has several plausible corrections differing by unit or by which element carries the value (`ELC_POWER` → `ELC_PWR_KW` or `ELC_PWR_TXT`; `HVC_PRESSURE` → `HVC_PRESSURE_DROP_PA` or `HVC_PIPE_PRESSURE_KPA`, which are different quantities; `BLE_FLOOR_FINISH_TXT` → `BLE_FLR_FINISH_TXT` or `BLE_FLR_FINISH_MAT_TXT`, which are different facts). Choosing is an authoring decision, and choosing wrong writes a value into the wrong field — worse than writing none, because a wrong value looks supplied. The candidates are recorded per name in `tools/param_name_targets_baseline.txt` so the decision is cheap for whoever owns the parameters. **The baseline only shrinks**: the gate fails on a new bad name, on a baselined name that starts existing, and on a baselined name nobody references any more. Seven narrowly-bound targets are **reported** rather than failed, satisfying the row's "visible rather than silent" requirement without making a registry decision. **Sabotage-verified four ways**: a new invented name, a baselined name that now exists, a stale baseline entry, and an unreadable `MR_PARAMETERS.txt` — the last because a checker whose authority fails to parse must fail loudly rather than report everything as fine. Still open: choosing the 17 corrections, and extending the extractor to the ISB schedule commands and the COBie Job/Spare maps (neither names parameters today). |
...
| PROD-3 | ~~Four of the seven columns in `STING_PROD_CODES.csv` are read by nothing~~ | **CLOSED 2026-09-09 (PR #897).** Took the row's second option, and then went further than it. The file header now states plainly that the plugin reads columns 0-2 and stops, that editing `DISCIPLINE` there changes no tag, and that the other four are reference only. But *documenting* silence does not end it, so the four are now **asserted**: `DISCIPLINE` and `SYSTEM` must be codes `TAG_CONFIG_v5_0_DISC_SYS_FUNC.csv` declares, no PROD code may declare two `SYSTEM`s (the mirror of the existing discipline test), and `DESCRIPTION` / `STANDARD_REF` must be present — all 277 rows pass today. **Six `SYSTEM` values are declared nowhere** (`FA`, `LTG`, `MGS`, `RAD`, `HV`, `BMS` — 43 rows); none is a typo, so the repair is to add SYS rows to the vocabulary, which feeds `TagConfig.SysMap` at runtime and therefore needs Revit. They are recorded with reasons, the list only shrinks, and a seventh cannot appear quietly. **Deliberately NOT wired**: putting `DISCIPLINE` into the tag's discipline segment changes tags on every existing model. **Deliberately NOT asserted**: that a row's `DISCIPLINE` equals the discipline its `CATEGORY` maps to — 105 of 277 differ, correctly, which is what `DISC_OVERRIDE` is for; that assertion would have failed 105 true rows, so a test now pins the reason instead. Two latent parser faults closed on the way: every reader splits on a bare comma (one quoted `DESCRIPTION` would shift `DISCIPLINE` into `SYSTEM`), and both readers skip exactly one line, so a `#` block above the header makes the header itself a data row — which is what the first attempt at this header did. Both now fail a test. |
| PROD-4 | ~~`ProdPatternMatcher`'s bare-substring path is an unanchored `Contains`~~ | **CLOSED 2026-09-08 (PR #896).** Both bare-substring sites now route through `Core/MaterialSchedule/PatternMatch.Contains`, so an alternative with no `*`, `?` or `[` matches a whole word: `PUMP` no longer matches inside `PUMPHOUSE`, `RC` inside `PORCELAIN`, or `DN20` inside `DN200` — a digit is a word character, so a gauge is not a prefix of another gauge. **The row's count was stale: 631 shipped alternatives, not 376** — but its conclusion held, because **0** of them are bare (measured the way `LoadProdCsv` reads the file, skipping `#` rows; a naive parse reports 5 and every one is a comment line). So no shipped tagging changes; what closes is the route in through a PROJECT overlay, which `Prod_GenerateRules` seeds from live family names. **`Strength()` was fixed too, not just `Matches()`** — they are separate loops, and fixing one alone leaves a rule that does not match yet still outranks the rule that does, which nothing would report. 17 new cases in `ProdPatternMatcherTests`, including an agreement test across the two paths; verified by reverting each half independently. |
...
| MATSCHED-3b | ✅ **Fixed (#700/#709 era; verified 2026-09-09, PR #899)** | The row described completed work but was never marked, so it read as open. `aggregate` and `tile-adhesive` matched neither a constituent kind nor a category and could never fire. **Corrected 2026-09-09: they were not both removed.** `aggregate` is gone; `tile-adhesive` is still in `STING_SUPPLIER_UNITS.json` and was REPAIRED, not deleted -- it now carries `matchKinds: ["tile_adhesive"]`, so it can fire. The substantive claim is unaffected and re-measured today: **0 rules match neither a kind nor a category**. Only the sentence was wrong, and a reader who checked the file would have concluded the row was lying.  `ShippedDataIntegrityTests.Every_Rule_Is_Reachable` guards against a third. **Verified by sabotage, not by the test existing**: adding a rule with no `matchKinds` and no `matchCategories` fails it. Dead config is worse than absent config — it advertises coverage the export does not have.
| MATSCHED-3c | ✅ **Fixed (verified 2026-09-09, PR #899)** | Also completed but unmarked. `STING_SUPPLIER_UNITS.json` and `STING_MATERIAL_STAGES.json` were each valid and each tested, yet disagreed — category rules inherited the ELEMENT's stage, filing wall paint under SUPERSTRUCTURE. A category rule must now declare its own `stageId`, and all three assertions the row promised exist by name: `Every_Category_Rule_Declares_Its_Own_Stage`, `Every_Declared_Stage_Exists_In_The_Stage_Library`, `No_Finish_Commodity_Is_Filed_Under_The_Frame`. **All three verified by sabotage**: blanking a `stageId`, naming a stage the library lacks, and filing paint under the frame each fail. Same class as the "valid JSON + green build + runtime-dead" risk recorded for this codebase.
...
| WF-6 | ~~No gate enforces `order` against array position~~ | **CLOSED 2026-09-08 (PR #894) — the row was stale when written.** Tier 3 of `tools/check_workflow_wiring.ps1` does exactly this and has since PR #635 (`bb52f6a42`); the row's claim that "the gate has Tiers 1, 2 and 4" was already false. Its second worry was unfounded too: the tier is a hard zero with no baseline and passes across all 48 presets / 387 steps, so no preset ever needed correcting or baselining — `order` and array position agree everywhere today, and the tier exists to keep it that way. Nothing to do beyond deleting the belief. |
| WF-7 | ~~`docs/UNREACHABLE_COMMANDS_TRIAGE.md`'s 126 figure predates the three-layer correction~~ | **CLOSED 2026-09-08 (PR #895).** Re-derived: **126 → 9**. The row said three layers; there are **six** — handler switches (6 files, not 1), `CommandRegistry` modules, `WorkflowEngine.ResolveCommand`, `.xaml.cs` code-behind, the legacy ribbon's `typeof(X).FullName`, and `.addin`/XAML/shipped-data references. Of 1,719 command classes, 1,688 are reached by a layer, **9** are named nowhere outside their own file, 0 are referenced only from non-dispatch code, and 22 are ambiguous (see WF-8). Now derived by `tools/recount_unreachable_commands.py`, gated with `--check` in CI, and no longer a hand-maintained roster of 103 class names. Two corrections the derivation forced, both of which had inflated the old figure: a name in a **comment** is not a reference (2 classes read as wired on prose alone), and a dispatcher may **declare** the command it dispatches (all 13 `Hub*Command` classes live in `StingToolsApp.cs` beside the ribbon call that reaches them). One Phase 177 claim in that file is **false** and is corrected there: `PluginOnboardingWizardCommand` was recorded as wired under tag `PlanscapeOnboarding`, which exists nowhere in the repository. |
| WF-8 | **11 `IExternalCommand` names are declared twice, so a bare-name reference cannot say which class it binds** | Found by the WF-7 re-derivation. 22 classes share 11 names — `ClashDetectionCommand`, `CrossModelClashCommand`, `MEPClearanceValidationCommand`, `NamingConventionAuditCommand`, `BOQExportCommand`, `ArcFlashCommand`, `AccessControlCommand`, `BatchPDFExportCommand`, `EnergyAnalysisCommand`, `LifecycleCostCommand`, `StickyNoteDashboardCommand`. Which one a `RunCommand<ClashDetectionCommand>` binds depends on the referring file's `using` directives, not on anything a reader can see at the call site — the same shape as the `LiveClashUpdater` namespace collision found under IM-6. **Five of the eleven are the same pair of files** (`Clash/ClashDetectionCommands.cs` against `Temp/DataPipelineCommands.cs`), which points at one wholesale supersession rather than eleven coincidences. Listed in `docs/UNREACHABLE_COMMANDS_TRIAGE.md`; resolving them means reading each pair and deleting or renaming the loser, which needs a judgement about which is current. |
...
| WF-4 | ~~Nine more step keys are silently dropped by `WorkflowStep`~~ | **CLOSED 2026-09-08 (PR #894).** Re-measured: the count is right but the framing was not. `tools/check_workflow_wiring.ps1` gained **Tier 5**, which fails on any step key `WorkflowStep` does not bind. The bound set is **derived** from the class's own `[JsonProperty]` attributes and property names (21 keys today), so a property added to `WorkflowEngine.cs` is understood immediately and no list can rot; if that parse yields fewer than 10 keys the gate fails loudly rather than reporting all 387 steps as broken. `order`/`id`/`_notes` are allowed as documentation and are **not** baselined — `order` is safe to leave in the files only because Tier 3 (which does exist — see WF-6) polices it against array position. The four that state real intent — `skipIfFamilyLoaded`, `scheduleNameFilter`, `drawingTypeId`, `sheetNumberFilter`, across two penetration presets — are recorded in `tools/workflow_step_key_baseline.txt`, one line each with the preset, the step and what the engine would need. That file is the live tracker and it **only shrinks**: the gate fails on a new unbound key, on a baselined key that becomes bound, and on a baselined key no preset writes any more. Choosing between implementing and deleting those four is a decision about what the penetration workflows should do in Revit and is not settleable from a terminal, so they are made visible rather than guessed at. |
...
| IM-18 | **`ProjectId` and `CoordContainerId` are project values living in a machine-scoped credentials file** | Found while making the ACC coordination cycle run unattended. `AccCredentials` (`StingTools/V6/AccIssueSync.cs`) is loaded from `%APPDATA%\Planscape\acc_credentials.json` — **per machine and per user** — and carries `ClientId` / `ClientSecret` / `RefreshToken`, which belong there, alongside `ProjectId` (the ACC Issues container) and `CoordContainerId` (the Model Coordination container), which do not. Those two identify **a project**, so the file as designed silently assumes one ACC project per coordinator. A coordinator on KUT and on a second job has one credentials file and needs two container ids; today the second job overwrites the first, and the only symptom is clashes pulled from the wrong container — which, since PR #927, at least reports itself rather than reading as a clean federation. **NOT FIXED, deliberately.** The pass that found it put the NEW project-scoped settings where they belong — `<project>/_BIM_COORD/acc/acc_settings.json`, resolved through `StingPaths`, read by `V6/AccOperatingPolicy.cs` — and did not migrate the two existing fields, because moving a value out of a credentials file is a migration with its own verification (every existing coordinator has a populated file; a reader that looks in the new place first and the old place second is another two-vocabularies-one-field shape, which is IM-17's failure mode). Fix: add both to `acc_settings.json`, have `AccCredentials` keep reading the old location for one release with a deprecation note, and delete the fields once the KUT project has been through one live cycle on the new path. Related: IM-17 (transmittal status), IM-14 (suitability vocabulary) — all three are the same shape, a field with two homes. |
| IM-6 | ~~`StingTools.Clash.Tests` does not build~~ **CLOSED 2026-09-08 — the row was stale; it was fixed by #553 and never updated** | Re-measured on `main` before believing the row: `dotnet build StingTools.Clash.Tests` reports **0 warnings / 0 errors**, and the suite runs **82 tests**. The 14 `CS0246` errors were real when written and were fixed by #553, which added `StingTools.Clash.Tests/TestHelpers/RevitStubs.cs` — the minimum `Autodesk.Revit.DB` surface for the linked sources to compile against net8.0 with no Revit DLLs on the host. That file's own header documents the same incident this row describes: `c98500b5a` added `using Autodesk.Revit.DB;` to five Clash files on 2026-05-17, and because a test project that does not compile reports **no red and no count**, 56 clash tests were offline for two and a half months while the coverage headline kept counting them. Its design rule is worth repeating: the stub TYPES exist so the code compiles, and their BEHAVIOUR THROWS — a stub returning an empty collection would let a test pass by measuring nothing. The row's remaining claim is also stale: coord-log unit tests are no longer only in `StingTools.Tags.Tests`. **One finding while verifying, recorded here because it is why the suite is 81/82 and not 82/82.** `DuplicateLiveClashUpdater_FileIsRemoved` is RED on `main`: `StingTools/Clash/ClashDetectionCommands.cs` was deliberately deleted and **is back** (65 KB, last touched by #796), still declaring `StingTools.Clash.LiveClashUpdater` whose `Register` logs *"Phase 106 stub: updater id reserved; triggers deferred"*. **The hijack the test names is latent, not active**: `StingToolsApp` sits in `namespace StingTools.Core` with `using StingTools.Core.Clash;` and no `using StingTools.Clash;`, so its bare `LiveClashUpdater.Register(application)` binds the REAL updater in `StingTools/Clash/LiveClashUpdater.cs`, which does attach triggers. The hazard is that any future file importing `StingTools.Clash` and calling the same bare name silently gets the no-op — and the stub's own comment says its GUIDs are distinct *"so both can coexist"*, so the duplication is deliberate rather than accidental. **Not resolved here**: the file also holds four live `IExternalCommand` classes (`ClashDetectionCommand`, `CrossModelClashCommand`, `MEPClearanceValidationCommand`, `NamingConventionAuditCommand`), so deleting it is a judgement about whether those commands are dispatched, and confirming the runtime effect needs a Revit session. |
