...
| PARAM-4 | `STING_VOLTAGE_TIER` / `STING_FEED_TYPE` are never stored on SLD symbols | `SLDGenerator.PlaceSymbols` stamps both on each placed symbol, but the symbols are Generic Annotation families (`STING_SLD_SYMBOLS.json` `familyType: GenericAnnotation`) and Revit will not bind a project parameter to that category, so defining them in `MR_PARAMETERS.txt` would bind nowhere. The warning is now reported once per parameter and family instead of once per symbol. To store them the SLD symbol builder must author both as family parameters. Revit's refusal is from the API rule (`AllowsBoundParameters`), **not re-checked in Revit on this branch**. |
| PARAM-5 | Projects that bound `ELC_CKT_NR` as NUMBER keep a NUMBER | `ELC_CKT_NR` is TEXT from 2026-09-24 (multi-pole numbers such as `1,3,5`). Revit keys a shared parameter by GUID, and the GUID is unchanged, so Load Params on an existing project leaves the old NUMBER binding; the writers already fall back to a numeric write and report `1,3,5` as refused. The fix for such a project is to remove the parameter and re-run Load Params (values on it are lost). A command that does this with a before/after count is not written. |
