...
| LIFE-6 | **`param_binding_resolver.py`'s outputs are stale against their own generator — 520 lines (deployable), 245 (audit view), 0 (gaps)** | Found while closing LIFE-5, and left alone deliberately. `tools/param_binding_resolver.py` generates this file, and re-running it on a clean tree is **not** a no-op: **+200** rows (`_TXT` mirrors added to `MR_PARAMETERS.txt` by later commits and never re-resolved — a genuine gap, since a mirror bound nowhere is a tag label reading nothing), **-10** rows (every `SHT_*` param loses its `Sheets` binding, because the generator excludes the `SHT` prefix; absent from this file means intentionally UNBOUND, so those ten go dark), and **~12** narrowed — seven of them `MAT_*`/`COMP_MAT_*` onto `Materials`, which `SharedParamGuids.LoadPerParamCategoryBindings` explicitly discards. So a regeneration is 200 fixes bundled with 22 regressions, and it belongs in its own change with its own evidence rather than inside a two-parameter PR. LIFE-5 therefore ran the generator, took only the two rows it needed, and asserted every other row byte-identical. Closing this means deciding, per group, whether the generator or the committed file is right — most likely by teaching the resolver about `SHT_*` and about `Materials` before re-running.<br><br>**One run writes three files, and they are three different ages.** `param_binding_resolver.py` writes `docs/RESOLVED_BINDINGS.csv` (the full audit view: param, group, source, categories, desc — every parameter, including the 384 bound to nothing), `docs/binding_gaps.csv`, and `StingTools/Data/RESOLVED_BINDINGS.csv` (the deployable projection: name + categories, unbound rows omitted). These are different projections rather than copies, so they are *meant* to differ — but because one run writes all three, they should be the same AGE, and they are not. Measured on a clean tree at `b14721617` (2026-09-06), by regenerating and diffing each output against its committed copy — `git diff --numstat` gives 355+165 for the deployable and 221+24 for the audit view, matching a plain line diff. The title previously read "574 lines", measured on an earlier tree; these figures move whenever `MR_PARAMETERS.txt` or the resolver changes, so **re-measure before acting on them** rather than trusting the number written here. As of that commit: `binding_gaps.csv` is current (0 lines), `docs/RESOLVED_BINDINGS.csv` is **245** lines stale, and the deployable is **520**. Three ages from one generator means outputs have been regenerated and committed separately, or edited by hand.<br><br>The consequence is worse than the staleness itself: **the file a human reads to check a binding is 275 lines fresher than the file the plugin ships**, so the audit view and the runtime disagree, and the audit view is the more reassuring of the two. Whoever closes this should regenerate and commit all three together, and consider whether the audit view needs to be a committed artefact at all — a view that can be regenerated on demand cannot drift from the deployable if it is not stored beside it. (Practical note: an accidental run dirties all three; restore all three.) |
