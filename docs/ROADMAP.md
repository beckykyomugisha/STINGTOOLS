...
| ID | Status | Detail |
|---|---|---|
| TOKPOL-1 | **OPEN** | The policy governs tag COMPOSITION (`TagConfig.BuildAndWriteTag`, `BuildSeqKey`). The DERIVATION layer keeps its own literals — `TokenAutoPopulator.PopulateAll` and `PopulateTagTokens` substitute `BLD1` / `Z01` / `L00`, and `SpatialAutoDetect.DetectLoc` / `DetectZone` return `"BLD1"` / `"Z01"` as their own last resort. Because derivation writes the token parameters FIRST, `BuildAndWriteTag` reads a non-empty LOC/ZONE and the policy's fallback value for those two never fires. Their LEVEL is still honoured (nothing is refused), but a project changing LOC's fallback to `"SITE"` will not see it. `tools/check_token_policy_wired.py` lists the seven sites. Closing this means routing the detectors through the policy — note `PopulateAll` compares `zone != "Z01"` to decide `zoneFromSpatial`, so the literal is load-bearing there and cannot simply be swapped. |
...
| TOKPOL-3 | **OPEN** | `SEQ`, `STATUS` and `REV` are described by the policy but never resolved through it — SEQ is generated by `SeqAssigner` (which already enforces the same refusal one layer earlier) and STATUS/REV are not part of the 8-segment string. Their entries are documentation. Either route them or say so in the file; it currently reads as if all ten are governed. |
...
| MAPTYPE-2 | **OPEN** | 28 targets were dead; the fix makes them writable, but a handful are `NUMBER`-typed targets fed from `p.AsValueString()` (`MapStringParam`) where the value string may carry a unit suffix ("3 W") that `int.TryParse` will reject. `WriteMapped` falls back to the raw internal value when the parse fails, which is right for `MapBuiltIn` but is a guess for `MapStringParam`. Worth a per-parameter look at `CST_FIX_LUMEN_OUTPUT_LM`, `ELC_PNL_MAIN_BRK_A`, `ELC_PNL_NUM_OF_WAYS_NR`, `FLS_PROT_FLS_RESISTANCE_RATING_MINUTES_MIN`, `LTG_FIX_LMP_WATTAGE_W`. |
| MAPTYPE-3 | **OPEN** | The 57 resolvable targets are the ones a regex can follow from `ParamRegistry.X` or a literal. `ParamRegistry.WriteContainers` resolves its targets from an array of VALUES with no parameter name at the call site (the reason `check_param_contract.py` is declaration-driven), so the true map-target count is higher than 57 and some of those may have the same defect. |
...
| ID | Status | Detail |
|---|---|---|
| TAGBIND-1 | **OPEN — 648 rows, 8 params** | `MEP_SYSTEM_NAME_TXT`, `MEP_SYSTEM_ABBREVIATION_TXT`, `MEP_UPSTREAM_ELEMENT_TXT`, `MEP_DOWNSTREAM_ELEMENT_TXT`, `COM_COMMISSION_STATUS_TXT`, `COM_TEST_CERT_REF_TXT`, `STR_STRUCTURAL_USAGE_TXT`, `STR_MATERIAL_GRADE_TXT` each appear in **81** category tags and are bound to only their own discipline. These came from a generic template applied to every category. Two defensible fixes and they are opposite: widen the bindings (MEP system name on a handrail is noise) or drop the rows from the categories that cannot use them (loses them where they WOULD work). Needs a decision per parameter, not a bulk edit. |
| TAGBIND-2 | **OPEN** | `Areas`, `Spaces` and `Zones` use the generic ASSET template — 15-row tier 2, 42-row tier 3 carrying Manufacturer, Model, Type Mark, Family Name, Host Element and MEP System. None of it applies to a spatial element, and several are dead for the same reason the room-name row was (no loadable family behind the category, so `ALL_MODEL_*` finds nothing). `Rooms` was given a bespoke 9-row tier 2; these three were not. They want the same treatment. |
| TAGBIND-3 | **OPEN — needs a new parameter** | Doors have **no clear-width parameter**. The tag's `Clear:` row was bound to `BLE_DOOR_WIDTH_MM` (the leaf width) and has been removed rather than left displaying a wrong number under a compliance-critical label. A real fix is `BLE_DOOR_CLEAR_WIDTH_MM` + binding + a writer that derives it (leaf width minus stop/projection, or read from the family), then a hand-authored label row. Approved Document M / BS 8300. |
...
| TAGBIND-5 | **OPEN** | The binding fixes only make a row *able* to display. Several of the newly bound parameters have **no writer**: nothing populates `BLE_ROOM_OCCUPANCY_NR`, `BLE_ROOM_FIRE_ESCAPE_CAPACITY_NR`, `BLE_DOOR_TYPE_TXT` or `BLE_DOOR_THRESHOLD_TXT` today, so those rows stay blank until a user types a value or a mapper is written. Binding was the necessary first step, not the whole job — and the tag now at least *can* show what someone enters. |
