...
## Material Schedule export — after Phase 245 (2026-09-06)
...
| MATSCHED-16 | ✅ **The model-authoring baseline is data, not a template file (#789)** | `STING_PROJECT_BASELINE.json` + `Baseline_Audit` (read-only) and `Baseline_Apply` (audit → confirm → mint). Not an `.rte`: a Revit template is binary, only authorable in Revit, and would help only NEW projects. Materials, wall/floor/roof/ceiling types and levels are creatable; family-backed types are reported as guidance and never listed as work Apply will do. A name match with a different build-up is a CONFLICT, never an overwrite. Material names are load-bearing, so the shipped baseline is tested against the classifiers that consume it. Levels ship empty — storey heights are a project decision. |
