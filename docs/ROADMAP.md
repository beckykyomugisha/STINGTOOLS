...
| LIGHTGRID-1 | **OPEN — found while binding; not a binding problem** | `LightingGridCommand` collects `OfCategory(OST_Rooms)` **only**, so on an MEP model that uses Spaces it finds nothing to light and silently does nothing. Binding the parameters to Spaces would not help — the command never reaches a Space to write to. Needs a Spaces branch in the collector. |
