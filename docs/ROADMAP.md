...
| ID | Item | Detail |
|---|---|---|
| KUT-1 | **59 workflow steps use `"tag"` instead of `"commandTag"` and are silently dead** | Measured identically on `origin/main` and the Phase 228 branch, so it is pre-existing, not a regression. `WorkflowStep` binds `commandTag`; presets including `WORKFLOW_ElectricalDesignReview.json`, `WORKFLOW_ElectricalPostFitOut.json`, `WORKFLOW_HealthcareCommissioning.json`, `WORKFLOW_MgasVerification.json`, `WORKFLOW_PressureRegimeAudit.json`, `WORKFLOW_RdsIssue.json`, `WORKFLOW_AntiLigatureAudit.json` and `WORKFLOW_NFPA110-GeneratorTest.json` instead emit `{ "order", "tag", "label", "continueOnFail" }`. Newtonsoft leaves `CommandTag` null without complaint, so **those presets run zero steps and report success** — the exact "valid JSON + green build, runtime-dead" failure mode. Fix: rename the field in the data files (cheap), or accept `tag` as an alias on the POCO, and add a build-time/unit validation pass over `Data/WORKFLOW_*.json` so this fails loudly. |
| KUT-2 | **66 workflow `commandTag`s do not resolve in `WorkflowEngine.ResolveCommand`** | Also identical on `origin/main`. Concentrated in `WORKFLOW_LpsCommissioning.json` (20), `WORKFLOW_DuctSpoolProduction.json` (8), `WORKFLOW_HVACDesign.json` (7), `WORKFLOW_PlumbingDesign.json` (8), `WORKFLOW_DailyFieldWalk.json` (7), plus `BOQ_DriftCheck` / `BOQ_ExportErp` / `ApplyParagraphPreset` / `SetParagraphDepth`. Each unresolved tag is a step that cannot run. All 5 `WORKFLOW_KUT_*` presets are clean. Fix alongside KUT-1 with one validation gate covering both. |
| KUT-3 | **Project-scope LOD verification only sees categories with an explicit rule** | `LodScope.Collect` builds project scope from `LodVerificationEngine.ExplicitCategories`, which excludes the `"*"` default. So Roofs, Ceilings, Curtain Walls, Stairs, Railings, Ramps, Structural Foundations, Electrical Fixtures, Duct/Pipe Fittings and Rooms are **never scanned** in a project-wide `LOD_Verify` — the `"*"` rule only applies to an explicit user selection. The retired `LOD_REQUIREMENTS.json` did carry minimums for several of them. Fix: either give those categories explicit rules in `STING_LOD_MATRIX.json`, or have project scope fall back to `"*"` when one is defined. |
...
| KUT-5 | **Division 02 cannot see Revit's demolition phase** | `CsiMasterFormat.Resolve` matches on category/family/type/SYS only. Revit expresses demolition through `Phase Demolished`, which the resolver never receives, so the Division 02 rows added in Phase 228 catch only elements *named* as removals plus the `Parts` category. A1 Deliverable B requires an "Existing Conditions & Removals Plan". Fix: pass phase state into the resolver and allow a `Phase` qualifier column. |
