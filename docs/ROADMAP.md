...
| LIGHTGRID-4 | **OPEN — bigger than the lighting cluster, needs its own decision** | `ParameterHelpers.BuildRoomIndex` collects Rooms only, and that index drives `SpatialAutoDetect.DetectLoc` / `DetectZone` — the LOC and ZONE tokens for **every tagged element**. On a Spaces-based model an element sits in a Space, so the index cannot place it and both tokens fall through to the policy fallback. That is not a lighting bug, it is the tagging pipeline, and widening it changes token derivation for every element in every project. It needs its own measurement and its own Revit check — not a drive-by at the end of a lighting sweep. |
