...
| DOCDRIFT-1 | **OPEN — one line** | `CLAUDE.md` states the block-load "`HVC_PEAK_*` stamps are TEXT-typed". `MR_PARAMETERS.txt` declares them **NUMBER**. The doc was describing the write, not the parameter — and the write was dead until MAPTYPE-6. |
...
| GENPH-2 | **OPEN — a naming call, one line each** | `GNL` and `GM` are codes I chose so the sentinel collision could be broken; they are not from a standard. `DiscMap` / `SysMap` / `ProdMap` / `FuncMap` are overridable from `project_config.json`, so a different house convention is a data edit rather than a code change. Worth confirming the letters. |
