...
| LIFE-6 | ~~`param_binding_resolver.py`'s outputs are stale against their own generator~~ **CLOSED 2026-09-08** | **Re-measured first, as the row asked.** At `c6a4bbe9a` the three outputs were **256 / 0 / 526** lines stale (audit view / gaps / deployable) — the earlier 245 / 0 / 520 had moved, which is why the row said to re-measure. All three are now regenerated and committed together. **Two pieces of pre-work, and the second reversed the row's own diagnosis.** (1) `SHT_*` — the resolver listed `SHT` among the excluded prefixes while the shipped deployable bound all ten `SHT_*` parameters to `Sheets`, so regenerating would have dropped them, and absence from that file means *intentionally unbound*. `Sheets` is a real bindable category (`CategoryEnumMap` → `OST_Sheets`); **the exclusion was the bug**, and it is now a `SHEET` domain. All ten survive. (2) **Materials — the row's premise was wrong.** It read the loader's `Materials` discard as a regression, but that discard is *by design*: `SharedParamGuids` drops the pseudo-category because material binding is done by a different mechanism entirely — `LoadSharedParamsCommand.IsMaterialRelevantParam` picks parameters **by name prefix** and `CleanMaterialBindings` binds them. All 108 `Materials` rows are recognised by that rule, so **nothing goes dark**. What the resolver gained instead is a **generation-time gate**: it reads the prefixes **out of the C# source** (a second copy of that list is the defect class this file exists to remove) and **fails** rather than write a `Materials` row the C# would not bind — the exact failure the code's own comment records for `BLE_MATERIAL_TXT`. **The "~22 regressions" did not materialise: 0 rows lost, 218 gained, 3,025 → 3,243 bindings.** Of 138 changed rows, 110 widened to universal and 28 re-scoped; of the 14 that narrow, **13 agree exactly with the hand-authored `CATEGORY_BINDINGS.csv`** — so the regeneration is correcting a stale deployable toward the authored sources, not regressing a good one. The seven `MAT_*`/`COMP_MAT_*` rows the row named are among them: `CATEGORY_BINDINGS.csv`, `FAMILY_PARAMETER_BINDINGS.csv` and the parameters' own descriptions all say `Materials`, and only the stale deployable said otherwise. **One genuine outlier, reported not buried: `MAT_TAG_6_TXT`** narrows from 37 categories to `Materials`, disagreeing with `CATEGORY_BINDINGS.csv` — but its four siblings `MAT_TAG_2..5_TXT` are all `Materials`-only there, so the 37-category row looks like the data error rather than the rule; it is still bound (by the material pass), just not on 37 element categories. **On whether the audit view should be a committed artefact at all: keep it, and gate it.** It is the file a reviewer reads, and deleting it would trade a drift problem for a discoverability one. The drift is closed instead by `--check`, which regenerates in memory, writes nothing, and exits 1 on any difference in any of the three — so three ages from one generator becomes a failure rather than a discovery months later. Asserted after: **3,243 ≥ 3,025** bindings and `MR_PARAMETERS.txt` still reports **763** non-TEXT parameters. Two sabotages confirmed failing: a hand-edited row in the deployable (`--check` exits 1), and `SHT` put back on the excluded list (all ten rows vanish). `StingTools.Tags.Tests` asserts the COBie field map against this very file and still passes at 307. |
...
| MATSCHED-T-VERIFY | 🟡 **Partly closed 2026-09-06 — three things ran in Revit, six have still never executed** | **PROVEN by a real session:** the T1-T5 readers (one export produced every scan line with its own denominator); B4's harvest pass (16 placed types, correctly prefixed, provisional); B6's per-category dimension fix (the three `Concrete-Rectangular-Column` types came back with `b`/`h` only — the 4500×12000 extents are gone); and the rate editor opening and seeding (38 rows, 25 unpriced, 4 memoranda withheld, resolved `_data/coord` path). **STILL NEVER EXECUTED:** the Ctrl+V paste path (T10), the mapping dialog and the `supplier_unit_patches.json` round trip (T12), `FamilySymbol.Duplicate` (B1), the `EditFamily` → `AddParameter` → `LoadFamily` round trip (B2), and catalogue adoption (B3). B1-B3 were INERT on the test project by design — the shipped baseline declares no family types and adopts no pack — so running `Baseline_Audit` there proves the readers, not the writers. **1,065 green tests say the logic is right and nothing about whether the Revit API behaves as assumed, and every bug this feature has had lived in that half.** Cheapest remaining checks, all read-only: paste a column into the rate grid; open Map to commodity and read the preview without applying. |
