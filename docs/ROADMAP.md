...
| ROOM-7 | **OPEN** | The room tag is code-first by design: tier 1 is `ASS_TAG_1_TXT`, so at Compact depth a room reads `A-BLD1-Z01-GF-ARC-FIT-RM-0007` and nothing else. That is correct for asset tagging and wrong for an architectural floor plan, where the name is the point. Worth deciding whether a second room-tag *type* (name-first, code in tier 2) belongs in the corporate set — it needs its own hand-authored label, so it is a real piece of work, not a config flag. |
