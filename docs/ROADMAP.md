...
| LIGHTGRID-3 | **OPEN — measured, deliberately untouched** | Of the 86 files collecting `OST_Rooms` and never `OST_MEPSpaces`, the lighting cluster is now clear. The remaining ~79 are mostly and correctly about Rooms — finishes, room data sheets, architectural QA, COBie zones. They are **not** a backlog: converting a command that is genuinely room-scoped would be a regression, not a fix. `SpatialCompat` is there for any that turns out to need it. |
