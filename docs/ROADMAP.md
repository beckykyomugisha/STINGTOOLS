...
| ID | Item | Detail |
|---|---|---|
| MATSCHED-1 | **Nothing has run inside Revit** | The engine is covered by 58 unit tests, but `MaterialScheduleBuilder`, `MaterialScheduleXlsxWriter` and the command need a `Document` and are unreachable from any test. Every defect found on this feature so far has lived in exactly that layer — three of them, all in `AppendManualRows`. **Before merge, on a model with modelled walls and slabs:** set `COST_COMPOUND_TAKEOFF=1`, run both priced and unpriced, and confirm section letters run A/B/C with no duplicates, the Summary letters and order match the body exactly, units read `Bags`/`Trips (Sino Truck)`/`No.` rather than `m3`/`bag`, both runs report the **same** order quantities, and the file lands in the project's SCHEDULES folder rather than MISC. Plan Task 14. |
...
| MATSCHED-3 | **Finishes still do not reproduce the reference sample; Roof does** | Roof sheets and tiles convert (category + type-pattern rules, PR #709). **Finishes do not, and the first attempt was withdrawn.** Rules for `paint-wall`, `floor-tile` and `paint-ceiling` matched a whole ELEMENT and converted it to a finish — a wall is not paint and a floor slab is not tiles, so a composite wall row whose type read "plastered" priced the entire wall area as paint buckets. `paint-ceiling` carried no type patterns at all, so every ceiling including a suspended grid became paint. All three were removed in the follow-up rather than left to produce confident wrong numbers. **A real finishes take-off needs painted AREA and tiled AREA, which `CompoundTakeoff` does not emit** — that is the remaining work, and it belongs in the take-off engine, not in a unit table. |
