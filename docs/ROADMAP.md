...
| 9 — Mobile 3D context in issue detail | **DONE** | Two complementary paths shipped: (a) **Phase 94** — `Planscape/app/(tabs)/issues.tsx:38–49` defines `openViewer(projectCode)` opening `{base}/viewer/index.html?model=<code>.xkt` via `WebBrowser.openBrowserAsync()`; `IssueCard` exposes a "🧊 View in 3D" action; detail screen `issue-detail.tsx:243` mirrors the same flow — useful as a fullscreen / unlinked-issue fallback. (b) **Phase 162** — inline `<ModelViewer>` embed in `issue-detail.tsx` gated on `issue.modelId`, with single pin at `(modelX, modelY, modelZ)` when the anchor is set; creation form in `issues.tsx` adds a "Linked model" chip-row picker driven by `listModels(projectId)`; server `CreateIssueRequest` accepts `ModelId` + `ModelElementGuid` + `ModelX/Y/Z` and validates `ModelId` ownership before insert. | None |
