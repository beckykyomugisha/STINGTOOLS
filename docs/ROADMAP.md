...
| CREATEPARM-1 | **OPEN — measured, needs a category per parameter** | **"Create Parameters" does not create all of them.** It delegates to `LoadSharedParamsCommand`, which binds a parameter only if it is universal OR has a per-parameter category binding; anything else is counted as a binding gap and skipped. Measured on the shipped data: **3,604 declared, 3,444 bound, 160 skipped, 0 conflicts.** Of the 160, **57 are parameters the code WRITES per element and that are bound to nothing** — `CachedLookup` returns null and the write returns false in silence. They include `HVC_PEAK_SENS_W` / `_LAT_W` / `HVC_OA_LS` (so the block-load stamps still do not land — MAPTYPE-6 fixed their TYPE mismatch, not their absence), the whole `ELC_WIRE_*` set, `PEN_*`, `PLM_FLOW_*` and `TAG_PARA_DEPTH_INT`. Fixing them means naming a category per parameter, which is derivable from the command that writes each one but is not something to guess in bulk. Ratcheted at 57 in `BINDING_SCOPE_BASELINE.json`. |
| CREATEPARM-2 | **OPEN — reported** | The other **103** skipped parameters are **not** written by the code: 53 `Qto_*` IFC quantity-set names (a mapping vocabulary, not project parameters — arguably declared in the wrong file), 41 `WARN_*` and 28 `TB_*` / `TAG_*` that are **family** parameters injected into `.rfa` files by `FamilyParamCreatorCommand`, where a project binding is neither needed nor correct. **None of the 160 is unused** — every one is named in code or in shipped data. |
