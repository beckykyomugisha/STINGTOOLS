...
| WF-6 | ~~No gate enforces `order` against array position~~ | **CLOSED 2026-09-08 — the row was stale when written.** Tier 3 of `tools/check_workflow_wiring.ps1` does exactly this and has since PR #635 (`bb52f6a42`); the row's claim that "the gate has Tiers 1, 2 and 4" was already false. Its second worry was unfounded too: the tier is a hard zero with no baseline and passes across all 48 presets / 387 steps, so no preset ever needed correcting or baselining — `order` and array position agree everywhere today, and the tier exists to keep it that way. Nothing to do beyond deleting the belief. |
