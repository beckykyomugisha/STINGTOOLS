...
| MAPTYPE-7 | **OPEN — candidates, NOT verified** | The same "type built-in read from an instance" signature appears outside the tag system, and there it is paired with a **fabricated fallback**, which is worse than a blank: `AcousticAnalysisEngine.cs:490,496` (`WALL_ATTR_WIDTH_PARAM` on an instance, then `FLOOR_ATTR_THICKNESS_PARAM ?? 0.5`), `SleeveEngine.cs:430` (`?? 0`), `StructuralAnalysisEngine.cs:549,4171,4433`, and `EmergencyLightingAuditCommand.cs:126` (`ALL_MODEL_TYPE_MARK` on a `FamilyInstance` → empty string → the keyword match can never fire, so the audit always passes). These are engines I cannot exercise, so they are **reported with line numbers rather than changed** — an acoustic or structural calculation is not somewhere to apply an unverified fix. |
