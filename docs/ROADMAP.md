...
| MATSCHED-T-VERIFY | 🔴 **NINE features and four baseline layers depend on Revit-side code that has never run** | Unexercised: the tile / screed / ceiling / membrane layer readers (T1-T3), T5's `ReadRoofEavesLengthM`, `FamilySymbol.Duplicate` (B1), the `EditFamily` → `AddParameter` → `LoadFamily` round trip (B2), catalogue adoption (B3) and the harvest pass (B4). T4 adds no reader but multiplies drivers from take-off paths that have never executed. **766 green tests say the logic is right and nothing about whether the Revit API behaves as assumed — and every bug this feature has had lived in that half.** Three read-only commands would settle almost all of it in minutes: one material-schedule export (T1-T5), `Baseline_Audit` (B1-B3 readers) and `Baseline_HarvestTypes` (B4). `Baseline_Apply` is the only invasive path and should wait until the audit has been seen once. |
