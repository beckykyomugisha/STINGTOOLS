...
**Closed by Work Items A–E** (see CHANGELOG): param naming normalized to `PRJ_*` (GUID-preserved) +
duplicate toggles consolidated (A); default row de-duplicated to SECURITY | CDE REF | LOD | SYSTEM (B);
**north-arrow / scale-bar / key-plan / legend now placed by code** at their reserved slots, toggle-gated
+ idempotent + remove-when-off (C) — closing the "verify scale bar / north arrow / key plan / legend"
placement gap and the W1 legend gap; annotation families generated by `TitleBlock_BuildGraphicsFamilies`
(D); a `TitleBlock_StampSheetGraphics` orchestrator + optional `DrawingProducer` hook (E). Remaining:
the **static** graphics only a family cell can carry (projection symbol, logo, suitability legend chip,
discipline colour strip) still need the seed `.rfa` UI follow-up below.
