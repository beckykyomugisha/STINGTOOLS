...
| ID | Status | Detail |
|---|---|---|
| MAPTYPE-1 | **OPEN — needs Revit, check this first** | `WriteMapped` now stores Revit-INTERNAL units in Double-storage targets and puts the converted display string in the `_TXT` mirror. The conversion is the risk. On a tagged door, `W:` should read ~**900** (the mirror, mm). The `BLE_DOOR_WIDTH_MM` parameter itself should show 900 mm in the properties palette — if it shows 274320 or 2.95, the raw/formatted arguments are the wrong way round. Check one door, one window, one room area, one pipe length. |
...
| PARAM-3 | EasyPower's line-to-ground fault level has no parameter | `EasyPowerImportCommand` wrote it to `ELC_FAULT_LG_KA`, which was never defined, so it was discarded silently. It is now reported in the import summary instead ("not stored"). Storing it needs a defined parameter on Electrical Equipment (e.g. `ELC_PNL_FAULT_LG_KA`, NUMBER) and a reader (fault schedule / SLD label). |
| PARAM-4 | ~~`STING_VOLTAGE_TIER` / `STING_FEED_TYPE` are never stored on SLD symbols~~ **CLOSED 2026-09-25** | The symbol builder (`SymbolLibraryCreator`) now authors both as instance TEXT **family** parameters in every annotation family built from a `STING_SLD_SYMBOLS*.json` catalogue, and in compound SLD symbols, so `SLDGenerator.PlaceSymbols`' stamp lands. `SymbolCacheManifest.GeneratorVersion` 2 -> 3, so the next symbol build rebuilds cached SLD families. A family built before the change still gets the once-per-parameter warning, which now says to rebuild the SLD symbols and regenerate. **Not run in Revit on this branch.** `STING_SYMBOL_ID` / `STING_SLD_ELEMENT_ID` (shared parameters, same Generic Annotation category) were not in scope and are not authored into the family. |
| PARAM-5 | ~~Projects that bound `ELC_CKT_NR` as NUMBER keep a NUMBER~~ **CLOSED 2026-09-25** | `Params_RebindCircuitNumberAsText` (`Tags/RebindCircuitNumberAsTextCommand.cs`, CREATE TAGS -> SETUP, beside Load Params) reads every value as text ("3", not "3.00"), confirms with the count, then in one transaction removes the binding and the parameter element, re-binds `ELC_CKT_NR` from `MR_PARAMETERS.txt` as an instance binding on the `RESOLVED_BINDINGS.csv` categories unioned with the old binding's (so no value loses its home), writes the values back and reports before/after counts and every value not restored. Already TEXT: says so, does nothing. **Not run in Revit on this branch** - in particular, re-inserting the same GUID in the transaction that deleted it is unverified; if Revit refuses, the transaction rolls back and nothing changes. |
| PARAM-6 | 147 `Project Information` rows in `CATEGORY_BINDINGS.csv` are still not honoured | The resolver now honours rows marked `Yes` (hand-authored). Rows marked `True` naming Project Information are not, because most of the curated file is polluted: they cover `PRJ_TB_*` (deliberately Sheets), 28 `WARN_BLE_*` mirrors, and `<ALL>` parameters such as `PRJ_NAME_TXT`, `PRJ_ADDRESS_TXT`, `PRJ_PHASE_TXT` and `ASS_DESIGN_*`. `<ALL>` is the 143 element categories and excludes Project Information, and the spec's cell cannot express "<ALL> plus Project Information". Each needs a decision (flip the row to `Yes`, or give the parameter a `PROJECT_INFO` rule) rather than a bulk flip. |
| PARAM-7 | Twelve parameter names are read or written by code and defined nowhere | None is in `MR_PARAMETERS.txt`, so Load Params never binds them: a write finds no parameter and a read returns empty, and the feature behind each is inert until a project hand-adds a parameter of that exact name. Checked 2026-09-25 by name against the `.txt`. `ELC_FEED_TYPE_TXT` — `Core/SLD/SLDCircuitTraverser.cs:304,309`, `Commands/Electrical/Validation/DualSourceValidationCommand.cs:109`; `ELC_CONDUIT_REF`, `ELC_CABLE_ROUTE_REF` — `Core/SLD/SLDCircuitTraverser.cs:526,528`; `ATEX_ZONE_TXT`, `ATEX_EX_RATING_TXT` — `Commands/Electrical/Validation/ATEXClassificationCommand.cs:73,107,108`; `ELC_FIRE_LOOP_REF` — `Commands/Electrical/Schematics/FireAlarmSchematicCommand.cs:54`; `MGS_SUPPLY_TYPE_TXT`, `MGS_ZV_ZONE_TXT` — `Commands/Electrical/Schematics/MGPSSchematicCommand.cs:87,96`; `ELC_JB_IP_RATING_TXT` — `Commands/Electrical/Routing/CableScheduleBuilderCommand.cs:294`; `HVC_SPACE_TYPE_TXT` — `Commands/Electrical/Lighting/ComCheckExportCommand.cs:65`, also `Commands/Hvac/HvacBlockLoadCommand.cs:301,358` and `Commands/Hvac/HvacCrossTalkAuditCommand.cs:429`; `PRJ_BUILDING_USE_TXT` — `Core/Electrical/ProjectSector.cs:33`, also `Core/Sustainability/SustainabilityEngine.cs:403`; `LIM_INSTALLED_BOOL` / `IPS_LIM_BOOL` — see PARAM-8. **Recommendation:** decide per name, the way PR #980 did — repoint to an existing parameter where one already means the same thing (check `ELC_CIRCUIT_REF_TXT` for the conduit ref, an existing `ASS_`/`PER_` space-type or building-use parameter for the last two, the `MGS_*` group for the medical-gas pair), and define only what has no equivalent, with a binding row for the category the code actually iterates. Do not bulk-define: several are read on categories where nothing writes them. |
| PARAM-8 | IPS Validation has no defined Line Isolation Monitor parameter | `Commands/Electrical/Validation/IPSValidationCommand.cs` looks for `LIM_INSTALLED_BOOL` or `IPS_LIM_BOOL` on each IPS board; neither is in `MR_PARAMETERS.txt`, so every IPS board reports "No Line Isolation Monitor recorded" unless a family happens to carry one of those names. `ELC_IPS_BOOL` ("Powered from Isolated Power System") marks the board as an IPS, not that it has a LIM, so it cannot stand in. Needs a YESNO parameter on Electrical Equipment (e.g. `ELC_IPS_LIM_BOOL`) plus a binding row; the command then reads it first. The undefined `IPS_PANEL_BOOL` fallback for the IPS flag itself was removed 2026-09-25 in favour of `ELC_IPS_BOOL`. |
| PARAM-9 | Four category names in the binding spec resolve to nothing and are dropped silently | `CATEGORY_BINDINGS.csv` (and from it `RESOLVED_BINDINGS.csv`) names `Anti-Ligature (Door)`, `Anti-Ligature (Lighting Fixture)`, `Anti-Ligature (Plumbing Fixture)` and `MEP Sleeve`. `SharedParamGuids.EnsureResolved` (`Core/SharedParamGuids.cs` ~357) skips any name `ParamRegistry.CategoryEnumMap` does not know, with no log line — and the map has none of the four (it has `MEP Sleeve (Fire-rated penetration)`, not `MEP Sleeve`). Today no binding is lost, because every affected row (`LIG_AREA_OBS_LOS_TXT`, `LIG_PRODUCT_RATING_TXT`, the eleven `SLV_*` sleeve parameters) also names the real category (`Doors`, `Lighting Fixtures`, `Plumbing Fixtures`, `Generic Models`). The defect is that the next row naming only a pseudo-category would bind nowhere with no signal. **Recommendation:** map the four names to their real categories in `CategoryEnumMap` (or strip them in the resolver), and make an unknown name a `StingLog.Warn` in `EnsureResolved` and a failure in `param_binding_resolver.py`. |
