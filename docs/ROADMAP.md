...
| ID | Status | Detail |
|---|---|---|
| BINDSCOPE-1 | ✅ **CLOSED** (this phase) | A newly placed door tagged `A-BLD1-Z01-L01-ARC-FIT-DR-` — seven tokens and a trailing separator. `ASS_SEQ_NUM_TXT` read `-215`, **greyed** in Properties, i.e. TYPE-scoped. A type-scoped parameter is not reachable through `element.get_Parameter()`, so `SetString(el, ParamRegistry.SEQ, …)` returned false in silence and the tag assembled with an empty final segment. It is also wrong on its own terms: bound to the type, every door of `750 x 2000mm` shares one SEQ. **1,198 rows** flipped (740 + 270 + 188, each pass after WIDENING the derivation rather than finding new data) Type → Instance in `CATEGORY_BINDINGS.csv`, derived by `tools/check_binding_scope.py` from the rule *if the code writes it per element, it must be Instance* — not hand-listed. |
| BINDSCOPE-2 | **OPEN — migration, needs Revit** | Editing the CSV does **not** re-bind an existing project. The binder reads it when it runs; a model bound earlier keeps its old scope. The project that produced this finding already disagrees with the shipped CSV **in both directions** — `ASS_TAG_1_TXT` was Type in the CSV and Instance in the model, `BLE_DOOR_HEAD_HEIGHT_MM` has no CSV row at all and is populated in the model. So the CSV is not evidence of what a given project has. Re-bind, then re-tag, then confirm SEQ appears. |
...
| ISO19650DISC-3 | **OPEN — second collision, same shape, bigger blast radius** | The healthcare pack uses `H` for **Healthcare**. `Iso19650Vocabulary.cs` says `H` is **Heating & Ventilation**. Identical to ISO19650DISC-1 but it touches the healthcare pack’s 60 tag families, disciplines and drawing types, and unlike the generic categories those tags may already be correct and issued — so this one has a real migration cost. `Y` (Specialist Designer) or a multi-letter house code such as `HC` would both avoid the collision. Not changed unilaterally. |
| ISO19650DISC-4 | **OPEN — reported** | `LV`, `FP` and `FLS` are multi-letter house extensions, so they cannot collide with the single-letter role table and are allowed. But they are **systems, not roles** — under the standard, LV and FLS work is issued by `E`, fire protection usually by `Y`. Whether STING’s DISC segment means "role" (standard) or "discipline/system" (current) is a BEP question worth answering explicitly. |
