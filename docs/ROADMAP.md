...
|-----|-------------|-----------|
| `DWG-STRUCT-P2A` Strip foundation detection (under walls) | For every detected structural wall, search the foundation layer for a parallel-line pair wider than the wall by `StripFndOversizeMm` each side, parallel within 5° to the wall centreline. Create as a structural `Floor` along the wall centreline. | New `DetectStripFoundations()` pass after `DetectedWall` collection in `StructuralCADPipeline.cs`. New config field `StripFndOversizeMm` (default 150). New row in PER-ELEMENT SIZE DETECTION table. |
| `DWG-STRUCT-P2F` Endpoint gap bridging at detection time | The `EndpointGapToleranceMm` config field landed in Phase 140 and is read by `BuildConfig`, but the spatial-index pair-detection inner loop in `StructuralDWGEnhancements.SpatialLineIndex` does not yet extend the shorter line of a pair when endpoints are within tolerance. | Modify `SpatialLineIndex.PairsForLine()` to compute longitudinal endpoint gap and virtually extend before computing centreline. |
| `DWG-STRUCT-P3B` Slab centroid → room seeding | After slabs are created, for each `DetectedSlab` whose centroid is not inside any void, call `doc.Create.NewRoom(level, UV(centroid.X, centroid.Y))` if no room already exists at that point. Use any text string detected on the slab layer (proximity search) for the room Name. | New post-pass after `CreateSlabsFromBoundaries`. New config field `SeedRoomsFromSlabs` (default true). Spatial proximity helper for layer-text → room name. |
| `DWG-STRUCT-P3C` Auto-create structural views after conversion | After `result.TotalCreated > 0`, check if `DrawingTypeRegistry` is available, call `DrawingDispatcher.Resolve(doc, "S", "*", "PLAN")`, create a `ViewPlan` per level that received elements, and apply via `DrawingTypePresentation.Apply()`. | New post-pass after `RunFullPipelineWithConfig` returns. New config field `CreateStructuralViewsAfterConversion` (default false). Hook to Phase 113 `DrawingTypeRegistry`. |
| `DWG-STRUCT-DEEP-1` Steel I-section vs concrete rectangle inference | Family selection for beams currently uses generic structural-framing types regardless of detected geometry. A pure-line beam should hint steel I-section, a parallel-pair beam should hint concrete rectangle. | New material classifier on `DetectedBeam` consumed by `StructuralTypeFactory.FindOrCreateBeamType()`. |
| `DWG-STRUCT-DEEP-2` Pile cap / raft / strip differentiation | All foundations currently route through `CreatePadFoundations` → isolated pad footings. A larger detected rectangle should consider raft; multiple rectangles in a circular pattern should consider pile cap. | Foundation classifier sitting between `DetectedRectangle` and the foundation creation pass. |
| `DWG-STRUCT-DEEP-3` Cantilever detection | Beam endpoints that lack a column or wall support should be flagged as cantilevers and offset analytically. | New pass after `BeamTrimmer` that classifies each endpoint as supported / cantilever based on column/wall proximity. |
| `DWG-STRUCT-DEEP-4` Beam-overlap ratio configurability | The 50% overlap ratio used for parallel-beam pairing in `StructuralDWGEnhancements.SpatialLineIndex` is hardcoded; short connection stubs and diagonal bracing don't pair. | New config `BeamOverlapMinRatio` (default 0.5); thread through to the pair detector. |
...
| `DWG-STRUCT-DEEP-6` Junction-type assignment | `DetectJunctions` from the Phase 140 prompt does not exist in code today; the only junction-aware step is `BeamTrimmer`. A real junction pass would classify L / T / Cross intersections and add connection elements. | New `DetectJunctions()` pass after column placement. New `DetectedJunction` record with type + participants. |
| `DWG-STRUCT-DEEP-7` Continuous columns via Top Constraint = top level | Both branches of `ColumnsContinuousThrough` create stacked discrete columns at each level — neither produces a single analytical column constrained Base→Top. | Replace stacked-column creation with a single column whose Base Constraint = base level, Top Constraint = top level when continuous flag is on. |
| `DWG-STRUCT-DEEP-8` Beams-on-walls applied per-beam, not globally | The `BeamsRestOnWalls` flag offsets every beam globally. Should check each beam's actual support to apply the offset only when the beam sits on a wall. | Per-beam support classifier reading column / wall proximity at the trimmed endpoints. |
