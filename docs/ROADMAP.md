...
| MATSCHED-3 | **Roof and Finishes will not fully reproduce the reference sample** | `CompoundTakeoff` emits masonry, RC and plaster constituents. It does **not** emit roofing sheets, paint litres, tile boxes or nails/kg, so those rows come through as BOQ rows in measured units until `STING_SUPPLIER_UNITS.json` and `STING_COMMODITY_RATES.csv` are extended to cover them. The rows are present and priced — they simply read `m2` where the sample reads `No.` or `Bkts`. Extending the two data files is additive and needs no code. |
