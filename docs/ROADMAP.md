...
| MATTAG-5 | **OPEN — decision** | `STING - Materials Tag` carries the universal label: **0 of its 68 parameters can appear on a material** (none is a Material built-in or bound to Materials; the tier gates need a type a Material does not have). Now **measured**: `MaterialTagLabelTests` checks every Materials tag spec against RESOLVED_BINDINGS' Materials rows + the Material built-ins, holding the legacy spec to a ratchet (`tools/material_tag_label_baseline.txt`, 68, may only shrink) and any other material tag spec to zero. `check_tag_row_bindings.py` could not see this — it counts `<ALL>` as bound for every category, and `<ALL>` is the one binding that never reaches Materials. Recommended fix: rebuild in place (see CHANGELOG 2026-09-24). |
