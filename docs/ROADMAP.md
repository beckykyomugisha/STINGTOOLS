...
| PARAM-2 | `ELC_LPS_MESH_SIZE_M` and `ELC_LPS_ROLLING_SPHERE_RADIUS_M` are LENGTH parameters that hold raw metres | LPS Class Setup writes them with `p.Set(metres)`, and every reader (`LpsEngine.GetDoubleParam`, `LpsValidator`, the LPS panel, now `LPS_Schematic`) reads the raw double back, so STING is self-consistent. But Revit stores LENGTH in feet: a 10 m mesh shows as 3048 mm in the Properties palette and in any schedule. Writer and every reader must switch to `UnitUtils` conversion together, or the parameters become NUMBER; changing one side alone would corrupt the other. Found 2026-09-24 while repointing `LPS_Schematic`. |
