...
| IM-4 | **Issue schema fork: `issue_id` vs `id`** | `BIMManagerEngine.CreateIssue` emits the identifier as `issue_id`; the `WarningsManager` / `Phase75Enhancements` auto-escalation paths emit `id`. `CoordStores.RowId` tolerates both, but readers that look up `issue_id` only (e.g. `BIMManagerCommands.cs:2306`) cannot see auto-created issues. Belongs with the Phase 2 `IssueStore` work. |
