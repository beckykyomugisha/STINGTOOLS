...
| ID | Item | Detail |
|---|---|---|
| MATSCHED-1 | **Verified in Revit 2026-08-17 — the three integrity criteria pass** | Run on a real model (16 categories, UGX 1.55bn modelled) with `COST_COMPOUND_TAKEOFF=1`: **section letters run A/B/C with no duplicates**, the **Summary letters and order match the body exactly**, and the **Validation sheet is clean** — no R1 rate inconsistency, no R2 lettering fault, no R3 unpriced commodity, no R4 negative wastage, no R5 blocked conversion. That closes the three defect classes the reference sample failed on (duplicate letters, summary/body divergence, one commodity at two rates). **Not yet reported:** whether the file landed in the project's SCHEDULES folder rather than MISC — the export-route backfill (v1 → v2 migration) fires on a project's first load after the build is live, so an older project may still route to MISC until then. Worth one glance next run. The Revit-bound layer remains untestable by design, so this stays a manual check after any change to `MaterialScheduleBuilder`, `MaterialScheduleXlsxWriter` or the command. |
