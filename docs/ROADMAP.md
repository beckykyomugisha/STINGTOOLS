...
| UNITAG-3 | **CLOSED 2026-09-24 except one** | 7 of the 8 now resolve. The cause was not truncation — `Tie-In Point (Duct — HVAC)` is not truncated and failed too. The `category_labels` key carries **"Tag" in the middle** (`Tie-In Point Tag (Duct — HVAC)`) and `Brace / Truss` uses a slash where the spec uses a hyphen; `strip_family` removes a TRAILING " Tag", not a middle one, so the prefix match never fired. Fixed with `csv_family_alias`, which the file's own description says exists to link the two — renaming either side would break consumers, and the tag-family FILENAMES cannot carry "/" at all. Resolving them exposed 20 schedule columns that were unbound (the debt the broken names were hiding); all 20 are now bound, including `STR_BRACE_LENGTH_MM`, which was bound nowhere exactly as this entry predicted. `check_tag_row_bindings.py`: 20 dead columns → 0. **Still open:** `Healthcare Warnings` has no `category_labels` entry AND no tag family on disk, so there is nothing to alias it to. |
...
| PROD-2 | ~~A negated material name still takes the material's code~~ | **CLOSED 2026-09-24.** Took the row's first option: `STING_MATERIAL_PROD_OVERRIDES.csv` gained an optional 4th column, `ExcludePattern`. When it matches, the rule does not apply and matching CONTINUES to later rules — returning null there would stop a later rule that legitimately matches. Deliberately not the `(?!-free)` hard-code the entry warned against: the next negation is a CSV edit, not another special case. A malformed exclusion drops the whole row rather than applying the rule without its guard, because the alternative silently restores the exact defect at the moment someone fat-fingers a bracket. 3-column rows are unchanged. `KnownUnresolved_...` flipped to `A_Negated_Material_Name_No_Longer_Takes_The_Materials_Code`, plus guards that plain lead still takes PB and that unexcluded rules are untouched. |
