...
| MATSCHED-T6 | **Not attempted — the deliverable is a written proposal first** | Splitting a door into leaf + frame + ironmongery requires the FAMILY to declare it; inferring it from a type name is the defect PR #710 withdrew three rules for. The route is to extend the family contract (`MAT_LEAF_TXT`, `MAT_FRAME_TXT`, `MAT_IRONMONGERY_SET_TXT`, `MAT_GLAZING_M2`), have `FamilyConformanceCheckCommand` report families that lack them, and emit constituents only from families that declare them — everything else stays one priced unit, which is the accurate answer rather than a limitation. No shipped family was modified. |
| MATSCHED-T-VERIFY | 🔴 **The Revit-side readers have never run — FIVE features now depend on them** | The runner for T1–T3 opens with a gate: draw a floor with `STING RC Slab 150 - Ceramic Tiled`, export, and confirm `Floor tiling` / `Tile adhesive` / `Tile grout` appear under FINISHES. **That gate was never satisfied** — the sessions that built T1, T2 and T3 could not run Revit — and all three ride the same `HostLayerCache` walk that the unexercised tile reader (#780) was extracted from. Everything Revit-free is tested; every bug this feature has ever had lived in the Revit-bound half. This is now the single highest-value thing to do to the material schedule, and it is one export: the four scan diagnostics (tile, screed, ceiling, membrane) each report their own denominator on the Validation sheet, so a single run says which of them saw anything and why the others did not. **Updated after T4/T5:** T4 adds no reader but multiplies driver totals produced by these same paths, so a wrong walled area gives a confidently wrong hoop-iron figure; T5 adds a NEW reader (`ReadRoofEavesLengthM`, via `FootPrintRoof.get_DefinesSlope`) that has never executed either. Six scan diagnostics now report on that one export — tile, screed, ceiling, membrane, consumables and roof accessories. |
| MATSCHED-B1..B4 | ⚠ **Baseline layers 2-4 landed (#811, #813, #814, #815); none has run in Revit** | **B1** mints a TYPE inside an already-loaded family (`FamilySymbol.Duplicate` + set type params by NAME); it cannot conjure the family, so an absent one stays guidance. **B2** adds SHARED parameters to loaded families via the `ExternalDefinition` overload with `isInstance: false` — the name/spec overload `FamilyAugmentationEngine` uses creates LOCAL parameters that cannot be scheduled together. **B3** ships opt-in catalogue packs (`STING_TYPE_CATALOGUES.json`), none active by default. **B4** `Baseline_HarvestTypes` reads PLACED types from a delivered model and writes a candidate pack. **Independently verified by a second session against the task's invariants, all passing:** a half-made `Duplicate` is deleted and an undeletable one is reported by name (#798's lesson); the augmenter resolves `ExternalDefinition`s and never calls the literal-name overload; the `STING ` prefix is a declared, enforced contract so a minted type survives a vendor reload; mint failures are reported per type (8 distinct failure sites) and only findings marked Missing are ever minted, so an existing vendor type is never edited; the corporate baseline adopts nothing and declares no `familyTypes` inline; `EA-RESIDENTIAL-V1` is `provisional` with a 375-character source note saying it is not a standard; the parameter schema has nowhere to put a value, so a corporate material default is unrepresentable rather than merely absent; and the harvest is `ReadOnly`, opens no transaction, writes through `StingPaths`, excludes already-minted types, and reports both `SkippedNotPlaced` and `SkippedAlreadyPrefixed`. Its button and handler case both exist and the three `Baseline_` tags match the three cases exactly. **NOT verified:** nothing Revit-side has run — the mint, the augment and the harvest reader have never executed against a real model, which is the same standing risk as MATSCHED-T-VERIFY and now covers eight features. |
