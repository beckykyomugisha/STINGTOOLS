...
| CATBIND-2 | **OPEN — reported, not guessed** | The other **85** names have no near match and are spec families rather than Revit categories — `Clinical Room`, `LPS Air Terminal`, `MEP Sleeve`, `Architectural Sheet`, `Structural Slab`, `Brace / Truss`. Their **242 binding rows are inert**. Some look like they SHOULD map (`Structural Slab` → `Floors`?), but binding a parameter to the wrong category is worse than leaving it unbound, so each needs a decision rather than a guess. |
| CATBIND-3 | **OPEN — measured, harmless today** | **206 duplicate (parameter, category) pairs / 207 extra rows** in `CATEGORY_BINDINGS.csv`. The loader dedups (`seen[param].Add(bic)`), so behaviour is correct — recorded so the count is not mistaken for 207 extra bindings, and because duplicates make every other count on this file harder to trust. |
| SYSAMB-1 | **OPEN — order-dependent fallback** | **27 categories are listed under more than one SYS code** — `Pipes` under all eight of DCW/DHW/FP/GAS/HVAC/HWS/RWD/SAN. For MEP that is by design and `GetMepSystemAwareSysCode` resolves per element. But the fallback `GetSysCode` returns **`list[0]`**, which is the first key encountered iterating a `Dictionary` — an order C# does not contract. So a pipe with no MEP system gets whichever code happens to be first. It is stable in practice and the discipline default (`GetDiscDefaultSysCode`) already exists as a principled alternative, so this is a behaviour decision rather than a bug fix. `Fire Protection` → FLS/FP and `Electrical Equipment` → LPS/LV are the two that look accidental rather than intended. |
