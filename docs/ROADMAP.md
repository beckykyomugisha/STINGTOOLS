...
| IM-7 | **Six issue writers still bypass `IssueStore`'s audit + server push** | `CreateIssuesFromWarningsCommand`, `AutoRaiseComplianceIssues`, and the four BCF import/sync sites in `PlatformLinkCommands.cs` still build a record with `BIMManagerEngine.CreateIssue` and persist with `SaveJsonFile`. They inherit the canonical schema and correct id minting (both helpers now delegate to `IssueSchema` / `IssueIdMinter`), so IM-4 and IM-5 do not regress through them — but they emit no `issue.*` audit entry and do not push to the Planscape server. Convert each to an `IssueStore.Begin(doc)` batch. |
| IM-8 | **`GetNextIssueId` reserves nothing between calls** | It builds a fresh `IssueIdMinter` per call, so it is only safe when the caller appends to the array in the same iteration — which all four current callers do. A future caller that batches creations before saving would get duplicates. Prefer `IssueStore.Begin`, whose batch holds one minter; consider deleting the helper once IM-7 lands. |
