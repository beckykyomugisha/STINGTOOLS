...
| HC-DEF-01b | **`RadShieldValidator` true barrier-distance geometry.** The unknown-distance path now assumes a conservative `1.0 m` (named constant, project-tunable via `PRJ_ORG_HEALTH_RAD_DEFAULT_DIST_M`) and warns (`RAD.DIST.ASSUMED`) rather than under-shielding at the old optimistic 2.0 m. Real 3D source-to-barrier distance is still not derived from model geometry. | The conservative default + optional per-element `RAD_DISTANCE_M_NR` + warning ship now; deriving the true distance needs source placement + barrier-face geometry, belonging with the QE write-back workflow (HC-DEF-01). |
