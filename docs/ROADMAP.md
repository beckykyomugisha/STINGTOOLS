...
## Material Schedule export — after Phase 244 (2026-08-17)

| ID | Item | Detail |
|---|---|---|
| MATSCHED-1 | **Still open. The blockers are fixed; nothing since #728 has been seen in a real export** | The two contradictory runs are now explained and both causes are closed: `COST_COMPOUND_TAKEOFF` was off (MATSCHED-7) and every unmatched row became a commodity (MATSCHED-8). What a real export has actually shown since: materials do appear, and appearing exposed four further defects — square metres sold as pieces, wastage applied twice, columns and foundations not decomposed, and raw binary floats in the order column. All four are fixed (#724, #725, #728) and all four are pinned by tests, but **the fixes themselves have only been tested, never run**. One number to re-read on the next export in particular: the last export's block figure implied 2,024 m² of wall while its mortar implied 994 m², from the same `area` in the same function — the unit guard should have collapsed that difference, and if it has not, the cause is elsewhere. Also still unconfirmed: that the file lands in SCHEDULES rather than MISC. |
