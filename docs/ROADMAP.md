...
| PARAMTYPE-3 | **OPEN — measured 2026-09-17, baselined** | **332 parameters in `FAMILY_PARAMETER_BINDINGS.csv` declare a `DataType` that disagrees with `MR_PARAMETERS.txt`, every one under the SAME GUID** — e.g. `HVC_VEL_MPS` TEXT vs NUMBER, 160-odd `CST_*` declared CURRENCY/VOLUME against TEXT, `RGL_*_BOOL` declared TEXT against YESNO. That file is read by `BatchAddFamilyParamsCommand`, which runs inside `ProjectSetupCommand` and `MasterSetupCommand` and creates **real family parameters** from it — so a wrong type here becomes a family that Revit refuses to load. `tools/check_shared_param_types.py` baselines the count at 332: may shrink, never grow. Fixing them is its own job, and needs a decision per parameter about which file is right. |
