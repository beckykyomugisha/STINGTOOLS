...
| ID | Gap | Why open |
|---|---|---|
| ~~HC-DEF-01~~ | ~~**Radiation shielding write-back.** `RadCalc*` commands compute but do not persist onto a barrier element.~~ | **CLOSED (Phase 199)** — `RadShieldWriteBackCommand` (`Healthcare_RadShieldWriteBack`) stamps the computed design onto a user-selected barrier (Wall/Door/Window/Generic Model), gated through `RadiationSignoffGate`: `RAD_SHIELD_STATUS_TXT` = APPROVED only with a QE on record, else DRAFT; `RAD_LEAD_REQ_MM_NR` (computed) + audit stamp (`RAD_COMPUTED_DT`/`RAD_COMPUTED_BY_TXT`); provided `RAD_LEAD_MM_NR` seeded only when empty. |
| HC-DEF-01b | **`RadShieldValidator` true barrier-distance geometry.** *(PARTIAL, Phase 199)* Per-element `RAD_DISTANCE_M_NR` capture now ships via the HC-DEF-01 write-back workflow (element param → panel → conservative project-tunable default, with a `RAD.DIST.ASSUMED` warning). Deriving the **true** 3D source-to-barrier distance from model geometry is still deferred. | Real distance needs source placement + barrier-face geometry; a fragile geometry guess is deliberately NOT shipped (spec guardrail). The conservative default + per-element capture stand. |
| HC-DEF-02 | **`RadCalcLinacVaultCommand` is a first-pass NCRP-151 estimate.** No occupancy/use/distance factors; maze = flat 40% rule-of-thumb; neutron component is narrative-only. | Indicative only — the full LINAC vault calc belongs with the Qualified Expert. Deferred in Phase 199: a data seam is not shipped because a coefficient overlay behind a static calculator is fragile and risks a guessed model; output stays DRAFT/QE-gated. |
| HC-DEF-03 | **`AdvancedRadShield` PET/SPECT/Brachy** use single-TVL constants (no build-up / scatter); NCRP-147 Archer coefficients are approximate digitisations. | High-energy photon shielding needs a fuller model; current output flags Pb-only designs for QE review rather than certifying thickness. Deferred in Phase 199 for the same reason as HC-DEF-02 — no fake certification-grade physics. |
| ~~HC-DEF-04~~ | ~~**`AdjacencyValidator` uses centroid distance**, false-flagging corridor-connected rooms.~~ | **CLOSED (Phase 199)** — replaced with a door/room-graph BFS via `RoomGraphBuilder` ("N doors apart"): mandatory (2) flags > 1 door, preferred (1) flags > 3 doors, forbidden (0) flags ≤ 2 doors. Centroid kept as a labelled fallback for rooms with no door connectivity. **Limitation (Phase 199 audit):** `RoomGraphBuilder` scans the active document only — rooms/doors in linked (federated) models are silently ignored, so the check degrades to the centroid fallback for link-hosted rooms. Federated-model door-graph traversal is a follow-up. |
| HC-DEF-05 | **Twin `BacnetReadback` / `OpcUaReadback` return empty.** *(SEAM shipped, Phase 199)* `TwinTransportRegistry` lets a host register a live BACnet/OPC-UA adapter behind the existing `TwinReadbackBase` contract (discovered by protocol; `Resolve()` never null). | Pluggable transport seam shipped; the **live adapter is external** — a 3rd-party BMS stack is deliberately kept out of the plugin assembly. |
...
| HC-DEF-07 | **Med-gas diversity factors for N₂ / CO₂ / He / dental.** *(SEAM shipped, Phase 199)* `MgasDiversityRegistry` (`HEALTHCARE_MGAS_DIVERSITY.json` + `<project>/_BIM_COORD/mgas_diversity.json`) lets a project supply the authoritative factor; `MgasFlowSolver` honours it, else the flagged 1.0 fallback. | Mechanism shipped; the **factor values are project-supplied** (HTM 02-01 Pt A Table 8 / NFPA 99 Table 5.1.13.3.4) — none guessed in-repo. |
| ~~HC-DEF-08~~ | ~~**HBN adjacency vocabulary is department-level**, misaligned with canonical room classes; `HEALTHCARE_ADJACENCY_HBN.csv` unused.~~ | **CLOSED (Phase 199)** — `AdjacencyTargetsRegistry` makes the CSV the live source; `AdjacencyValidator` groups rooms by both canonical code and `RoomClassCodes` department, so `IMG-CT` resolves the IMAGING rule. Target values preserved. |
| HC-DEF-09 | **`FgiAdoptionTracker.ResolveSeverity` has zero callers.** *(SEAM shipped, Phase 199)* `FgiAdoptionContext` reads `PRJ_ORG_HEALTH_FGI_JURISDICTION_TXT` + `PRJ_ORG_HEALTH_DESIGN_FREEZE_DT`, loads a finding→clause map (`HEALTHCARE_FGI_CLAUSE_MAP.json` + project override) and escalates Warning→Error for adopted clauses; wired into `AntiLigatureValidator`. | Mechanism shipped; the **jurisdiction, freeze date and clause mapping are project-supplied** — empty by default (behaviour unchanged), no mapping guessed. **Follow-up:** only `AntiLigatureValidator` currently opts into `FgiAdoptionContext.Escalate`; extending the opt-in to other clearly FGI-governed findings (e.g. behavioural-health / patient-room rules) is a future step — the seam is one-way and inert until a clause map is supplied, so opting more validators in is low-risk. |
| ~~HC-DEF-10~~ | ~~**Clinical-equipment COBie / SFG20 handover export not built**; the `CEQ_CLINICAL` cluster was consumed by nothing.~~ | **CLOSED (Phase 199)** — `ClinicalCobieBridge` (invoked from `COBieHandoverExportCommand`, plus a `Healthcare_CobieClinical` entry) reads the `CEQ_*` cluster off clinical elements, resolves each to its COBie TypeCode via `COBIE_TYPE_MAP`, and emits Attribute/Job/Spare rows from the ships-already `CEQ-*` CSV pack. The `CEQ_*` cluster is no longer orphaned. |
...
- ~~**`CEQ_CLINICAL` cluster**~~ → **CONSUMED (Phase 199, HC-DEF-10)** — `ClinicalCobieBridge`
  now reads the `CEQ_*` cluster (decon method, endoscope reprocessing AER/cycle/last-repro/
  scope-id, GMDN + UMDNS, SFG20 ref, infection tier, imaging load) into the COBie handover
  export. No longer orphaned.
...
- ~~**`FgiAdoptionTracker`** table~~ → **WIRED (Phase 199, HC-DEF-09)** — invoked via
  `FgiAdoptionContext` from `AntiLigatureValidator`; jurisdiction / freeze date / clause
  mapping are the project-supplied inputs.
