...
| LIGHTGRID-2 | **OPEN — the same defect, measured across the suite** | Fixing LIGHTGRID-1 prompted the obvious question. **86 files collect `OST_Rooms` and never `OST_MEPSpaces`.** Most are legitimately about Rooms (finishes, room data sheets, architectural QA). The concerning subset is the **lighting suite, where all seven are hard-locked to `OfType<Room>()` with zero `SpatialElement` usage**: `ComCheckExport`, `EmergencyLightingAudit`, `LightingCalcSheet`, `LightingControlZone`, `LightingPowerDensity`, `QuickLuxEstimate`, `PhotometricDesignReview`. On an MEP model each silently does nothing, exactly as the grid command did. They are **not** bulk-fixed here: each has its own downstream use of `Room` (number, boundary, finishes) that has to be checked for Space-safety one at a time, and seven untested edits would be worse than one proven template. LIGHTGRID-1 is that template. |
