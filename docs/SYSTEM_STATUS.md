...
| 1 | **Geofence** enforcement | 🟡 PARTIAL (enforces; seed FIXED) | Enforcement works (out-of-bounds → 403, proven). Fixed real seed bug (PR #300): NHW boundary stored `[lat,lon]` but `GeofenceValidationService` reads GeoJSON `[lon,lat]` → box landed off W. Africa so no London point was ever inside. **Caveat:** after hand-correcting the live row, an in-box create still 403'd — `GET /projects/{id}` returns `boundaryPolygon:null` while the geofence read sees a value, i.e. an inconsistent/cached boundary read worth a deeper look; needs a fresh seed/restart to confirm the accept path. Issue lifecycle was proven on a non-geofenced project instead |
