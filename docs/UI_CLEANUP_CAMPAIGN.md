...
**Z-25 — Timber biogenic carbon: USER CHOSE PATH C (split fossil + biogenic columns)** 📤 prompt ready in `PHASE_Z_NUMERIC_FOLLOWUPS.md`
Discovered during Z-20 — the agent stopped at timber: `BLE timber −900 kgCO₂/m³ is biogenic-inclusive; mixing it with the now-gross steel/concrete makes totals misleadingly low. Need a decision.` **Decision made: Path C** — split into `PROP_CARBON_FOSSIL_KG_M3` + `PROP_CARBON_BIOGENIC_KG_M3` (legacy net `PROP_CARBON_KG_M3` = fossil + biogenic, kept for backwards-compat). Matches RIBA 2030 / LETI / RICS WLCA separated-reporting convention. ~1d: schema change across material CSVs + resolver `GetCarbonFossil`/`GetCarbonBiogenic` API + tests. Coordinates with Z-24b (timber LOOKUP rows must carry the split). Will MOVE delivered carbon totals for any report summing timber — that's the point (gross totals stop being misleadingly low); deltas flagged for sign-off.
