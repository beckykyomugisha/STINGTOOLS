...
**Z-23b — Rebar lap + concrete over-order as opt-in measured additions** ✅ MERGED to main (commit `b292b9b95`→rebased `dc960fc2c`, branch `feat/qs-rebar-lap-concrete-buffer`) — done by a QS specialist agent
**Professional NRM2 call (cited):** rebar lapping is a **MEASURED ADDITION, not waste** — NRM2 measures reinforcement by mass of steel in the finished work; design laps are *added* to that quantity, while cutting/offcut waste is the separate 5% `COST_DEFAULT_WASTE_PCT`. Lap ≠ waste (distinct percentages, different accounting categories) so a lap allowance on top of the 5% is NOT a double-count. Concrete over-order is a **procurement BUFFER, not measured waste** — NRM2 measures concrete net; spillage/over-excavation/pump-priming is deemed in the rate / general waste, so the buffer is opt-in OFF by default (the 5% already covers spillage; defaulting on would double-count).
**Implementation:** two `TagConfig.GetConfigDouble` knobs mirroring COST_DEFAULT_WASTE_PCT — `REBAR_LAP_ALLOWANCE_PCT` (default **0/OFF**, NRM2-typical 8-12% on lapped members) + `CONCRETE_OVERORDER_PCT` (default **0/OFF**, typical +5%). New pure `StingTools/BOQ/MeasuredAddition.cs` — `GrossUp` sums the distinct allowances and applies them ONCE to the net base (`qty = base × (1 + (waste% + addition%)/100)`), never a second waste pass; applied only on the rebar (kg/tonne) + concrete (m³) branches in `DeriveQuantity`, **after and separate from** the WasteFactor step. With knobs at 0, `GrossUp(...,0)` is bit-for-bit identical to `WasteFactor.Apply` — a no-op.
**No-double-count PROOF (the key test):** `RebarWastePlusLap_EachAppliedOnce_NotCompoundedAsSecondWaste` — 1000 kg @ 5% waste + 10% lap → asserts **1150** (additive 15%), AND `≠ 1155` (1.05×1.10 compounded), AND `≠ 1102.5` (1.05² double-waste). +9 tests → 72 total, build 0/0.
**Defaults OFF → zero delivered-number change in this PR** (the anti-double-count guarantee; a project opts in per job). Self-corrected during work (initially slipped the commit onto the carbon branch, reset it, rebuilt clean off current main).
