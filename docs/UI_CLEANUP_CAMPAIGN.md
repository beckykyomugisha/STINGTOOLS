...
**Z-25b — Surface fossil / biogenic / net in the carbon report** 📤 NEW (the deliverable Path C enables)
Path C added the split DATA + resolver API but nothing RENDERS it yet — the BOQ carbon report / dashboard still shows the single net number. Z-25b wires `GetCarbonFossilPerM3` / `GetCarbonBiogenicPerM3` into the carbon report UI to show the RIBA 2030 / LETI three-line breakdown (A1-A3 fossil + A1-A3 biogenic + net), and decides whether delivered totals should use fossil-only (gross, the conservative WLCA default) or net (with sequestration). **This is where the actual delivered-number decision lives** — Path C deliberately left it gated so the report PR owns the "does our headline carbon include sequestration?" call. Needs the same carbon-engineer / QS sign-off that Z-25's path-choice did.
