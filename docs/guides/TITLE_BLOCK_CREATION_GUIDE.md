...
> ### What changed in Jul 2026 — Value bindings, resolution, per-size placement, slot accuracy & a new Part 2
>
> 1. **MEP `SYSTEM` cell** (`PRJ_SHEET_SYSTEM_TXT`) on every working sheet, auto-filled via the `{sys}` token — see the new **§22**. (P4)
> 2. **Fabrication cells populate** — SPOOL # / WEIGHT / FAB LOC / STATUS / BOM REV are real param-bound labels written by the composer. (P9)
> 3. **Per-size SYSTEM + DISCIPLINE** render once at each paper size's own coordinates (id-based label override). (P10)
> 4. **Leaf-wins inheritance** — size bases now correctly override the A1 root; A0/A3 no longer print "A1" in the PAPER cell. **Rebuild families (`TitleBlock_CreateAll`) to pick this up.** (P11)
> 5. **Logical→concrete family resolution** — `TitleBlockResolver` maps `STING_TB_SHEET_A1` → the real `STING_TB_A1_BIM_v2.0` by size/orientation/mode. See **§23.7**. (P5)
> 6. **Slot accuracy** — fractional coordinates, fit-to-slot scaling, one drawable-rect reference frame, LocationPoint offset. See the new **§23**. (P12)
> 7. **New Part 2** — dedicated sections on **MEP (§22)**, **Slots (§23)** and an **end-to-end usage tutorial (§24)**; the complete 75-parameter shared file at `Data/STING_TITLE_BLOCK_PARAMETERS.txt`.
> 8. **Catalogue correction** — the §4.1 "15-kit" table is *conceptual*; the real catalogue is 25 families named by size + BIM mode. IFC/IFT/As-Built are *suitability states*, not families; A2/A4 are not shipped. See the callout in §4.1.
>
> **Note:** items 5 & 6 (P5/P12) are landed in code but **pending an in-Revit acceptance sweep** — described as intended design; verify on your model.
...
**Part 2 — MEP, Slots & Using the Kit (Jul 2026)**

22. [MEP in depth — title blocks for M / E / P](#22-mep-in-depth--title-blocks-for-mechanical-electrical--public-health)
23. [Slots — deep dive](#23-slots--deep-dive)
24. [Using the kit — end-to-end layman tutorial](#24-using-the-kit--end-to-end-layman-tutorial)
...
> ### ⚠ Read this first — the table below is the *conceptual* kit, not the current file names
>
> The table lists **one family per purpose** to explain the *ideas*. The
> **actual built catalogue** (`STING_TITLE_BLOCKS.json`, generated by
> `TitleBlock_CreateAll`) is **25 concrete families** on a different axis — read
> §23.7 for how the two connect. Key differences to keep straight:
>
> - **Working sheets are named by size + BIM mode, not by purpose:**
>   `STING_TB_<SIZE>[_PORT]_<BIM|NONBIM>_v2.0` — i.e. A0 / A1 / A3 × Land / Port ×
>   BIM / NONBIM = **12 families**. A `mep-plan` and an `arch-plan` share the *same*
>   A1 border; the discipline/system live in cells, not in the file name.
> - **IFC / IFT / As-Built are *suitability states*, not families.** You do **not**
>   build a `STING_TB_IFC_A1.rfa`. You issue the ordinary BIM family with
>   `PRJ_DWG_SUITABILITY_COD_TXT` / status set to the issue purpose — the banner
>   and CDE state follow from that.
> - **Fabrication (4):** `STING_TB_ASSEMBLY_{PIPE\|DUCT\|COND\|HANGER}_v1.0` — all
>   **A1 landscape** (the "HANGER = A2" in the row below is outdated).
> - **Specialty:** `STING_TB_PRESENT_A1_v1.0` (+`_MONO`), `_COVER_A1`, `_DIVIDER_A1`,
>   `_REGISTER_A1`, `_SUBMISSION_{KCCA\|ERA\|NEMA}`, `_CLARIFICATION_A3`.
> - **No A2 / A4.** The `MARKETING_A2` row is superseded by the A1 presentation
>   family; A2/A4 sizes are intentionally not shipped.
>
> So use the descriptions below to understand *what each surface is for*, but for
> the real file name a recipe resolves to, trust §23.7 and `STING_TITLE_BLOCKS.json`.
...
| `PRJ_SHEET_SYSTEM_TXT` | **MEP system / service** printed in the SYSTEM cell (`HVAC`, `DCW`, `DHW`, `SAN`, `LV`, `LTG`, `MGPS` …). Auto-filled from the recipe's `system` field via the `{sys}` token; blank for non-MEP. See §22.1. |

> **The complete parameter file:** all **75** title-block shared parameters
> (`PRJ_ORG_*`, `PRJ_TB_*`, `PRJ_SHEET_*`, `STING_SHEET_*`, `ASS_*`) are packaged,
> grouped and ready to load, at **`Data/STING_TITLE_BLOCK_PARAMETERS.txt`** —
> *Manage ▸ Shared Parameters ▸ Browse*, bind to **Title Blocks** (instance).
...
> The sections above teach you to **author** title blocks. Part 2 teaches
> the three things beginners ask about most: how MEP disciplines use title
> blocks, how the **slot** system decides where views land, and a single
> end-to-end walkthrough from an empty project to an issued MEP sheet.
>
> **Verification note:** the slot-accuracy features in §23 (fit-to-slot,
> fractional coordinates, family-name resolution) landed in code (P5/P12) but
> are **pending an in-Revit acceptance sweep** at the time of writing. Treat
> the behaviour described as the intended design; confirm on your own model.

## 22. MEP in depth — title blocks for Mechanical, Electrical & Public-Health

Most of this guide treats a title block as discipline-agnostic — and for the
**border itself** that is correct: an MEP plan uses the *same* A1 sheet border
as an architectural plan. The discipline is a *value* printed in a cell, not a
different family. MEP does, however, need three things the other disciplines
don't. This section covers each.

### 22.1 The SYSTEM cell (what MEP engineers always want on the sheet)

Every working-sheet family carries **`PRJ_SHEET_SYSTEM_TXT`** — a small cell,
next to DISCIPLINE, that prints the MEP **system / service** the sheet is about:
`HVAC`, `DCW` (domestic cold water), `DHW`, `SAN`, `HWS`, `LV`, `LTG` (lighting),
`MGPS` (medical gas), and so on. It is **blank for architectural / structural**
sheets, so it costs nothing there.

- **It auto-fills.** You do *not* type it per sheet. The drawing-type recipe
  carries a `system` field, and the engine substitutes it through the `{sys}`
  token into `PRJ_SHEET_SYSTEM_TXT`. So a `plumb-dcw-schematic` sheet prints
  `DCW`, a `mep-hvac-duct` sheet prints `HVAC` — automatically.
- **One vocabulary.** The system codes are the *same* codes the element tags
  use (the SYS segment — `TagConfig.SysMap`). Do **not** invent new codes; a
  sheet labelled `DCW` then matches the pipes tagged `DCW`.

*Layman analogy:* DISCIPLINE says "this is a plumbing drawing"; SYSTEM says
"…and specifically the cold-water system." A plumber flipping through 40 sheets
finds the cold-water riser by the SYSTEM cell, not by reading every pipe.

### 22.2 Three kinds of MEP sheet — three title-block routes

| MEP sheet kind | Title-block family used | What auto-fills |
|---|---|---|
| **Design** (plans, sections, risers, schematics) | the ordinary working family (`STING_TB_A1_BIM_v2.0` …) + SYSTEM cell + discipline colour band | project identity, DISCIPLINE, SYSTEM |
| **Fabrication** (spools) | dedicated `STING_TB_ASSEMBLY_{PIPE\|DUCT\|COND\|HANGER}_v1.0` | SPOOL # / WEIGHT / FAB LOC / STATUS / BOM REV (from the fab composer, §7.3) |
| **Schedule** (equipment, panel, valve) | a working family hosting a `ViewSchedule` | the schedule fields themselves |

The key beginner insight: **only fabrication uses a special MEP border.** Design
and schedule sheets reuse the ordinary border and let the SYSTEM/DISCIPLINE
cells carry the MEP meaning.

### 22.3 Per-discipline title-block selection

The **Project Setup Wizard** (Disciplines page) has a title-block dropdown *per
discipline* — Mechanical, Electrical, Plumbing, Fire, LV, General. Leave them on
`(Default)` and every discipline shares one working family (recommended — the
DISCIPLINE + SYSTEM cells + colour band already distinguish them). Override only
when an office wants, say, a distinct electrical border. The producer then picks
the right family per sheet automatically (see §23.7).

### 22.4 MEP schedule sheets

Three MEP schedule drawing types produce a ready-fielded schedule on a sheet:

- **Mechanical-equipment schedule** — AHUs / FCUs / pumps with duties.
- **Electrical-panel schedule** — the panel's circuits.
- **Valve schedule** — valve tag / type / size / service.

They mirror the existing door/window schedule recipes — pick the recipe, produce,
and the `ViewSchedule` lands on the correct sheet type with the SYSTEM cell set.

### 22.5 Worked example — issue an HVAC plan, a DCW schematic and a pipe spool

1. **HVAC plan.** Produce with the `mep-hvac-duct` recipe → engine resolves the
   A1 working family, places the plan in the main slot, stamps DISCIPLINE = `M`,
   SYSTEM = `HVAC`, sheet name `M ... HVAC Plan`. Issue as normal.
2. **DCW schematic.** Produce with `plumb-dcw-schematic` → NTS drafting view in
   the main slot, DISCIPLINE = `P`, SYSTEM = `DCW`.
3. **Pipe spool.** Run the fabrication workspace → the composer creates the spool
   views, places the `STING_TB_ASSEMBLY_PIPE_v1.0` border, and fills SPOOL # /
   FAB LOC / STATUS / BOM REV. *(WEIGHT populates once the composer numeric-write
   fix lands — see the CHANGELOG.)*

## 23. Slots — deep dive

A **slot** is a rectangle on the sheet where the engine drops a viewport. This is
the single most important concept for automated sheet production, and it has
evolved — here is the current model in plain language.

### 23.1 What a slot carries

Each slot has: an **id** (`S01`…`S07`, `KP`), a **position + size** (where and how
big), a **`purposeTag`** (what it's *for* — `main-plan`, `key-plan`, `section`,
`schedule`, `legend`), and optional **`viewportType`** / **`scaleHint`**.

### 23.2 purposeTag routing — placement by meaning, not by number

The old way matched view #1 → slot #1 by position. The current way matches by
**meaning**: a view whose purpose is `main-plan` lands in the slot tagged
`main-plan`, wherever that slot physically is. `Data/STING_VIEWPORT_PLACEMENT_RULES.json`
maps `(viewType, name pattern) → purposeTag`. This is why the same recipe works
across different borders — it asks for "the main-plan slot," not "slot S01."

### 23.3 Absolute vs fractional coordinates (P12)

Historically slot positions were **absolute millimetres**, authored once *per
paper size*. That is why an A0 and an A3 needed their own copies — and why a cell
placed only on A1 leaked onto A0/A3 (the bug §P10/P11 fixed).

Slots can now be authored as **fractions (0..1) of the drawable zone**. A slot at
`fracAnchor [0.03, 0.05]`, `fracSize [0.94, 0.90]` means "3 % in from the left,
5 % up, 94 % wide, 90 % tall" — and that **one definition scales correctly to
A0, A1 and A3 with no per-size copies.** Absolute coordinates still work (they're
the fallback); fractional is the sustainable way to author new families.

### 23.4 Fit-to-slot scaling (accuracy)

Before, the engine placed a view at the slot's *centre* and ignored the slot's
size — so a big plan could overflow its slot. Now the engine reads the slot's
width/height, computes the scale at which the view **fits inside the slot**, and
rounds up to the next standard scale (1:50, 1:100 …). `scaleHint` acts as an
override/floor. This applies only to cropped graphical views — schedules,
legends and 3D views are left alone.

### 23.5 The drawable rect — one reference frame (P12.B)

Every size base now declares a structured **`drawable`** rectangle (x, y, w, h in
mm, plus a margin), origin at the sheet's bottom-left. Both placement paths —
purposeTag and fractional — resolve against this *one* rectangle, so a slot lands
in the same place regardless of how it was authored. Legacy families with no
`drawable` rect fall back to the historic "title-block bounding box − 25 mm."

### 23.6 Per-size placement & leaf-wins inheritance (P10/P11)

Families inherit from a common base via `extends`. The inheritance is
**leaf-wins**: a size base (A0/A3/portrait) that re-declares a cell or slot
**overrides** the base's version instead of duplicating it. This is what lets the
SYSTEM and DISCIPLINE cells sit at each paper size's own coordinates, and what
stopped A0/A3 sheets from printing "A1" in their PAPER-SIZE cell.

### 23.7 How family names resolve (P5)

Drawing-type recipes name a *logical* family — `STING_TB_SHEET_A1` — not the real
built name. **`TitleBlockResolver`** turns the logical name (or a blank field)
into the concrete built family using paper size + orientation + BIM mode:
`STING_TB_SHEET_A1` → `STING_TB_A1_BIM_v2.0`, an A3 portrait →
`STING_TB_A3_PORT_BIM_v2.0`, a spool → `STING_TB_ASSEMBLY_PIPE_v1.0`. It loads the
`.rfa` from `Families/TitleBlocks/` on demand, so a project that ran
`TitleBlock_CreateAll` once never hits "family not found."

## 24. Using the kit — end-to-end layman tutorial

The shortest real path from an empty project to an issued MEP sheet.

**Step 0 — Prerequisites.** STING installed; a Revit project open; your levels and
a scope box or two in place.

**Step 1 — Build the families.** Run **`TitleBlock_CreateAll`**. This reads the
JSON specs and generates every `.rfa` into `Families/TitleBlocks/`. *You must
re-run this whenever the title-block parameters change* — instance parameters
added by an update only exist on freshly generated families.

**Step 2 — Load the shared parameters (once).** *Manage ▸ Shared Parameters ▸
Browse* → `Data/STING_TITLE_BLOCK_PARAMETERS.txt` (all 75 title-block parameters,
grouped). Bind to the **Title Blocks** category, **instance**.

**Step 3 — Set up the project.** Run the **Project Setup Wizard**. Fill project
identity (feeds `PRJ_ORG_*` / `PRJ_TB_*` → the title-block cells), pick your
disciplines, and leave the per-discipline title-block dropdowns on `(Default)`
unless you need a bespoke border.

**Step 4 — Produce an MEP sheet.** Pick the recipe for what you're drawing (e.g.
`mep-hvac-duct` for an HVAC plan) and produce. The engine resolves the concrete
family (§23.7), places the view in the main-plan slot fitted to size (§23.4), and
stamps DISCIPLINE + SYSTEM.

**Step 5 — Issue.** Set the deliverable status / revision and issue. The
transmittal + revision cells fill from `PRJ_TB_*`.

> **Common beginner mistakes**
> - *"The SYSTEM / fab cells are blank."* → You didn't re-run `TitleBlock_CreateAll`
>   after an update, so the loaded family lacks the new parameters. Re-run it,
>   reload the family.
> - *"My A0 sheet says A1 in the paper cell."* → An old family built before the
>   leaf-wins fix. Rebuild with `TitleBlock_CreateAll`.
> - *"The view overflows its slot."* → An old family without a `drawable` rect, or
>   an uncropped view (fit-to-slot needs an active crop box).
> - *"Family not found during production."* → Run `TitleBlock_CreateAll` once so
>   the `.rfa` exists on disk for the resolver to load.
