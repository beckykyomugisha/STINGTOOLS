...
| File | Effect |
|---|---|
| `_BIM_COORD/owner_standards.json` | Enables the `KUT-ZZZ-XX-XX-M3-A-0001` sheet-number rule; narrows discipline codes to the temple team (A/S/M/E/P/FP/LV/G); adds the `ffe-fohlio-ref` FF&E link check — **enabled, severity WARN** (non-blocking: it reports FF&E elements not yet linked to Fohlio, it does not fail a gate) |
