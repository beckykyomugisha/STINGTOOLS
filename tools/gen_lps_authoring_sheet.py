...
w("tier already does.")
w("")
w("> **The FIRST row's Spaces and the LAST row's Break are not editable.**")
w("> Revit greys both out. Spaces counts the gap to the PRECEDING parameter, so")
w("> row 1 has nothing to space from and sits at 1; Break ends a line, so the")
w("> last row has nothing to break before. The printed values for those two rows")
w("> are what Revit fixes them to, not something to set.")
w("")
